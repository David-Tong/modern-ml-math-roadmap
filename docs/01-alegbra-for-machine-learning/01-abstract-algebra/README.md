...
从普通工程视角看，神经网络是一堆层、一堆参数、一堆矩阵乘法和激活函数。

从代数视角看，可以换一种说法：

> 神经网络是在一串表示空间之间不断做变换。

先看一个具体的小网络：它用来做 10 类图片分类。输入是一张 $28\times 28$ 的灰度图片，压平成 784 个数字；中间用一个 128 维隐藏表示；最后输出 10 个类别分数。

![神经网络中的表示空间与变换](assets/deep-network-representation.svg)

现在在这个例子上统一术语。

**表示**：就是网络在某一层拿来描述输入的那组数字。$x_0$ 是输入表示，$x_1$ 是隐藏表示，$x_2$ 是输出表示。比如做图片分类时，$x_0$ 可以是像素，$x_1$ 可以是模型提取出的特征，$x_2$ 可以是各个类别的分数。

**表示空间**：可以先理解成这些表示所在的向量空间。比如输入图片的像素常常被归一化到 0 到 1 之间，所以真实输入样本通常落在 $[0,1]^{784}$ 这个区域里；但为了做线性代数运算，我们把它所在的表示空间看成 $\mathbb{R}^{784}$。隐藏表示 $x_1$ 往往是真实数值特征，可以看成在 $\mathbb{R}^{128}$ 里；输出表示 $x_2$ 可以看成在 $\mathbb{R}^{10}$ 里。大白话说，表示空间主要说明“这些表示有多少个坐标，并且我们允许用什么数域来描述这些坐标”。

**变换**：就是把一个表示变成下一个表示的规则。$T_1$ 把输入表示 $x_0$ 变成隐藏表示 $x_1$，$T_2$ 把隐藏表示 $x_1$ 变成输出表示 $x_2$。在真实网络里，$T_1,T_2$ 可以由 Linear、ReLU、Conv、Attention、LayerNorm 等模块组成。
...
其中每个 $T_i$ 都可以看成作用在表示上的变换，只是具体规则不同。

这里要小心一个词：机器学习里的“表示”和抽象代数里的“表示论”不是同一个概念。机器学习里的表示，通常指模型学到的特征向量；表示论研究的是怎样把抽象操作变成矩阵或线性变换。它们的联系在于：两边都会把问题放到向量空间里看，并且关心变换怎样作用在这个空间上。这里先不展开表示论，只说明这个连接。

在这个具体网络里，宽度和深度也更好理解：

- **宽度**：某一层表示空间的维度。比如 128、64 就是隐藏层宽度，它决定这一层能同时保存多少个特征数字。
- **深度**：从输入到输出连续做了多少轮变换。这个例子里，$T_1$ 和 $T_2$ 组成了一条从输入表示到输出表示的加工链。

这就是代数视角有用的地方。它不会只盯着某一个参数是多少，而会问：
...
这些问题一问出来，神经网络就不只是“经验上有效的工程装置”，而开始显出代数结构。下面的线性层例子，就是这个视角最简单也最干净的入口。

### 纯线性网络：为什么多层会合并成一层

再看一个更贴近机器学习的例子：神经网络里的线性层。

先只看没有激活函数的线性层。一个线性层可以写成矩阵：
...
如果所有线性层的输入输出维度都相同，比如都是 $n$ 维到 $n$ 维，那么这些层对应的就是所有 $n\times n$ 矩阵。

这些线性层之间有两种自然操作。

第一种是相加。两个线性层 $W_1,W_2$ 相加，就是矩阵相加：

$$
(W_1+W_2)x=W_1x+W_2x
$$

第二种是“相乘”。这里的乘法不是逐元素相乘，而是层的复合：先做 $W_1$，再做 $W_2$，整体效果是：

$$
W_2(W_1x)=(W_2W_1)x
$$

也就是说，两个线性层连续做，等价于一个新的线性层。

把这两种操作翻译成神经网络结构，就更直观了：

![神经网络设计里的加法与复合](assets/neural-ring-design.svg)

- **相乘 / 复合**：对应把层串起来。先做第一层，再做第二层，再做第三层。网络的“深度”主要来自这种连续复合。
- **相加 / 汇合**：对应把几条路径的输出加在一起。残差连接、跳跃连接、某些多分支结构，本质上都在使用这种“把路径合并”的动作。

所以这里的“加法”和“乘法”不是为了好看，而是直接对应两类网络设计动作：**乘法让变换一层接一层地复合，加法让多条计算路径重新汇合**。

这不是从“环”硬推出的结论，而是矩阵乘法本身已经告诉我们的事实。
...
它看起来有三层，但从函数表达能力上看，仍然只是一层线性变换。这里真正重要的是：**线性变换连续复合以后，还是线性变换**。

这时我们再回头看环。所有 $n\times n$ 矩阵，也就是所有从 $\mathbb{R}^n$ 到 $\mathbb{R}^n$ 的线性层，在“矩阵加法”和“矩阵乘法”下形成环：

- 加法部分是交换群：线性层可以相加，有零层，也有相反层。
- 乘法满足结合律：连续复合多个线性层时，括号怎么打包不影响最终线性变换。
- 乘法对加法满足分配律：先把两个线性层相加再复合，等价于分别复合后再相加。

所以环在这里不是给“矩阵乘法”换一个高深名字，而是在告诉我们：线性层的相加、复合、展开和合并，已经被同一套结构规则管住了。只要一直留在这个结构内部，不管堆多少层，本质上仍然是一层。

这个视角真正有价值的地方，是它把网络设计里的边界指出来了：有些复杂度只是形式上变深，最后会被结构吸收掉；有些复杂度会改变结构，带来新的表达能力。纯线性层连续相乘属于前一种，中间插入 ReLU、Sigmoid、GELU 这类非线性环节属于后一种。非线性激活的关键作用，就是让整体变换跳出“一个矩阵就能合并”的范围。

从这个角度看，非线性激活不是装饰，而是在结构上打断“纯线性复合会合并成一层”这件事。线性层负责把特征重新组合，非线性负责改变可表达函数的类型。深度网络真正有力量，来自“线性变换”和“非线性变换”交替复合，而不是来自矩阵自己重复相乘。

所以代数视角不是替我们调学习率、选 batch size，而是先逼我们问结构问题：这几层连续做以后，会不会其实等价于一层？这几条路径是应该串联，还是应该相加？有没有必要保留一条恒等路径，让信息可以直接通过？这些问题会直接影响网络结构设计。

### 残差连接：从 $I+W$ 看“保留原样 + 学习改动”

再看残差连接。这里不是说整个非线性 ResNet 都是环，而是说：我们可以先借用线性层环的语言，看清残差块最基本的代数骨架。下面这张图采用 ResNet 论文里的标准示意：主路径经过若干 weight layer，旁边有一条 identity shortcut，最后在加法处汇合。
...
这里的 $I$ 是恒等层，表示什么都不改；$W$ 是一个线性层，表示对输入做一次改动。所以 $I+W$ 的大白话就是：

> 在“保留原输入”的基础上，再加上一点可学习的变化。

这正好用到了环里的两种操作：

- 加法：把“原输入”这条路和“线性变换”这条路加起来。
- 乘法：把多个残差块连续接起来。
...
$$
(I+W_2)(I+W_1)=I+W_1+W_2+W_2W_1
$$

这就是分配律在起作用。它说明两个残差块堆起来以后，不只是“第 1 层加第 2 层”，而是自动包含了几类路径：

- $I$：什么都不做的直通路径。
- $W_1$：只经过第一层变化。
- $W_2$：只经过第二层变化。
- $W_2W_1$：先经过第一层，再经过第二层的组合变化。

如果有三层，就会出现直通项、单层项、两层复合项、三层复合项。也就是说，残差结构把一个单纯的长链条，变成了“保留原样 + 各种改动组合”的展开。

这就是环结构给出的视角：加法让不同路径相加，乘法让层和层连续复合，分配律把残差块的堆叠展开成很多组合项。

这也解释了为什么这个代数视角能引导结构设计。普通串联网络主要依赖“乘法”：一层接一层，信息必须穿过整条长链。残差网络额外加入“加法”：一条路径保留原输入，另一条路径学习改动，然后两条路径相加。这样网络不必每一层都重新发明完整变换，而可以学习“在原来的表示上修正一点”。
...
这里的 $\mathcal{F}$ 对应图中的主路径。在卷积网络里，它往往不是一个单独矩阵，而是一小串卷积、归一化、激活函数等模块。严格说，整个非线性 ResNet 不能简单说成一个环，因为非线性复合不一定满足环定义里的分配律。

但环的线性版本仍然给了一个很有用的骨架：残差块不是让输入完全被一层新变换覆盖，而是始终保留一条“原样通过”的路径。哪怕 $\mathcal{F}$ 学到的改动很小，$x$ 这条直通路径还在。这就是残差连接帮助深层网络更容易训练的一个代数直觉。

这个模型也能帮助我们理解网络的宽度和深度。

宽度说的是一层里有多少通道、多少坐标、多少特征位置。宽度越大，同一层能并排保存的信息越多。用代数语言说，就是这一层所在的向量空间维数更大。

深度说的是变换做了多少轮。层数越多，后一层就能在前一层已经形成的特征上继续加工，形成更长的组合链。在线性情况下，这条组合链会合并成一个矩阵；但在线性层和非线性激活交替出现以后，复合不再能简单合并，深度就开始带来真正的表达能力。

所以不能简单说“深一定比宽好”。更准确地说，深度常常带来更高的**组合效率**：有些复杂函数如果只用很浅的网络表示，可能需要非常大的宽度；而多层网络可以逐层复用中间特征，用更少参数表达出来。

环的例子先告诉我们线性部分的基本规则：线性层可以相加，可以复合，可以按分配律展开。真正让深度网络变强的，是这些线性结构和非线性激活交替出现以后，组合不再只是合并成一个矩阵。

所以环不是只有整数、多项式、矩阵这些“纯数学对象”才有。神经网络里的线性部分，也在使用同一套结构：线性变换可以相加，也可以复合，而且这两种操作满足环的规则。残差网络这一节则进一步说明：即使真实网络里有非线性操作，环的语言仍然可以帮助我们看清它的线性骨架。
