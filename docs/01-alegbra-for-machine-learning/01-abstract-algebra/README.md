...
一个操作的意义，是它把魔方从一种状态变成另一种状态。两个操作之间也可以继续组合：先做操作 $A$，再做操作 $B$，合起来就是一个新的操作。

现在检查“所有合法魔方操作”是不是群：

- 封闭性：两串合法操作连续做，合起来仍然是一串合法操作。
- 结合律：连续做三串操作时，先把前两串打包，还是先把后两串打包，最终效果一样。
- 单位元：什么都不做，是一个操作；它不会改变魔方。
- 逆元：每个操作都能撤销。比如先转右面一层，再反方向转右面一层，就回到原状；一整串操作的逆操作，就是把步骤倒过来，并且每一步都反向做。

所以，魔方的合法操作在“连续执行”这个运算下组成一个群。

到这里，“群”已经不是一个名字，而是开始产生结论了。最直接的一条是：**任何合法打乱都可以撤销**。
...
意思是先转右面，再转上面。那么它的撤销操作不是 $R^{-1}U^{-1}$，而是：

$$
S^{-1}=U^{-1}R^{-1}
$$

也就是先撤销最后一步，再撤销前一步。这不是魔方玩家硬背出来的口诀，而是群的逆元规则逼出来的：

$$
(AB)^{-1}=B^{-1}A^{-1}
$$

这句话非常重要。它说的是：**撤销一串操作时，顺序必须反过来**。
...
从群的角度看魔方，还有更像“高招”的东西。

先用加法作类比。在普通加法里，如果你不想让一个操作留下影响，就加上它的相反数：

$$
+5-5=0
$$

魔方里也是一样。如果一个操作是 $A$，它的撤销操作就是 $A^{-1}$。所以：

$$
AA^{-1}=e
$$

这里 $e$ 表示什么都不做。也就是说，一串操作后面接上它的逆操作，整体效果就会消失。

这会带来一个很重要的套路：**搬过去，处理，搬回来**。

如果你会一个小技巧 $A$，它只能处理某个固定位置；但现在目标块不在那个位置，就可以先用 $P$ 把目标搬过去，做 $A$，再用 $P^{-1}$ 把魔方搬回来：

$$
PAP^{-1}
$$

这叫共轭。它的意思不是“多转几下”，而是把一个已知技巧搬到新的位置使用。
...
这叫交换子。它看起来像乱转，但结构很精巧：先做 $A$，再做 $B$，再把 $A$ 撤销，再把 $B$ 撤销。很多影响会互相抵消，最后只留下少数块被改变。

所以群视角下，魔方公式不是一堆要死背的动作，而是在利用三件事：

- 逆操作：把影响撤销。
- 共轭：把技巧搬到别的位置。
- 交换子：让大部分影响抵消，只留下想要的局部变化。

这就是定义群的意义：我们先证明某个具体系统满足群的四条规则，然后就能把群的一般结论搬进来。魔方里的“合法打乱一定能按反向步骤撤销”“共同前缀可以消去”，不是临时发明的技巧，而是所有群都共享的结构规则。
