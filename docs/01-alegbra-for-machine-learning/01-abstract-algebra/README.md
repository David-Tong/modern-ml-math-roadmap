...
有时候，一个群太细了。我们不想区分每个元素，而是想把一批元素看成同一类。

最熟悉的例子还是整数。

如果只关心一个整数除以 4 的余数，那么很多整数就会被看成同一类。比如：

$$
1,5,9,13,\ldots
$$

它们都除以 4 余 1，所以在“模 4”的眼光下，它们没有区别。

为什么是这些数被放在一起？因为它们两两之间都差一个 4 的倍数。

比如：

$$
5-1=4,\qquad 9-1=8,\qquad 13-1=12
$$
...
这四组就叫 $4\mathbb{Z}$ 在 $\mathbb{Z}$ 里的**陪集**。

大白话说，陪集就是：

> 拿一个子群当“步长”，把整个群按这个步长分成一组一组。

接下来，如果我们不再把每个整数当成元素，而是把这四组本身当成元素，就得到一个更粗的结构。
...
意思就是：余数 1 的那一组，加上余数 2 的那一组，会落到余数 3 的那一组。
...
> 先把原来的群分成一组一组，再把每一组当成一个新元素来运算。

所以商群不是凭空造出来的新东西，而是把原来的群“压缩”以后得到的结构。它忽略了某些差异，只保留更粗的分类。

这和前面讲等价关系的思想是连在一起的。等价关系负责“哪些元素应该看成同一类”，商群进一步问：“这些类本身能不能继续组成一个群？”

这里先不展开正规子群的技术条件。暂时只要知道：不是每个子群都能随便拿来做商群；要让陪集之间的运算定义得好，需要额外条件。这个条件后面会叫**正规子群**。
...
如果一个环里，除了 0 以外，每个元素都能做除法，那么它就更像我们熟悉的数系。

这样的结构叫**域**。

可以先把域理解成：

> 一个可以自由做加、减、乘、除的系统，当然不能除以 0。
...
因为两个有理数相加、相减、相乘、相除，只要不除以 0，结果还是有理数。
...
区分环和域，不是为了多背两个名字，而是为了知道哪些操作合法。

在域里，很多熟悉的代数操作都可以放心做。比如解一次方程：
...
但如果你坚持只在整数里解，那就没有整数解。因为 $\frac{2}{3}$ 不在整数里。

所以“能不能除”不是小细节，而是结构差异。
...
2 和 3 都不是 0，但乘起来等于 0。这说明它们不可能都有乘法逆元。直觉上，除法会出问题。
...
因此 $\mathbb{Z}/5\mathbb{Z}$ 是域。

这里先记住一个事实：当 $p$ 是素数时，$\mathbb{Z}/p\mathbb{Z}$ 是域。有限域、编码理论、密码学里会大量用到这种结构。

### 环与域这一段先抓住什么

现在可以把群、环、域的关系粗略整理成这样：
...
> 这个系统里有几种运算？这些运算满足什么规则？哪些操作能撤销？除法是否总是合法？

这就是为什么整数、多项式、矩阵、有理数、实数、复数可以被放进同一张结构地图里。
