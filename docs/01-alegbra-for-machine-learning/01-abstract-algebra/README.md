# 普通工科生也能看懂的抽象代数：用结构看数学

很多人第一次听到“抽象代数”，会以为它是把高中代数、线性代数再往难处推一层：更多符号、更多定义、更多看不出用途的证明。

但这个理解并不准确。

抽象代数真正想做的事情，不是把具体计算变得更复杂，而是换一种视角看数学对象。它关心的不是某个数具体等于多少，也不是某个公式怎样展开，而是：

- 一批对象之间可以做什么操作？
- 这些操作满足什么规则？
- 哪些对象虽然长得不一样，却有相同的规则？
- 哪些变化只是表面变化，背后的规则其实没有变？

这就是抽象代数最核心的主题：结构。

整数、矩阵、多项式、函数、旋转、置换，看起来不像同一种东西。整数可以加减乘除，矩阵可以相乘，函数可以先做一个再做另一个，图形可以旋转，座位可以重新排列。表面上，它们分别属于不同数学分支；但抽象代数会问：如果暂时不看它们的外形，只看“操作规则”，它们之间有没有共同模式？

这就是抽象代数的厉害之处。它不是只研究某一种对象，而是研究对象背后的组织方式。它把数学从“算某个东西”推进到“理解一类规则”。

## 一、换一个角度看数学：从对象看到结构

抽象代数的入口，不是先背“群、环、域”的定义，而是先换一个角度看数学。

我们平时学数学，常常先盯着对象本身：

- 这是一个整数。
- 这是一个矩阵。
- 这是一个多项式。
- 这是一个函数。
- 这是一个几何旋转。

这种看法当然有用。整数有整数的算法，矩阵有矩阵的算法，多项式有多项式的展开方式，函数有函数的求导，也可以先做一个函数再做另一个函数。但抽象代数会往后退一步问：

> 如果暂时不看这些对象长什么样，只看它们能做什么操作，以及这些操作满足什么规律，会发生什么？

这一步就是视角变化。

从这个视角看，整数不只是一个个数，矩阵不只是一个数字表，多项式不只是带着 `x` 的表达式，函数也不只是画在坐标系里的曲线。它们都可以被看成某个系统里的对象；这些对象之间可以做操作；操作又满足一些稳定规律。

这就是抽象代数说的结构。

### 什么是结构

在这一章里，可以先把结构理解成一句话：

> 结构 = 对象集合 + 运算或关系 + 这些运算或关系满足的规则。

这句话有三层意思。

第一，有一批对象。

这些对象可以是整数、实数、矩阵、多项式、函数，也可以是旋转、平移、反射、置换这类操作本身。

第二，这批对象之间有某种运算或关系。

整数可以相加，矩阵可以相乘，函数可以先做一个再做另一个，旋转也可以先做一个再做另一个。比如先旋转 90 度，再旋转 180 度，合起来就是旋转 270 度。

第三，这些运算或关系满足稳定规则。

比如整数加法满足结合律：

$$
(a+b)+c=a+(b+c)
$$

它还存在一个什么都不改变的元素 0：

$$
a+0=a
$$

每个整数也都有一个可以把它抵消回 0 的相反数：

$$
a+(-a)=0
$$

这些公式看起来很简单，但它们说明了一件事：整数在加法下面不是一堆散乱的数字，而是一个有组织的系统。抽象代数关心的，就是这种组织方式。

所以，抽象代数不是把数学变得更玄，而是把注意力从“具体对象是什么”转向“对象之间的结构是什么”。它会问：

- 这里有哪些对象？
- 对象之间允许做什么操作？
- 操作以后是否还留在原来的对象集合里？
- 连续操作时，括号位置是否影响结果？
- 有没有一个什么都不改变的单位元素？
- 一个操作能不能被撤销？

这些问题不是在增加复杂度，而是在帮我们看清楚一个数学系统的骨架。

### 同一批对象，换一种操作，结构就变了

结构不是对象自己天然唯一携带的标签。同一批对象，只要关注的操作不同，就可能形成不同结构。

还是看整数。

如果只看整数加法，那么我们研究的是：

$$
(\mathbb{Z},+)
$$

这里的对象集合是整数 `Z`，关注的运算是加法 `+`。在这个结构里，0 是单位元，每个整数都有加法逆元，任意两个整数相加以后仍然是整数。

但如果同时关心加法和乘法，那么我们研究的是：

$$
(\mathbb{Z},+,\times)
$$

这时问题变得更丰富：我们不只问加法怎样工作，还要问乘法怎样工作，以及加法和乘法之间怎样配合。比如乘法对加法满足分配律：

$$
a(b+c)=ab+ac
$$

所以，结构不是只由对象决定的，还由我们选择关注哪些运算、哪些关系决定。抽象代数不是看到一个集合就结束，而是继续问：这个集合上有什么操作？这些操作共同形成了什么结构？

### 同一个符号，可能对应不同结构

同样叫“乘法”，不代表背后就是同一种结构。实数、矩阵、多项式、向量、函数都可以谈“乘法”或类似乘法的组合，但它们相乘的对象、算法和结果类型并不一样。

比如实数乘法：

$$
2\cdot 3=6
$$

矩阵乘法：

```math
\begin{bmatrix}
2 & 0 \\
0 & 3
\end{bmatrix}
\begin{bmatrix}
0 & 1 \\
1 & 0
\end{bmatrix}
=
\begin{bmatrix}
0 & 2 \\
3 & 0
\end{bmatrix}
```

向量点积：

$$
(1,1)\cdot(1,2)=1\cdot 1+1\cdot 2=3
$$

向量外积：

```math
\begin{bmatrix}
1 \\
1
\end{bmatrix}
\begin{bmatrix}
1 & 2
\end{bmatrix}
=
\begin{bmatrix}
1 & 2 \\
1 & 2
\end{bmatrix}
```

函数逐点乘法：

$$
h(x)=f(x)g(x)
$$

函数复合，也就是先做 `f` 再做 `g`：

$$
(g\circ f)(x)=g(f(x))
$$

多项式乘法：

$$
(x+1)(x+2)=x^2+3x+2
$$

这些例子都用了“乘法”或“组合”的想法，但结果类型完全不同：有的是数，有的是多项式，有的是矩阵，有的是函数。它们不是同一个运算。

现在我们反过来做一件事：给向量创造性地定义一种新乘法，让它专门模拟多项式乘法。这样可以更清楚地看到：同样是向量，只要乘法规则换了，结构就变了。

多项式里有：

$$
(x+1)(x+2)=x^2+3x+2
$$

如果用基底：

$$
x,\ 1
$$

来写一次多项式，那么：

$$
x+1
\quad\leftrightarrow\quad
(1,1)
$$

因为它是：

$$
1\cdot x+1
$$

同理：

$$
x+2
\quad\leftrightarrow\quad
(1,2)
$$

现在让这两个向量“相乘”。这里的乘法不是点积：

$$
(1,1)\cdot(1,2)\ne 1\cdot 1+1\cdot 2
$$

如果用点积，结果会是一个数 `3`，这显然不是多项式乘法的结果。

也不是刚才的外积。我们要定义的是另一种新乘法：按照多项式展开规则，把所有项两两相乘，再合并同类项：

$$
(x+1)(x+2)
=x\cdot x+x\cdot 2+1\cdot x+1\cdot 2
$$

也就是：

$$
x^2+3x+2
$$

如果结果用基底：

$$
x^2,\ x,\ 1
$$

表示，那么结果向量就是：

$$
(1,3,2)
$$

所以在这个表示下，可以说：

$$
(1,1)*(1,2)=(1,3,2)
$$

这里的 `*` 不是普通向量点积，也不是矩阵乘法，而是“按多项式乘法规则定义出来的向量乘法”。它更像是在做系数卷积：

$$
(1,1)*(1,2)=(1,\ 1\cdot 2+1\cdot 1,\ 1\cdot 2)=(1,3,2)
$$

这个例子说明：同样是向量，换一种乘法规则，结构就变了。抽象代数关心的不是“它是不是写成向量”，而是这个乘法到底按什么规则把两个对象组合成第三个对象。

## 二、抽象代数要寻找什么：不同对象背后的统一性

理解了“结构”以后，就可以说出抽象代数真正想找的东西：统一性。

它不是满足于分别研究整数、矩阵、多项式、函数、旋转、置换，而是问：

> 这些看起来不同的对象，背后有没有同一套结构规则？

如果有，我们就可以用同一种语言研究它们。

这就是抽象代数里的“抽象”二字。抽象不是为了远离具体，而是为了从很多具体例子里提取共同模式。

### 从一个具体例子看统一性

看一个正方形。把它绕中心旋转，可以旋转：

- 0 度
- 90 度
- 180 度
- 270 度

这四个动作做完以后，正方形看起来仍然和原来重合。更重要的是，这些旋转可以组合。

比如：

$$
90^\circ + 180^\circ = 270^\circ
$$

再比如：

$$
270^\circ + 90^\circ = 360^\circ
$$

但旋转 360 度和什么都不做一样，所以它等价于 0 度。

这件事和“模 4 加法”非常像。

这里只关心正方形能和自己重合的四种旋转状态：

$$
0^\circ,\ 90^\circ,\ 180^\circ,\ 270^\circ
$$

把它们分别编号成：

$$
0,\ 1,\ 2,\ 3
$$

这时“旋转 90 度”就相当于“加 1”，“旋转 180 度”就相当于“加 2”。转四次回到原位，对应的就是：

$$
3+1 \equiv 0 \pmod{4}
$$

这和“旋转 270 度以后再旋转 90 度，回到原位”是同一个规则。

所以，正方形的四个旋转动作和模 4 加法，虽然一个来自几何，一个来自算术，但背后的结构是相同的。抽象代数关心的正是这种相同。

它不满足于说“这两个东西有点像”。它要给出精确语言：

- 什么叫两个结构相同？
- 什么叫运算规则被保留？
- 什么叫一个结构只是换了一种表示？
- 什么叫不同对象本质上遵守同一套规则？

这就是后面要讲的同态和同构。

### 一个统一结构的例子：群

再看一批更广的例子：

- 整数在加法下
- 非零实数在乘法下
- 从一个集合到自身的可逆函数，并且把“先做一个函数，再做另一个函数”当作运算
- 正方形的四个旋转，并且把“先做一个旋转，再做另一个旋转”当作运算
- 一个集合上的所有重新排列方式，并且把“先做一次重排，再做另一次重排”当作运算
- 可逆矩阵在矩阵乘法下

它们表面上差别很大：有的是数，有的是函数，有的是几何动作，有的是重排元素的方式，有的是矩阵。但如果只看各自选定的那一种运算，会发现它们都在回答同一组问题。

#### 封闭性

两个对象运算以后，结果还在原来的集合里。

整数加整数，还是整数。

非零实数乘非零实数，还是非零实数。

两个可逆函数先后做，结果还是可逆函数。

两个正方形旋转接起来，还是正方形的四个旋转之一。

两个重新排列方式连续做，结果还是一种重新排列方式。

两个可逆矩阵相乘，还是可逆矩阵。

#### 结合律

连续做三次运算时，括号位置不影响结果。

整数加法满足：

$$
(a+b)+c=a+(b+c)
$$

非零实数乘法满足：

$$
(ab)c=a(bc)
$$

可逆函数也满足：先把前两个函数合成，还是先把后两个函数合成，最后得到的函数一样。

正方形旋转也满足：先把前两个旋转合成，还是先把后两个旋转合成，最后整体旋转效果一样。

重新排列方式也满足同样规则。下面用它具体算一次。

比如对 `{红,蓝,绿,黄}` 做三次重排。设：

$$
A=\text{交换红和蓝}
$$

$$
B=\text{交换蓝和绿}
$$

$$
C=\text{交换绿和黄}
$$

结合律比较的是“先把 `A` 和 `B` 打包，再接上 `C`”，还是“先把 `B` 和 `C` 打包，再接到 `A` 后面”。

第一条路线是 `(AB)C`：

$$
(\text{红},\text{蓝},\text{绿},\text{黄})
\xrightarrow{AB}
(\text{绿},\text{红},\text{蓝},\text{黄})
\xrightarrow{C}
(\text{黄},\text{红},\text{蓝},\text{绿})
$$

这里 `AB` 的意思是先做 `A`，再做 `B`：

$$
(\text{红},\text{蓝},\text{绿},\text{黄})
\xrightarrow{A}
(\text{蓝},\text{红},\text{绿},\text{黄})
\xrightarrow{B}
(\text{绿},\text{红},\text{蓝},\text{黄})
$$

第二条路线是 `A(BC)`：

$$
(\text{红},\text{蓝},\text{绿},\text{黄})
\xrightarrow{A}
(\text{蓝},\text{红},\text{绿},\text{黄})
\xrightarrow{BC}
(\text{黄},\text{红},\text{蓝},\text{绿})
$$

这里 `BC` 的意思是先做 `B`，再做 `C`：

$$
(\text{蓝},\text{红},\text{绿},\text{黄})
\xrightarrow{B}
(\text{绿},\text{红},\text{蓝},\text{黄})
\xrightarrow{C}
(\text{黄},\text{红},\text{蓝},\text{绿})
$$

两条路线的打包方式不同，但实际执行顺序都是先做 `A`，再做 `B`，最后做 `C`，所以最终结果都是：

$$
(\text{黄},\text{红},\text{蓝},\text{绿})
$$

这里很容易把结合律和交换律混在一起。结合律没有改变操作顺序，只是在改变打包方式。比如三次重排分别叫 `A`、`B`、`C`，那么结合律比较的是：

$$
(AB)C
$$

和：

$$
A(BC)
$$

两边实际执行顺序都是先做 `A`，再做 `B`，最后做 `C`。

交换律不一样。交换律比较的是：

$$
AB
$$

和：

$$
BA
$$

这时操作顺序真的反过来了。对于重新排列这种操作，先做哪个会影响后面操作作用到谁身上，所以交换律通常会失败。

可逆矩阵乘法可以写成：

$$
(AB)C=A(BC)
$$

#### 单位元

存在一个什么都不改变的元素。

整数加法里是 0：

$$
a+0=a
$$

非零实数乘法里是 1：

$$
a\cdot 1=a
$$

可逆函数里是恒等函数，也就是输入什么就输出什么的函数：

$$
\operatorname{id}(x)=x
$$

正方形旋转里是“旋转 0 度”。

重新排列方式里是“什么都不动”的排列。

比如对 `{红,蓝,绿}` 来说，单位元就是：

$$
(\text{红},\text{蓝},\text{绿})\to(\text{红},\text{蓝},\text{绿})
$$

它不改变任何位置。

可逆矩阵里是单位矩阵 `I`：

$$
AI=IA=A
$$

#### 逆元

每个对象都有一个可以撤销它的对象。

整数 `a` 的加法逆元是 `-a`：

$$
a+(-a)=0
$$

非零实数 `a` 的乘法逆元是 `1/a`：

$$
a\cdot \frac{1}{a}=1
$$

可逆函数可以用它的反函数撤销。如果 $f$ 的反函数是 $f^{-1}$，那么先做 $f$ 再做 $f^{-1}$，就回到原来的输入：

$$
f^{-1}(f(x))=x
$$

旋转 90 度可以用旋转 270 度撤销，旋转 180 度可以用自己撤销。

一次重新排列也可以撤销：只要把每个元素放回原来的位置。

比如先交换红和蓝：

$$
(\text{红},\text{蓝},\text{绿})\to(\text{蓝},\text{红},\text{绿})
$$

再交换红和蓝，就回到原来：

$$
(\text{蓝},\text{红},\text{绿})\to(\text{红},\text{蓝},\text{绿})
$$

可逆矩阵 `A` 可以用逆矩阵 `A^{-1}` 撤销：

$$
AA^{-1}=A^{-1}A=I
$$

现在就能看清楚“统一性”在哪里了：这些对象的外表完全不同，但它们都满足同一套结构规则：

- 封闭性
- 结合律
- 单位元
- 逆元

> 满足这套规则的结构，就叫群。

这时，“群”不是又多了一个难背的定义，而是给一大批现象起了一个共同名字。只要证明某个系统是群，我们就立刻知道它拥有这一整套结构规则。

这就是抽象代数的核心工作：在不同对象之间寻找统一结构。它让我们看到，函数里的可逆变换、几何里的旋转、算术里的整数加法、线性代数里的可逆矩阵、集合论里的置换，并不是完全孤立的知识点。它们可以共享同一套语言。

当然，抽象代数也会关心差异。前面这些例子都满足群的基本规则，但它们不一定都满足交换律。

整数加法可以交换顺序：

$$
a+b=b+a
$$

非零实数乘法也可以交换顺序：

$$
ab=ba
$$

正方形的四个旋转也可以交换顺序。比如先转 90 度再转 180 度，和先转 180 度再转 90 度，最后都是转 270 度。

可逆函数通常不能交换顺序。一般来说：

$$
g\circ f\ne f\circ g
$$

也就是说，先做 `f` 再做 `g`，和先做 `g` 再做 `f`，结果可能不同。

但一个集合上的重新排列方式通常不能交换顺序。比如在 `{红,蓝,绿}` 里，设：

$$
A=\text{交换红和蓝}
$$

$$
B=\text{交换蓝和绿}
$$

交换律要比较的是：

$$
AB
$$

和：

$$
BA
$$

也就是“先做 `A` 再做 `B`”，和“先做 `B` 再做 `A`”。

第一条路线是 `AB`，也就是先交换红和蓝，再交换蓝和绿：

$$
(\text{红},\text{蓝},\text{绿})
\xrightarrow{A}
(\text{蓝},\text{红},\text{绿})
\xrightarrow{B}
(\text{绿},\text{红},\text{蓝})
$$

第二条路线是 `BA`，也就是先交换蓝和绿，再交换红和蓝：

$$
(\text{红},\text{蓝},\text{绿})
\xrightarrow{B}
(\text{红},\text{绿},\text{蓝})
\xrightarrow{A}
(\text{蓝},\text{绿},\text{红})
$$

最后一个得到（绿，红，蓝），另一个得到（蓝，绿，红），所以顺序不能随便交换。

可逆矩阵乘法通常也不能交换顺序：

$$
AB\ne BA
$$

所以，群不要求交换律。满足交换律的群叫交换群，也叫阿贝尔群。整数加法、非零实数乘法、正方形四个旋转都是交换群；一般的可逆函数群、一般的置换群和一般的可逆矩阵群不是交换群。

这就说明：共同规则说明统一性，额外差异说明结构还可以继续细分。

所以抽象代数的视角可以概括成一句话：

> 不先问对象长什么样，而先问它们遵守什么结构；不只看一个例子怎么计算，而要找出许多例子背后的同一套规则。

## 三、从集合、运算与关系搭出结构

前面一直在说结构：

> 结构 = 对象集合 + 运算或关系 + 这些运算或关系满足的规则。

这一节就按这个顺序把这句话拆开。抽象代数后面会讲群、环、域、同态、商结构，但它们都离不开几个最底层的概念：集合、运算、关系，以及这些运算或关系满足的规则。

这几个词看起来很基础，但它们决定了后面所有代数结构到底是在什么对象上建立的。

### 集合：先说清楚对象是谁

集合就是一批对象放在一起。

比如：

$$
\mathbb{Z}=\{\ldots,-2,-1,0,1,2,\ldots\}
$$

表示所有整数。

再比如：

$$
\mathbb{R}
$$

表示所有实数。

还可以有更具体的集合：

$$
S=\{\text{红},\text{蓝},\text{绿}\}
$$

或者：

$$
R=\{0^\circ,90^\circ,180^\circ,270^\circ\}
$$

这里 `R` 可以表示正方形绕中心旋转后仍然和自己重合的四种旋转。

集合本身只回答一个问题：

> 哪些对象在我们的讨论范围内？

这一步很重要。因为一个运算能不能做、做完以后结果是否合法，都要看结果是否还留在这个集合里。

比如在整数集合里：

$$
2+3=5
$$

结果还是整数。

但：

$$
2\div 3=\frac{2}{3}
$$

结果不是整数。

所以，如果我们只在整数集合里讨论，普通除法就不是一个总能留在整数里的运算。

这就是为什么抽象代数总要先说清楚“在哪个集合上”。

### 元素：集合里的单个对象

集合里的单个对象叫元素。

如果 `a` 是集合 `S` 里的元素，就写成：

$$
a\in S
$$

如果 `a` 不在集合 `S` 里，就写成：

$$
a\notin S
$$

比如：

$$
3\in\mathbb{Z}
$$

但：

$$
\frac{1}{2}\notin\mathbb{Z}
$$

这些符号本身不难。真正重要的是：以后我们说“在某个集合上定义运算”，意思就是这个运算的输入和输出都要围绕这个集合来判断。

### 运算：把对象组合成对象

集合说清楚对象是谁以后，下一步就可以问：这些对象之间能不能组合？

运算是在生成新对象。

比如整数加法：

$$
2+3=5
$$

输入两个整数，输出一个整数。

矩阵乘法也是运算。输入两个矩阵，输出一个矩阵。

函数也可以有运算，但这里要分清两种常见情况。

一种是函数的逐点乘法。给两个函数 `f` 和 `g`，可以定义一个新函数 `h`：

$$
h(x)=f(x)g(x)
$$

这时我们是在同一个输入 `x` 上分别算 `f(x)` 和 `g(x)`，再把两个数相乘。

另一种是函数复合，也就是先做一个函数，再做另一个函数：

$$
(g\circ f)(x)=g(f(x))
$$

这里输入两个函数 `f` 和 `g`，输出一个新函数 `g\circ f`。它和逐点乘法不是一回事。

还可以沿用前面的彩色球例子。设集合是：

$$
S=\{\text{红球},\text{蓝球},\text{绿球}\}
$$

如果我们人为规定一种“组合”规则，比如：

> 红和蓝组合成绿，蓝和绿组合成红，绿和红组合成蓝。

那么这也可以看成一个定义在 `S` 上的运算。比如：

$$
\text{红球}*\text{蓝球}=\text{绿球}
$$

$$
\text{蓝球}*\text{绿球}=\text{红球}
$$

这里的 `*` 不是普通乘法，而是我们给彩色球定义的一种组合规则。关键不在于对象是不是数字，而在于规则是否明确：给定两个输入，输出能不能被确定下来，并且输出是否还在同一个集合里。

在抽象代数里，最常见的是二元运算。所谓二元运算，就是一次吃进两个对象，吐出一个对象。

可以写成：

$$
*:S\times S\to S
$$

这句话的意思是：

- 输入来自集合 `S` 的两个元素。
- 用某种规则 `*` 把它们组合起来。
- 输出仍然是集合 `S` 里的一个元素。

比如整数加法可以看成一个函数：

$$
\operatorname{add}:\mathbb{Z}\times\mathbb{Z}\to\mathbb{Z}
$$

意思是两个整数相加，结果还是整数。

这里把加法临时叫作 `add`，是为了强调：加法不是天上掉下来的符号，它也是一个有输入、有输出的规则。

### 封闭性：结果必须留在集合里

二元运算最先要检查的是封闭性。

如果我们说在集合 `S` 上定义了一个运算 `*`，通常意味着：

$$
a,b\in S \quad\Rightarrow\quad a*b\in S
$$

这就是说：只要输入在集合里，输出也必须在集合里。

整数加法满足封闭性：

$$
2,3\in\mathbb{Z},\quad 2+3=5\in\mathbb{Z}
$$

整数乘法也满足封闭性：

$$
2\cdot 3=6\in\mathbb{Z}
$$

但整数除法不满足封闭性：

$$
2,3\in\mathbb{Z},\quad 2\div 3=\frac{2}{3}\notin\mathbb{Z}
$$

所以，普通除法不是整数集合上的二元运算。

这个判断非常关键。很多时候，一个操作在更大的集合里合法，在更小的集合里就不合法。

比如：

- 在实数里，非零数可以相除。
- 在整数里，很多除法结果会跑到整数外面。
- 在可逆矩阵里，两个可逆矩阵相乘仍然可逆。
- 但两个任意矩阵相乘以后，不一定可逆。

所以抽象代数总是先问：

> 这个运算到底定义在哪个集合上？做完以后还留在这个集合里吗？

### 运算规则：结构从这里开始出现

有了集合和运算以后，结构就开始出现了。

但仅仅有运算还不够。我们还要问这个运算满足哪些规则。

比如：

- 是否满足结合律？
- 是否满足交换律？
- 有没有单位元？
- 每个元素有没有逆元？
- 如果有两个运算，它们之间是否满足分配律？

这些规则决定了这个结构到底是哪一类结构。

如果一个集合上有一个运算，并且满足封闭性、结合律、单位元、逆元，那么它就是群。

如果它还有第二个运算，并且两个运算像加法和乘法那样配合起来，就会走向环和域。

### 关系：对象之间是否满足某种判断

除了运算，集合上的对象之间还可以有关系。

运算是在生成新对象，关系则是在判断两个对象是否满足某个条件。

比如：

$$
2+3=5
$$

这是运算。

而：

$$
2<3
$$

这是关系。

再比如在整数里，可以定义一个关系：

> `a` 和 `b` 有相同的奇偶性。

这就是说：

- 2 和 4 有关系，因为它们都是偶数。
- 3 和 7 有关系，因为它们都是奇数。
- 2 和 3 没有关系，因为一个是偶数，一个是奇数。

在平面上的点之间可以定义“距离小于 1”的关系；在集合之间可以定义“包含于”的关系；在向量之间可以定义“垂直”的关系。

关系的作用是帮助我们把对象组织起来。后面讲商结构时，最重要的一类关系就是等价关系。

### 等价关系：把对象分成一类一类

等价关系可以理解成一种“看作同类”的规则。

比如整数里，我们可以说：

> 如果两个整数除以 4 的余数相同，就把它们看作同类。

这样：

$$
1,5,9,13,\ldots
$$

属于同一类，因为它们除以 4 都余 1。

同样：

$$
0,4,8,12,\ldots
$$

属于同一类，因为它们除以 4 都余 0。

这就是模 4 的基本直觉。我们不再关心两个整数具体差多少，而是只关心它们除以 4 的余数是否一样。

一个关系要成为等价关系，通常要满足三件事。

#### 自反性

每个对象都和自己等价。

比如任何整数和自己除以 4 的余数当然一样。

#### 对称性

如果 `a` 和 `b` 等价，那么 `b` 和 `a` 也等价。

比如 1 和 5 除以 4 的余数一样，那么 5 和 1 除以 4 的余数也一样。

#### 传递性

如果 `a` 和 `b` 等价，`b` 和 `c` 等价，那么 `a` 和 `c` 也等价。

比如 1、5、9 除以 4 都余 1，所以它们都在同一类里。

等价关系的作用，是把一个集合切成若干个互不重叠的类。每个类里放的是“按照某个标准看起来一样”的对象。

这件事很重要。抽象代数里很多结构，都是从“把某些差异忽略掉”开始的。模运算就是最简单的例子：它忽略具体整数大小，只保留余数结构。

所以，集合、运算和关系不是抽象代数的“预备废话”。它们是在搭地基：

- 集合决定对象范围。
- 运算决定对象怎样组合。
- 运算规则决定对象组合以后形成什么结构。
- 关系决定对象之间怎样比较或归类。
- 等价关系决定怎样把对象分成同类。

理解了这些，后面再看群、环、域，就不会觉得是在背一堆凭空出现的定义。

## 四、群：描述可逆操作的语言

前面已经见过群的雏形：

> 满足封闭性、结合律、单位元、逆元这套规则的结构，就叫群。

现在正式把它说清楚。

群不是一种具体对象，而是一种结构。它可以出现在整数里，也可以出现在旋转里、函数里、矩阵里、重新排列里。只要一批对象配上一种运算，并且满足同一套规则，我们就可以用“群”这个词来描述它。

群特别适合描述对称性、变换、置换和可逆操作，是因为这些例子里的“对象”常常不是静止的东西，而是一个个可以执行的动作：

- 旋转正方形，是一种几何变换。
- 重新排列一组元素，是一种置换。
- 从 X 到 X 的可逆函数，是一种把对象搬到另一个位置、又能搬回来的变换。
- 可逆矩阵，是线性空间里的可逆线性变换。

这些动作都可以连续执行。先做一个动作，再做另一个动作，仍然得到一个同类动作；什么都不做，是单位元；每个动作如果能被撤销，就有逆元。所以群抓住的不是对象长什么样，而是：

> 一批可连续执行、可保持在同一系统内、并且可撤销的操作，整体遵守什么规则？

这也是为什么群会成为描述对称性的语言。一个图形的对称性，就是那些做完以后图形看起来没有变的可逆变换；所有这些变换放在一起，通常就形成一个群。

### 群的正式定义

一个群通常记作：

$$
(G,*)
$$

这里 `G` 是对象集合，`*` 是定义在 `G` 上的运算。

如果它满足下面四条规则，就叫群。

#### 封闭性

任意两个元素 `a,b` 都在 `G` 里时，它们运算后的结果也在 `G` 里：

$$
a,b\in G \quad\Rightarrow\quad a*b\in G
$$

也就是说，运算不会把我们带出这个集合。

#### 结合律

任意三个元素 `a,b,c` 都在 `G` 里时：

$$
(a*b)*c=a*(b*c)
$$

也就是说，连续做三次运算时，括号怎么放不影响结果。

#### 单位元

集合里存在一个特殊元素 `e`，它和任何元素运算，都不改变那个元素：

$$
e*a=a
$$

并且：

$$
a*e=a
$$

这个 `e` 叫单位元。

不同群里的单位元长得不一样。整数加法里的单位元是 0，非零实数乘法里的单位元是 1，矩阵乘法里的单位元是单位矩阵。

#### 逆元

对每个元素 $a$，都能找到一个元素 $a^{-1}$，使得它们运算后回到单位元：

$$
a*a^{-1}=e
$$

并且：

$$
a^{-1}*a=e
$$

这个 $a^{-1}$ 叫 $a$ 的逆元。

这里的写法 $a^{-1}$ 不一定表示普通倒数。它只是表示“能把 $a$ 撤销掉的对象”。在整数加法里，$a$ 的逆元不是 $1/a$，而是 $-a$，因为：

$$
a+(-a)=0
$$

注意，这四条规则里没有要求：

$$
a*b=b*a
$$

也就是说，群不一定要求交换律。原因是很多重要结构本来就不交换，比如矩阵乘法、函数复合、空间变换的连续执行。它们仍然有封闭性、结合律、单位元和逆元，所以应该被群这个概念包括进来。

如果一个群额外满足交换律，才叫交换群，也叫阿贝尔群。可以把关系理解成：

> 阿贝尔群是群的一种特殊情况，不是群的默认要求。

### 怎么判断一个结构是不是群

判断一个结构是不是群，不是靠感觉，而是按四步检查。

第一，先说清楚集合是什么。

集合就是我们这次讨论的对象范围。也就是说，先规定“哪些东西算合法对象，哪些东西不在讨论范围里”。群不是只看一个运算符号，而是先看这个运算符号作用在哪一批对象上。

比如，如果集合是：

$$
\mathbb{Z}
$$

意思就是我们只讨论所有整数：

$$
\ldots,-2,-1,0,1,2,\ldots
$$

这时 `1/2`、`sqrt(2)`、`pi` 都不属于这个集合。它们当然是数学对象，但不在当前讨论范围里。

如果集合是：

$$
\mathbb{R}\setminus\{0\}
$$

这个符号可以拆开看：

- ℝ 表示所有实数。
- {0} 表示只含有 0 这一个元素的集合。
- ∖ 表示“从前面的集合里去掉后面的集合”，读作“去掉”或“差”。

所以：

$$
\mathbb{R}\setminus\{0\}
$$

意思就是“所有实数去掉 0”，也就是所有非零实数。这里把 `0` 特意拿掉，是因为后面如果讨论乘法，`0` 没有乘法逆元。也就是说，没有任何实数能和 `0` 相乘得到乘法单位元 `1`。

如果集合是：

$$
GL_n(\mathbb{R})
$$

这个符号也可以拆开看：

- $GL$ 是 general linear group 的缩写，中文通常叫“一般线性群”。这里的 linear 指线性变换或矩阵，group 指这些对象在复合或矩阵乘法下要组成群。为了组成群，每个元素都必须有逆元，所以只保留可逆的线性变换，也就是可逆矩阵。
- 下标 $n$ 表示矩阵的大小是 $n\times n$。
- $\mathbb{R}$ 表示矩阵里的每个数都是实数。

所以 $GL_n(\mathbb{R})$ 是一个集合，里面装的是所有 $n\times n$ 可逆实矩阵。这个集合里的每个元素不是普通数字，而是一整个矩阵。并且不是所有矩阵都能放进来，只有可逆矩阵才属于 $GL_n(\mathbb{R})$。

为什么要强调“可逆”？因为后面如果把矩阵乘法当作运算，群要求每个元素都有逆元。一个矩阵只有可逆，才存在另一个矩阵能把它“乘回”单位矩阵。

所以，“先说清楚集合是什么”不是形式主义。它是在回答：

> 我们到底允许哪些对象参与运算？

同一个运算，放在不同集合上，性质可能完全不同。整数加法、非零实数乘法、可逆矩阵乘法之所以要分开写，就是因为它们讨论的对象范围不同。

第二，说清楚运算是什么。

比如加法、乘法、矩阵乘法、函数复合、重新排列的连续执行。

第三，检查四条规则：

- 运算结果是否还在集合里？
- 括号位置是否不影响结果？
- 有没有单位元？
- 每个元素能不能被撤销？

第四，特别注意：群不要求交换律。

也就是说，一个群不一定满足：

$$
a*b=b*a
$$

如果额外满足交换律，才叫交换群，也叫阿贝尔群。

### 例子一：整数加法

整数集合配上加法：

$$
(\mathbb{Z},+)
$$

是一个群。

封闭性：两个整数相加，结果还是整数。

结合律：

$$
(a+b)+c=a+(b+c)
$$

单位元是 0：

$$
a+0=0+a=a
$$

逆元是相反数：

$$
a+(-a)=0
$$

所以整数在加法下构成群。

而且它还是交换群，因为：

$$
a+b=b+a
$$

### 例子二：非零实数乘法

非零实数集合配上乘法：

$$
(\mathbb{R}\setminus\{0\},\times)
$$

也是一个群。

封闭性：两个非零实数相乘，结果还是非零实数。

结合律：

$$
(ab)c=a(bc)
$$

单位元是 1：

$$
a\cdot 1=1\cdot a=a
$$

逆元是倒数：

$$
a\cdot \frac{1}{a}=1
$$

注意这里必须把 0 排除掉。因为 0 没有乘法逆元。你找不到一个实数 `b`，使得：

$$
0\cdot b=1
$$

所以：

$$
(\mathbb{R},\times)
$$

不是群。

这也说明：同样是乘法，集合选得不一样，结构可能完全不同。

### 例子三：可逆函数

设 X 是一个集合。考虑所有从 X 到 X 的可逆函数。

这里的运算是函数复合，也就是先做一个函数，再做另一个函数。

如果 f: X → X 和 g: X → X 都可逆，那么：

$$
g\circ f
$$

也可逆。

单位元是恒等函数：

$$
\operatorname{id}(x)=x
$$

每个可逆函数都有反函数。也就是说，如果 `f` 可逆，就存在 `f^{-1}`，使得：

$$
f^{-1}(f(x))=x
$$

所以，从 `X` 到 `X` 的所有可逆函数，在函数复合下构成群。

这个群通常不交换。一般来说：

$$
g\circ f\ne f\circ g
$$

也就是说，先做 `f` 再做 `g`，和先做 `g` 再做 `f`，结果可能不同。

### 例子四：正方形的旋转

正方形绕中心旋转后仍然和自己重合的旋转有四个：

$$
0^\circ,\ 90^\circ,\ 180^\circ,\ 270^\circ
$$

运算是连续旋转。

比如：

$$
90^\circ+180^\circ=270^\circ
$$

再比如：

$$
270^\circ+90^\circ=360^\circ
$$

但旋转 360 度和什么都不做一样，所以回到：

$$
0^\circ
$$

这个结构和模 4 加法非常像。把四个旋转编号为：

$$
0,1,2,3
$$

每次旋转 90 度就相当于加 1。转四次回到原位，就对应：

$$
3+1\equiv 0\pmod{4}
$$

所以正方形的四个旋转在连续旋转下构成一个群。

### 例子五：可逆矩阵

所有 `n×n` 可逆实矩阵组成的集合，通常记作：

$$
GL_n(\mathbb{R})
$$

这里的运算是矩阵乘法。

两个可逆矩阵相乘，结果还是可逆矩阵。

单位元是单位矩阵：

$$
I
$$

每个可逆矩阵 `A` 都有逆矩阵 `A^{-1}`：

$$
AA^{-1}=A^{-1}A=I
$$

所以：

$$
(GL_n(\mathbb{R}),\times)
$$

是一个群。

但它通常不是交换群，因为一般情况下：

$$
AB\ne BA
$$

这就是矩阵乘法里顺序很重要的原因。

### 非例子：整数乘法为什么不是群

整数集合配上乘法：

$$
(\mathbb{Z},\times)
$$

不是群。

它满足封闭性：两个整数相乘，结果还是整数。

它也满足结合律。

它有乘法单位元 1。

但大多数整数没有乘法逆元。

比如 2 的乘法逆元应该是：

$$
\frac{1}{2}
$$

但：

$$
\frac{1}{2}\notin\mathbb{Z}
$$

所以 2 在整数乘法下没有逆元。

因此：

$$
(\mathbb{Z},\times)
$$

不是群。

这个例子非常重要。它说明判断群时，不能只看运算熟不熟悉，还要看逆元是否仍然留在同一个集合里。

### 群的直觉：可撤销的操作系统

群可以先理解成：

> 一套可以连续执行、可以撤销、并且不会跑出系统外的操作。

整数加法里，加上 `a` 可以用加上 `-a` 撤销。

非零实数乘法里，乘以 `a` 可以用乘以 `1/a` 撤销。

函数复合里，做一个可逆函数可以用它的反函数撤销。

正方形旋转里，转 90 度可以用转 270 度撤销。

可逆矩阵里，乘以 `A` 可以用乘以 `A^{-1}` 撤销。

所以，群不是一个孤立定义。它是在捕捉同一种模式：

> 对象可以组合，组合有稳定规则，每一步都可以撤销。

理解了这个直觉，后面再讲子群、循环群、置换群和群作用，就不会只是在背定义，而是在研究这些“可撤销操作系统”的内部结构。

## 五、接下来要讲什么

后面的内容会沿着这条线展开。

### 环与域：把加法和乘法组织起来

群通常只组织一种运算，而环和域同时组织加法与乘法。整数、多项式、矩阵、实数、有理数都可以放在这个视角下理解。这里会特别强调：什么时候能做除法，什么时候不能做除法，这其实是结构差异。

### 同态、同构与商结构

同态研究不同结构之间怎样建立保持运算的映射；同构研究什么时候两个结构本质上一样；商结构研究怎样把某些对象视为等价以后得到新的结构。这一部分会帮助我们理解“结构比表示更重要”这句话到底是什么意思。

### 作用、对称性与不变量

群本身描述一组变换，群作用描述这些变换具体怎样作用在数据、空间或函数上。不变量和等变性则研究在变换下什么不变、什么按规则变化。这部分会为后面连接几何、表示论和机器学习里的结构化模型做准备。

### 机器学习里的代数思维

最后再回到机器学习语境：什么时候应该用代数视角看问题，什么时候不必强行代数化。重点会放在结构化输入、对称性约束、表示学习、图与集合模型、编码和离散状态空间这些场景。

这一章的目标不是让读者一次性掌握所有抽象代数定理，而是先建立一种眼光：看到一个数学对象时，不只问它是什么，还要问它带着什么结构；看到一个公式时，不只问怎么算，还要问这个计算依赖哪些规则。

这就是抽象代数的入口。
