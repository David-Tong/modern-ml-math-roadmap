...
这会得到第三章的 Euler 法。Runge–Kutta 法则在一步内估计多次变化率。它们计算离散时刻的近似值，前面的 Picard 迭代则逐次更新整条函数。
...
这把当前的差异分成初值差异和沿途累积的差异。在适当的 Lipschitz 条件下，可以进一步用积分不等式控制误差，得到第四章的连续依赖估计。
...
下面也把方程与具体模型联系起来：先辨认未知量和各项的意义，再看解法如何揭示变化规律。同一种方程结构可以描述不同系统，具体的物理意义要由建模时的假设决定。
...
沿用第二章从积分近似得到的更新思路，在网格 $t_n=t_0+nh$ 上，用 $x_n$ 近似 $x(t_n)$，就得到显式 Euler 法：
...
| 已知两个齐次独立解的二阶非齐次线性方程 | 二阶常数变易（选读） | 标准化、Wronskian 不为零 |
| 可化为标准特殊函数方程 | 特殊函数或幂级数表示 | 标准化、奇点与初值或边界条件 |
| 没有方便解析表示的初值问题 | Euler、Runge–Kutta 等数值方法 | 步长、误差、稳定性与存在区间 |

这些方法覆盖了本篇后续最常遇到的 ODE，但并不是所有 ODE 的完整解法。其他方程还可能需要换元、恰当方程或更进一步的级数方法等工具。进入 PDE 以后，我们会反复使用本章的方法：先把场的问题化为一个或一组 ODE，再结合初始条件、边界条件还原所求的场。

## 四、解的基本理论：存在性、唯一性与解的含义
...
应用语境里，“解析解”常指用函数、积分或收敛级数给出的精确表达。例如 $y'=-y$、$y(0)=1$ 的解是 $y=e^{-t}$。解也可以用特殊函数表示，不必只含多项式、指数和三角函数；第五章会介绍这种情形。
...
“数值解”描述的是近似表示方式，可以用来逼近经典解，也可以用来逼近弱解。例如有限元法直接从弱形式构造近似，第三章的 Euler 法则从初值问题的积分关系构造近似。
...
前面经常把解写成指数、正弦和余弦。但不是每个 ODE 的解都能由有限次初等运算和初等函数组合表示。遇到反复出现的标准方程，我们可以研究它的解，选定规范化条件，并给这些解命名。这就得到了一批常见的特殊函数。
...
第四章已经区分了解的表达方式、经典解、光滑解、弱解和数值解。来到 PDE，仍然使用这些概念，但要进一步说明空间区域、边界条件，以及怎样衡量整个场的误差。
...
第四章通过 $-y''=f$ 说明了分部积分怎样降低对解的求导要求。对齐次 Dirichlet 问题
