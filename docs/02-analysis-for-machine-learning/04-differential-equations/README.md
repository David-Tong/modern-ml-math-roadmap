...
求解时，可分离变量的方程用分离变量后积分；一阶线性方程用积分因子或常数变易法；二阶常系数齐次方程用特征方程，非齐次方程则求“齐次通解加一个特解”。二阶常数变易法列为选读：已知两个独立的齐次解时，它也适用于变系数方程。最后用对数代换求解柯西–欧拉方程，展示一类线性变系数方程的解法。
...
这类方程的物理背景可以接上前面的弹簧模型：Newton 的运动定律把合力与加速度联系起来，Hooke 定律把弹簧回复力与位移联系起来。十八世纪的振动研究进一步推动了二阶线性方程的发展；方程中的二阶导数，正是从加速度进入模型的。
...
**特征根的物理意义。** 对弹簧模型 $my''+\gamma y'+ky=0$，特征方程是 $mr^2+\gamma r+k=0$。当 $\gamma=0$ 时，根为 $\pm i\sqrt{k/m}$，对应振幅不变的自由振动。当 $\gamma>0$ 时，判别式 $\gamma^2-4mk$ 决定回到平衡的方式：小于零时，解一边振荡、一边按指数衰减，称为欠阻尼；等于零时称为临界阻尼；大于零时是两个衰减指数的组合，称为过阻尼。后两种情形没有持续往复的振荡。复根的虚部决定振荡角频率，实部决定振幅增长或衰减的快慢。

一般常系数方程并不都描述稳定的弹簧系统。下面也保留一个有正实根的例子，展示指数增长的情形。
...
第二个例子具体取无量纲的质量、阻尼和刚度分别为 $1,2,5$。Newton 第二定律是 $y''=-2y'-5y$，整理后得到一个有阻尼的自由振动方程：
...
**历史背景。** 二阶非齐次线性方程随着对受外力作用的运动、振动和天体摄动的研究逐步发展，并非由一个人一次提出。十八世纪，Euler 研究了非齐次线性方程的求解；Euler 与 Lagrange 等人发展的常数变易思想，则提供了从已知齐次解出发处理附加作用的途径。待定系数法和常数变易法的具体历史放在下面各自的方法中介绍。
...
前面讨论了二阶常系数齐次方程。现在加入非零的右端项，研究二阶非齐次线性方程。它的一般形式是
...
记 $y_h$ 为齐次方程的通解，$y_p$ 为非齐次方程的一个特解。下标 $h$ 来自 homogeneous（齐次），$p$ 来自 particular（特解），都只是标签。“一个特解”指一个确定的、能满足非齐次方程的函数，不要求它已经满足题目给定的初值。

既然 $y_h$ 满足齐次方程、$y_p$ 满足非齐次方程，就有
...
$$
y_p''+p(t)y_p'+q(t)y_p=r(t).
$$

令 $y=y_h+y_p$。求导得到 $y'=y_h'+y_p'$、$y''=y_h''+y_p''$，代入原方程左边：

$$
\begin{aligned}
y''+p(t)y'+q(t)y
&=(y_h''+y_p'')+p(t)(y_h'+y_p')+q(t)(y_h+y_p)\\
&=\underbrace{y_h''+p(t)y_h'+q(t)y_h}_{0}
+\underbrace{y_p''+p(t)y_p'+q(t)y_p}_{r(t)}\\
&=r(t).
\end{aligned}
$$

所以 $y_h+y_p$ 确实满足非齐次方程。

反过来，若 $y$ 是非齐次方程的任意一个解，那么

$$
\begin{aligned}
(y-y_p)''+p(t)(y-y_p)'+q(t)(y-y_p)
&=[y''+p(t)y'+q(t)y]\\
&\quad-[y_p''+p(t)y_p'+q(t)y_p]\\
&=r(t)-r(t)=0.
\end{aligned}
$$

这说明 $y-y_p$ 必须属于齐次解族。因此，所有非齐次解都能写成

$$
y=y_h+y_p.
$$

求解便分成三步：求齐次通解，找一个非齐次特解，把它们相加。最后再用初值或边值确定齐次通解中的任意常数。

**这个分解的物理意义。** 在受迫振动中，$y_p$ 给出一种与外力相适应的运动，$y_h$ 则用来调整初始位置和速度。相同外力下，两组不同初值产生的运动之差满足齐次方程，因此初值差异怎样演化，仍由自由运动的规律决定。

对于 $m,k,\gamma>0$ 的弹簧系统，如果外力是周期性的，可以选择周期特解作为持续受迫响应；齐次部分随时间衰减，最后留下周期响应。无阻尼时，齐次振动不会自行消失；若周期外力的频率恰好等于固有频率，还可能出现共振，振幅随时间增长。所以下面的“找特解”，也就是在求系统怎样响应给定的外力。

这里的“齐次通解加一个特解”说明的是解的结构；下面的待定系数法和常数变易法，都是寻找其中 $y_p$ 的具体方法。待定系数法适合常系数方程中某些常见形式的右端；常数变易法适用范围更广，但需要先知道齐次方程的两个线性无关解。

#### 求特解：待定系数法

**历史背景。** 本节的待定系数法与 Euler 的非齐次方程研究有关，其发展可以追溯到他在 1753 年研究月球运动时使用的技巧：根据外部项选择特解形式，再代入确定系数。

##### 为什么可以按右端形式猜特解

对于常系数方程，如果右端是多项式、指数、正弦余弦或它们的有限组合，可以尝试同类形式的特解。原因是这些函数求导后仍在相应的函数组合中：例如 $e^{3t}$ 求导后仍是 $e^{3t}$ 的倍数。
...
$$
y_p'=3Ke^{3t},\qquad y_p''=9Ke^{3t}.
$$

代回非齐次方程：

$$
9Ke^{3t}-3\cdot3Ke^{3t}+2Ke^{3t}=e^{3t}.
$$

合并左边的同类项，得到 $2Ke^{3t}=e^{3t}$。因为 $e^{3t}\ne0$，两边除以它，就有 $2K=1$，所以
...
例如给定 $y(0)=0$、$y'(0)=0$。先求通解的导数：

$$
y'=C_1e^t+2C_2e^{2t}+\frac32e^{3t}.
$$
...
第二式减第一式，得到 $C_2+1=0$，所以 $C_2=-1$；再代回第一式，得到 $C_1=\frac12$。满足这组初值的解就是

$$
y(t)=\frac12e^t-e^{2t}+\frac12e^{3t}.
$$

##### 猜测形式与齐次解重复时会怎样

考虑 $y''+y=\sin t$。如果试取 $y_p=A\cos t+B\sin t$，就有
...
无论怎样选 $A,B$，左边都等于零，不可能等于 $\sin t$。这是因为猜测形式已经属于齐次解族。

待定系数法通常通过乘以适当的 $t$ 的幂来调整猜测；它适合常系数方程和上述特定形式的右端。下一节用常数变易法处理这个例子，直接求出变化的系数。

#### 求特解：常数变易法（选读）

这一部分展开二阶情形的完整推导。初学时可以先掌握上面的解的结构和待定系数法，再回来阅读。
...
如果原方程中的 $\alpha$、$\beta$ 换成一般函数 $\alpha(x)$、$\beta(x)$，代换后就会出现 $\alpha(e^t)-1$、$\beta(e^t)$，系数通常仍然随 $t$ 变化。因此，要先确认方程具有上述结构，不能看到变系数就直接套用这个方法。即使左边成功化为常系数，右端 $g(e^t)$ 也未必适合待定系数法，仍需根据它的具体形式选择求特解的方法。
...
线性方程之所以容易组织和求解，一个关键原因是**微分运算保留线性组合**。以二阶线性算子
...
| 二阶常系数线性非齐次方程 $ay''+by'+cy=r(t)$ | 齐次通解加一个特解；特解可用待定系数法或二阶常数变易法求出 | 受迫振动：外力持续驱动系统，解反映自由运动与外力响应的共同作用 |

一阶线性方程中的 $p(t)$、$q(t)$ 可以随时间变化；二阶部分主要研究常系数方程。待定系数法适用于右端为多项式、指数、正弦余弦等适合形式的情形；二阶常数变易法要求先知道两个线性无关的齐次解，在这个前提下也适用于变系数方程。
...
遇到非线性 ODE，可以先检查能否分离变量或通过代换化成已知方程。对单摆这类模型，小角度下可以用线性近似简化方程，但要留意近似成立的范围；不能方便地求出公式时，还可以用 Euler 或 Runge–Kutta 方法进行数值计算，并检查误差与步长。

非线性方程不一定需要特殊函数，线性方程也不一定有初等函数解。下一章继续讨论怎样用微分方程、积分和级数定义函数，以及怎样用特殊函数表示解。

## 五、ODE 的解怎样表示：从初等函数到特殊函数

前两章分别学习了线性方程的基本解法，以及非线性方程的代换、线性近似和数值求解。本章继续追问：如果熟悉的初等函数不足以表示解，怎么办？先从指数、三角和双曲函数看怎样用微分方程定义函数，再介绍积分定义和 Airy、Bessel 等特殊函数。不能用初等函数表示，并不等于没有解；我们还可以用新的函数、积分和级数准确地描述它。

### 用微分方程定义函数：指数、三角与双曲函数

“用方程定义函数”并不陌生。指数函数 $e^t$ 可以刻画为初值问题

$$
y'=y,\qquad y(0)=1
$$

的唯一解。类似地，$\cos t$ 和 $\sin t$ 分别对应
...
$$
\begin{cases}y''-y=0,\\y(0)=1,\quad y'(0)=0,\end{cases}
\quad\Longrightarrow\quad y=\cosh t=\frac{e^t+e^{-t}}2,
$$

$$
\begin{cases}y''-y=0,\\y(0)=0,\quad y'(0)=1,\end{cases}
\quad\Longrightarrow\quad y=\sinh t=\frac{e^t-e^{-t}}2.
$$

$y''=-y$ 对应三角函数的往复振荡，$y''=y$ 对应双曲函数所包含的指数增长与衰减。方程只差一个符号，解的行为却明显不同。

因此，不一定先有一个熟悉的函数，再去验证它满足某个方程；也可以先给方程和足够的条件，再把选出的解作为一个函数来研究。

### 用积分定义函数：sinc 与正弦积分

函数也可以直接由积分定义。例如采用不带 $\pi$ 的 sinc 约定，令

$$
\operatorname{sinc}(t)=\int_0^1\cos(st)\,ds.
$$

当 $t\ne0$ 时，对积分变量 $s$ 求原函数，得到
...
当 $t=0$ 时，直接由积分得到 $\operatorname{sinc}(0)=\int_0^1 1\,ds=1$。因此，这个积分定义同时给出了 $\sin t/t$ 在零点的连续延拓。信号处理中也常用 $\sin(\pi t)/(\pi t)$ 的归一化约定，使用时要说明采用哪一种。
