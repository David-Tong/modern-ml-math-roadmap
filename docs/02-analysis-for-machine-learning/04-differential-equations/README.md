...
**十七世纪：微积分让变化规律成为方程。** Newton、Leibniz 以及 Bernoulli 家族等人的工作，把几何、运动和变化率联系起来。早期微分方程研究大量围绕一阶、二阶方程展开，求积、变量代换和级数都是重要工具。这是 ODE 发展的起点之一。

**十八世纪：弦、流体和势场推动 PDE 发展。** 描述一个质点，可以求它的位置 $y(t)$；描述整根弦，就要同时记录不同位置的位移 $u(x,t)$。d'Alembert 在 1747 年发表的弦振动研究中给出了波方程，Euler 等人继续推动偏微分方程及其物理应用。研究对象从有限个状态扩展到空间中的连续分布，空间导数因而成为方程的一部分。

**十九世纪：热传导把函数展开与 PDE 求解紧密联系起来。** Fourier 在 1822 年出版《热的解析理论》，用三角级数研究热传导。一个复杂的初始温度分布可以分解成许多空间模式，每个模式的系数按自己的 ODE 演化，再叠加回完整温度场。这条思路把函数展开、ODE 求解与 PDE 中的空间模式联系起来。

**十九至二十世纪：从求公式扩展到研究解本身。** 存在性、唯一性以及定性行为逐渐成为独立的研究目标；二十世纪的弱导数、Sobolev 空间与弱解理论，又使一些不能逐点求导的对象进入 PDE 理论。数值方法则提供了另一种求解方式：用有限次计算近似轨迹或场，而不必先得到闭式公式。

这条历史线索说明的是研究对象和工具怎样扩展。ODE 与 PDE 此后一直共同发展：ODE 仍研究动力系统与控制，PDE 则把空间耦合、边界和函数空间的问题带到中心位置。

### 从求解公式到研究解的性质：另一条历史主线

除了研究对象从轨迹扩展到场，数学家对“求解”的理解也在逐步扩展。

**十七、十八世纪：尽量把解求成公式。** 微积分建立后，数学家努力把微分方程化成积分，或用熟悉的函数写出解。变量分离、积分因子、特征方程和常数变易等方法，体现了这种追求。

**面对更复杂的方程：求公式之外，还要能近似和计算。** 尤其对非线性方程，简单代换只适用于特殊结构。近似与数值方法很早就与解析方法并行发展；十九世纪的定性理论又进一步强调，不求出显式公式，也可以研究轨迹怎样变化。

**十八、十九世纪：初等函数不够，就扩展表示解的函数。** 特殊函数在十八世纪已有先例，十九世纪随着数学物理的发展得到系统研究。用微分方程、积分或级数定义新函数，使许多无法用初等函数表示的解仍能得到精确描述。这里的困难是“用什么写出解”，解本身仍可能十分光滑。

**十九、二十世纪：进一步追问解是否存在，以及什么才算解。** 存在性、唯一性和对数据的依赖，与求解公式的研究并行推进。后来在 PDE 中，粗糙数据、间断和奇性又使经典解的要求有时过于严格，促使数学家用积分关系定义弱解，并在函数空间中研究它们。

这条主线可以概括为：**求公式 → 寻找近似与更丰富的函数表示 → 研究解的性质，必要时扩大解的定义。** 各条路线相互补充，并非依次取代。尤其要区分“没有初等函数表达式”和“没有光滑解”：前者是表示问题，后者是正则性问题，不能混为一谈。
...
十七、十八世纪，随着微积分与力学的发展，Newton、Leibniz、Bernoulli 家族、Euler 等数学家的一项重要目标，是把微分方程的解求出来：尽量化为积分，或写成已知函数的公式。本章沿着这条路线，学习怎样利用方程的特殊结构得到解析表达。
...
本章接着第三章讨论“公式难求时怎么办”：特殊结构可以代换，适当范围内可以线性近似，还可以数值计算。难以写出初等函数解，不代表没有光滑解；求表达式与研究解的性质，是需要区分的两件事。
...
## 六、ODE 解的基本理论：存在性、唯一性与解的性质

微分方程的发展还带来了更基础的问题：即使有形式上的公式或级数，它是否真的给出一个解？解是否唯一，能延续多久？十九世纪 Cauchy、Lipschitz、Picard 等人的工作推动了这些问题的严格研究，它与特殊函数理论的发展是并行的。

本章只在 ODE 的范围内研究解的性质：是否存在、是否唯一、对初值是否连续依赖、能延续多久，以及具有多少阶连续导数。重点从“怎样写出答案”转向“这个初值问题能否确定一条可靠的解”。下面以显式初值问题
...
存在、唯一、连续依赖合在一起，就是 ODE 初值问题适定性的基本要求。
...
前面讨论了存在性、唯一性、对数据的连续依赖和最大存在区间。还需要区分几个经常一起出现的词：解析表达、经典解、光滑解和数值解。它们关注的维度不同，并不是互斥的类别。

#### 解析表达：解能不能写成公式

应用语境里，“解析解”常指用函数、积分或收敛级数给出的精确表达。例如 $y'=-y$、$y(0)=1$ 的解是 $y=e^{-t}$。解也可以用特殊函数表示，不必只含多项式、指数和三角函数；第五章已经介绍了这种情形。

这里要区分“有解析表达”与“函数是解析的”：数学上的解析函数要求在每点附近能展开成收敛的幂级数。写出一个公式，并不自动保证其中的函数具有这种正则性。例如初值问题 $y'=|t|$、$y(0)=0$ 有显式解 $y(t)=t|t|/2$，但它在零点没有二阶导数，因此并不是解析函数。
...
对显式 ODE $x'=f(t,x)$，若右端 $f$ 在解所经过的区域内是 $C^k$ 函数，则经典解可逐步获得 $C^{k+1}$ 正则性。例如 $f$ 连续时，$x'=f(t,x(t))$ 连续；若 $f$ 还有连续的一阶偏导数，再对右边用链式法则求导，就得到连续的 $x''$。因此，光滑右端对应的局部解也光滑；无法写成初等函数，并不会使解失去光滑性。

#### 数值解：用有限信息近似解

数值解通常给出离散时刻的近似值 $X_n\approx x(t_n)$，或用有限个系数表示一个近似函数。它一般不会精确满足原来的微分方程，需要研究误差、稳定性和收敛性。

这里“数值解”描述的是对 ODE 精确解的近似。例如第四章的 Euler 法从初值问题的积分关系构造离散更新；所得数值序列不是原方程的精确解，需要用误差分析说明近似有多可靠。

#### 适定性：公式之外，还要检查问题是否可靠

存在、唯一、连续依赖是对整个问题的要求。说解连续依赖于数据时，需要说明使用什么范数、在哪个区间比较解；这与解是否有闭式公式无关。

因此读到一个“解”时，可以依次问：它是精确表达还是数值近似？是否具有方程所需的导数、逐点满足原方程？在什么区间成立，具有多少正则性？给定数据是否确定唯一解，误差能否得到控制？这些判断都不依赖于我们是否已经找到初等函数形式的答案。
...
第六章在 ODE 中区分了解的表达方式、经典解、光滑解和数值解。来到 PDE，还要考虑空间区域、边界和较低正则性的解，下面进一步引入弱解。
