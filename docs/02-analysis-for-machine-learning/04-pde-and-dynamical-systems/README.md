...
这里还要分清两层：空间算子决定候选形状，区域和边界条件负责筛选。二阶导数这个算子本身会让指数、正弦、余弦这类函数变得很自然；但在区间 $[0,L]$ 上再加上两端固定

$$
X(0)=X(L)=0
$$

以后，余弦项和绝大多数频率都被排除了，最后只留下
...
为什么会有一串函数？因为一个连续系统通常不只有一种允许形状。两端固定的细杆可以只鼓起一次，也可以中间多一个节点，还可以有更多起伏：
...
一个复杂的初始温度分布，通常不可能只靠其中一个模式表示出来。就像一个普通向量不一定只沿着一个坐标轴，它往往要由多个基向量合成；一个函数也往往要由多个基本函数合成。所以初始条件会变成一串系数：

$$
f(x)=\sum_{n=1}^{\infty}b_n\sin\frac{n\pi x}{L}.
$$

这里的 $b_n$ 就像坐标。$b_1$ 表示第一个低频模式占多少，$b_2$ 表示第二个模式占多少，$b_3$ 表示第三个模式占多少。函数原来是一整条曲线；展开以后，它变成了一串数。求 PDE 的过程也因此变得清楚：先把初始状态投影到基本模式上，再让每个模式按照方程规定的方式演化。

物理上，这些特征函数就是系统的固有模态。弦振动里，它们是基本振型；热传导里，它们是基本温度形状；声腔、电磁腔、量子系统里，也会出现类似的模态。复杂现象不是一团没有结构的复杂，而是许多简单模态的叠加。每个模态有自己的空间形状，也有自己的时间命运：热方程里，高频模态衰减得快；波方程里，模态会振荡；量子问题里，模态会对应能级。

这件事当然和 Fourier 变换有关系。Fourier 级数处理的是有限区间或周期问题，基本模式是一串离散的正弦、余弦；Fourier 变换处理的是整条实线上的问题，空间没有固定长度，频率不再是一串离散编号，而变成连续变量 $\xi$：
...
所以 Fourier 级数和 Fourier 变换不是两件完全无关的技巧。它们共享同一个深层思想：用最适合微分算子的基本波形来表示函数。有限区间上是离散模态，无限直线上是连续频率；背后的精神都是“把复杂函数拆到基本模式上”。

更系统的理论叫 Sturm-Liouville 边值问题。很多空间方向的 ODE 最后会变成

$$
-(p(x)X')'+q(x)X=\lambda r(x)X.
$$

在合适的边界条件下，它会产生一组特征值和特征函数。这些特征函数具有正交性，意义很像线性代数里的正交基：复杂函数可以投影到这些特征函数上，展开成一串模式。前面热方程里出现的正弦函数，就是这一大类理论中最简单、最经典的例子。

不过也要补一句：不是所有 PDE 都能漂亮地变成“求一组基”。这种图像最适合线性方程、规则区域和自伴随算子。遇到强非线性、复杂边界、冲击波、湍流或者解不够光滑的问题，事情会困难得多，可能需要弱解、能量估计、数值方法或动力系统方法。但在第一遍学习数学物理方程时，特征函数级数仍然是最重要的入口之一，因为它把 PDE 的数学结构和物理图像都说清楚了。

所以，分离变量法真正教给我们的不是“热方程答案长这样”，而是一种看待线性 PDE 的方式：先找到系统自己的坐标系，再把初始状态放进这个坐标系里，最后让每个坐标按照方程规定的方式运动。空间算子决定候选模式的形状类型，区域和边界条件决定哪些模式被允许，初始条件决定每个模式有多重，方程本身决定这些模式如何随时间演化。
## 五、PDE 求解方法的工具箱
