...
最典型的双曲型 PDE 是波方程。特别是在一维情形里，标准形式也常叫**达朗贝尔方程**：
...
$$
a(x,y,z)\frac{\partial u}{\partial x}
+b(x,y,z)\frac{\partial u}{\partial y}.
$$
...
这不是从 $ds/dx$ 变形来的，也不是把分母倒过来算出来的；它是通过“比较系数”选出来的。$s$ 是沿曲线走的参数，$dx/ds$ 表示沿曲线走时 $x$ 方向的变化速度，$dy/ds$ 表示 $y$ 方向的变化速度。我们把这两个速度选成 PDE 里 $u_x,u_y$ 前面的系数，链式法则才会和原方程对上。
...
$$
\frac{z}{x}=\frac{C_3}{C_1}.
$$

因此解可以写成

$$
\frac{z}{x}=\Phi\!\left(\frac{y}{x}\right),
$$
...
前面为了讲清楚输运方程，我们把特征线写成

$$
x=x(t).
$$

这等于把 $t$ 当成参数，让 $x$ 跟着 $t$ 走。对

$$
\frac{\partial u}{\partial t}+c\frac{\partial u}{\partial x}=0
$$

这种方程来说，这样做很自然，因为特征线满足
...
但这不是说所有特征线都必须写成 $x=x(t)$。如果最后算下来发现不能这样写，通常不是特征线法彻底错了，而是我们选错了参数。
...
表示 $u$ 沿着 $x$ 方向不变。它的意思是：固定一个 $t$，从左往右走，$u$ 的值不变。所以特征线是

$$
t=\text{常数},
$$

也就是 $(x,t)$ 平面里的水平线。这时硬要写成 $x=x(t)$ 就很不自然，因为沿着这条线走的时候，$t$ 根本没有变化。更合适的写法是另取一个参数 $s$：
...
这里 $s$ 可以理解成“沿着特征线走了多远”。这样就不用强迫 $x$ 一定是 $t$ 的函数。

所以更一般的一阶特征线法，不应该先死记 $x=x(t)$，而应该记住下面这件事：我们要找一条曲线
...
让 PDE 变成关于 $s$ 的 ODE。只有当 $t$ 可以作为合适参数时，才把它简化写成 $x=x(t)$。

还有一种更严重的失败：特征线存在，但不同特征线后来相交。比如非线性输运方程里，传播速度可能依赖于 $u$ 本身，快的特征线追上慢的特征线。相交以后，同一个点可能被两条特征线带来两个不同的值，经典光滑解就不能继续保持。这时就会进入弱解、激波、熵条件这些内容。第一遍不用展开，只要先知道：特征线法的假设不是无条件成功的；它成功时能把 PDE 化成 ODE，失败时要么换参数，要么换解的概念，要么换求解方法。
