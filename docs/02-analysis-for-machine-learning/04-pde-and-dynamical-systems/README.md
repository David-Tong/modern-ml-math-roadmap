...
拉普拉斯算子最常见、也最应该先理解的对象，是标量场。所谓标量场，就是每个空间位置上放一个数。温度场 $u(x,y)$、电势场 $u(x,y)$、高度场 $u(x,y)$ 都是这样：给定一个位置，它返回一个数。
...
它不是在问“这个点的数值有多大”，而是在问“这个点和周围相比有多突出”。一个点本身温度很高，但周围也同样高，那么它并不特别突出；另一个点温度未必最高，但如果它比周围明显高出一截，它就是一个局部热峰。拉普拉斯算子看的正是这种局部对比。

#### 一维标量场：看左右邻居

如果标量场只沿一条线变化，比如细杆上的温度 $u(x,t)$，固定时间 $t$ 后就得到一个一维函数 $u(x)$。这时拉普拉斯算子就是二阶空间导数：
...
拉普拉斯算子的直觉，就是把中间点和左右邻居比较。右边邻居和中间点的差是 $u_{i+1}-u_i$，左边邻居和中间点的差是 $u_{i-1}-u_i$。两边加起来：
...
当点越来越密，离散的 $u_i$ 变成连续的 $u(x)$，这个差分就变成二阶导数：
...
所以在一维里，$\Delta u$ 可以读成：这个点相对左右邻居是凸出来、凹下去，还是差不多平。

#### 二维标量场：看四周邻近位置

如果标量场铺在平面上，比如温度图 $u(x,y)$ 或电势图 $u(x,y)$，那么一个点不只有左右邻居，还有上下邻居。拉普拉斯算子就把两个空间方向上的二阶变化加起来：
...
它的意思仍然是同一句话：这个点相对周围是不是突出。只是现在“周围”不再只是左右两个点，而是平面里一圈邻近位置。

如果当前位置比周围高，像一个局部山峰，通常 $\Delta u<0$；如果当前位置比周围低，像一个局部谷底，通常 $\Delta u>0$；如果当前位置和周围平均水平差不多，$\Delta u$ 接近 $0$。

三维标量场也是同样思路，只是再加一个 $z$ 方向：
...
历史上，拉普拉斯算子和势函数问题关系很深。十八、十九世纪研究引力、电势、流体和热传导时，人们经常遇到这样的问题：空间里每个点都有一个势函数或温度函数，它在没有源、没有汇、没有外部注入的地方，应该怎样达到局部平衡。这个平衡条件后来写成：

$$
\Delta u=0.
$$

这就是 Laplace 方程。它以 Pierre-Simon Laplace 的名字命名。这里要稍微说准确一点：不是说这个方程完全凭空由 Laplace 一个人“发明”。类似的方程在 Euler、d’Alembert 等人研究流体和复变函数早期问题时已经出现过。它后来被称为 Laplace 方程，主要是因为 Pierre-Simon Laplace 在十八世纪后期研究引力势和天体力学时系统使用了它，并把它变成势理论里的核心方程。后来人们又把方程里的这个核心空间算子单独拿出来，称为 Laplace operator，也就是拉普拉斯算子。
