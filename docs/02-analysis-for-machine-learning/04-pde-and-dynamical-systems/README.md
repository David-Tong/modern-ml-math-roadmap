...
难点不在于方程写不出来，而在于一般情况下解不出简单公式，而且长期运动可能非常复杂。后面讲动力系统时，可以把三体问题作为一个重要例子：它说明即便规律是确定的，系统行为也可能对初始条件非常敏感。
...
这里要区分两个名字：**达朗贝尔方程**通常指一维波动方程本身；**达朗贝尔公式**指这个方程在整条直线上的一个具体解公式。前者是方程，后者是解法给出的结果。多维情形一般更常直接叫波方程。

如果它描述的是一根理想弦、一个理想弹性体或者真空中的电磁波，而且没有阻尼、没有能量损失，那么它通常不会像热方程那样自动走向一个平衡终点。波会传播、反射、叠加，能量在不同位置和不同形式之间来回转换。所以，纯粹的波动方程本身不应该理解成“长时间以后自动变成 Laplace 方程”。

但是，如果我们研究的是**时间不再变化的静态状态**，那情况就不同了。假设 $u$ 已经不随时间变，便有
...
这时出现的确实是 Laplace 方程。注意这里不是说波动过程自己必然耗散到 Laplace 方程，而是说：一旦我们只看时间无关的静态场，双曲型方程里的时间部分消失，剩下的空间约束常常就是椭圆型方程。

中间的演化过程要另外看。如果系统没有阻尼，波动方程描述的是能量在场里传播和交换，通常会一直振荡下去，不一定收敛到静态解。如果系统有阻尼、摩擦、辐射损失，或者边界不断吸收能量，那么暂态波动会慢慢衰减。这个过程中，双曲型方程负责描述扰动怎样以有限速度传出去；阻尼项负责让振荡能量逐渐消失；等到时间变化项和速度项都可以忽略时，剩下的才是静态的空间方程。
...
这里 $\partial^2u/\partial t^2$ 表示惯性带来的振荡，$\gamma\partial u/\partial t$ 表示阻尼，$c^2\Delta u$ 表示空间相互作用。演化初期像波，会传播、反射、振荡；阻尼不断消耗运动，长时间以后如果真的达到静止，就有
...
## 四、分离变量法：把 PDE 拆成基本模式

这一章先集中讲一种最重要、也最适合入门的 PDE 求解方法：分离变量法。它的核心不是背公式，而是把一个复杂的场拆成许多简单模式：先找空间形状，再找时间变化，最后把这些模式叠加起来。

不过在真正分离变量之前，要先知道一个 PDE 问题由哪些部分组成。ODE 里常常是“方程 + 初始值”；PDE 一旦多了空间变量，就会多出区域、边界条件、初始条件这些东西。把这些对象认清楚，后面才知道为什么边界会选出正弦模式，为什么初始条件会决定 Fourier 系数。
### 先看一个 PDE 问题由什么组成

在真正讲求解方法之前，先要知道一个 PDE 方程到底在问什么。PDE 不是只写出一个等式就完了。通常要同时说明：未知量是什么，方程是什么，在哪个区域里求解，边界上发生什么，一开始是什么状态。

#### 和 ODE 相比，多了什么

先和 ODE 对照一下。一个常微分方程通常研究一条轨迹，比如

$$
x'(t)=f(x(t),t).
$$

未知量是 $x(t)$，它主要依赖时间 $t$。给定初始值 $x(0)$ 后，很多 ODE 问题就变成了：这一个状态点以后怎样运动？所以 ODE 的图像常常是一条轨迹。

PDE 多出来的关键，是空间变量。未知量不再只是 $x(t)$，而可能是

$$
u(x,t),\qquad u(x,y,t),\qquad u(x,y,z,t).
$$

也就是说，我们不是只追踪一个点，而是在追踪一整片空间里的量。温度、电势、位移、压强、浓度、速度，都可以这样变成场。

空间变量一出现，就会多出几件 ODE 里不那么突出的事情。首先要说清楚区域：是在一根杆上、一个矩形里、一个圆盘里，还是整个空间里？其次要说清楚边界：区域边缘的温度、电势、位移或通量被怎样约束？最后，如果问题还随时间演化，就要再给初始条件：一开始整个场是什么样。

所以可以粗略地说：ODE 常常是“方程 + 初始值”；PDE 往往是“方程 + 区域 + 边界条件 + 初始条件”。这就是 PDE 比 ODE 多出来的核心复杂性。

#### 未知量：要求的场是什么

PDE 的未知量通常是一个场，也就是依赖多个变量的函数。比如热传导里，未知量可以写成

$$
u(x,t).
$$

这里 $x$ 表示空间位置，$t$ 表示时间，$u(x,t)$ 表示“位置 $x$ 在时间 $t$ 的温度”。所以我们要求的不是一个数，也不是一条曲线，而是一整个温度场。

如果是二维区域里的温度，就可能写成 $u(x,y,t)$，因为温度可能随时间变化；如果是静电势或稳态电势问题，常常写成 $u(x,y)$，这里没有 $t$，表示我们关心的是已经平衡下来的空间分布，而不是它怎样随时间演化。如果是流体速度，未知量甚至可能是向量场，比如

$$
\boldsymbol{v}(x,y,t)=(v_1(x,y,t),v_2(x,y,t)).
$$

所以第一步永远是问：未知函数到底依赖哪些变量？它表示标量场，还是向量场？

#### 方程：场必须满足什么变化规律

方程给出的是场的内部规律。比如一维热方程

$$
\frac{\partial u}{\partial t}=a^2\frac{\partial^2u}{\partial x^2}
$$

是在说：温度随时间的变化，由空间上的弯曲程度决定。哪里温度分布弯得厉害，哪里就会更快发生变化。

这里的方程只管区域内部发生什么。它还没有告诉我们细杆有多长，两端温度是多少，也没有告诉我们一开始温度怎么分布。

#### 区域：在哪个地方求解

区域说明变量的范围。比如一根长度为 $L$ 的细杆，对应的空间区域就是
...
所以热方程问题不是在整个实线上求解，而是在这根有限细杆上求解。区域一变，解法经常也会变：有限区间容易出现 Fourier 级数；无限直线更自然地用 Fourier 变换；圆域、球域则常常引出 Bessel 函数和 Legendre 函数。

#### 边界条件：边缘上被怎样约束

边界条件说明区域边缘发生什么。对细杆来说，边界就是两个端点 $x=0$ 和 $x=L$。如果两端温度固定为 $0$，就写成
...
这叫 Dirichlet 边界条件：直接规定边界上的函数值。

还有别的边界条件。比如如果规定端点没有热量流出，就会出现类似

$$
\frac{\partial u}{\partial x}(0,t)=0
$$

这样的条件。它不是规定温度本身，而是规定温度在边界处的空间变化率，这类条件常叫 Neumann 边界条件。

边界条件很重要，但它不是单独决定空间形状。更准确地说，方程里的空间微分算子先给出候选形状，区域和边界条件再筛选哪些形状、哪些频率真正允许出现。后面分离变量法里，两端固定为 $0$ 正是把候选模式筛成正弦函数序列的原因。

#### 初始条件：一开始是什么样

如果 PDE 描述的是随时间演化的问题，还需要初始条件。比如

$$
u(x,0)=f(x)
$$

表示 $t=0$ 时，整根细杆的温度分布是 $f(x)$。这里 $f(x)$ 不是一个数，而是一条沿空间展开的初始温度曲线。

初始条件回答的是“从哪里开始演化”。边界条件回答的是“边缘一直受到什么约束”。这两个东西不要混在一起：初始条件管 $t=0$ 这一刻，边界条件管空间边缘；在热方程这种问题里，两者要配合得上。
...
这几行分别告诉我们：内部按什么规律变化，在哪个区域里变化，边界怎样被固定，一开始是什么温度分布。只有这些信息合在一起，才真正构成一个可以求解的 PDE 问题。

### 分离变量法的主线

#### Fourier 法这个名字从哪里来

分离变量法是数学物理方程里最经典的方法之一，也常被叫作 Fourier 法。这个名字和 Joseph Fourier 本人研究热传导有关。Fourier 在研究固体中的热怎样传播时发现，一个复杂的初始温度分布，可以分解成许多正弦、余弦模式；每个模式单独按方程演化，最后再叠加起来。他在 1822 年出版的《热的解析理论》系统展示了这种思想，后来 Fourier 级数、Fourier 变换也因此成为数学物理方程的基本语言。

所以在很多数学物理方程教材里，“Fourier 法”常常就是“分离变量法 + Fourier 展开”的合称。先分离变量，是为了找出方程允许哪些基本模式；再做 Fourier 展开，是为了把给定的初始条件或边界条件写成这些基本模式的叠加。具体说，就是先用分离变量把 PDE 拆成 ODE，再得到一组正弦、余弦或更一般的特征函数，最后把初始条件或边界条件展开成这些模式的级数。这样一来，一个复杂问题就被拆成了一串简单问题。


#### 适合什么样的问题

它适用于区域形状规则、边界条件比较整齐、方程本身具有线性结构的问题。有限区间上的热方程、两端固定弦的波动方程、矩形区域里的 Laplace 方程、圆域或球域里的边值问题，都经常用这个方法。它的基本想法是：把多变量函数拆成几个单变量函数的乘积。


### 例子：两端固定温度的细杆

下面用一个具体例子说明这套方法。想象一根长度为 $L$ 的细杆，位置用 $x$ 表示，时间用 $t$ 表示，温度场写成 $u(x,t)$。两端一直和外界接触，被固定在温度 $0$。杆内部的热传导满足：
...
这表示杆的左端和右端温度始终为 $0$。初始条件是：

$$
u(x,0)=f(x).
$$

这里 $f(x)$ 是一开始整根细杆的温度分布。它不是一个常数，而是一个函数：每个位置 $x$ 开始时有多热，都由 $f(x)$ 给出。
...
呢？这里不是在断言所有解一开始就一定长成一个乘积，也不是凭空猜最终答案。分离变量法更像是在问：这个方程有没有一些最简单的特解，它们的空间形状和时间变化可以分开？如果有，我们就先把这种最简单的特解找出来。
...
代入方程时，先分别算两个偏导数。因为 $X$ 只依赖 $x$，$T$ 只依赖 $t$，所以：

$$
\frac{\partial u}{\partial t}
=
\frac{\partial}{\partial t}\bigl(X(x)T(t)\bigr)
=X(x)T'(t),
$$
...
\frac{\partial^2u}{\partial x^2}
=
\frac{\partial^2}{\partial x^2}\bigl(X(x)T(t)\bigr)
=X''(x)T(t).
$$
...
这里 $T'(t)$ 表示 $T(t)$ 对时间 $t$ 的普通一阶导数，也就是时间因子变化得多快；$X''(x)$ 表示 $X(x)$ 对位置 $x$ 的普通二阶导数，也就是空间形状弯曲得多厉害。最后写成 $T'(t)$ 和 $X''(x)$，是因为分离变量以后，$T$ 已经只是单变量函数，$X$ 也只是单变量函数；对单变量函数求导，用普通导数记号更简洁。
...
注意这里左边只依赖时间 $t$，右边只依赖位置 $x$。可是它们要对所有 $x$ 和 $t$ 都相等，这只有一种可能：两边其实都等于同一个常数。这个常数叫分离常数。
...
在两端固定为 $0$ 的边界条件下，这个方程会自然给出正弦函数。也就是说，$-\lambda$ 不是从天上掉下来的，而是“两个互不依赖的变量要处处相等，所以只能等于常数”；负号只是为了让后面的空间方程写得更符合正弦模式。


#### 第三步：空间方程选出形状

分离变量以后，PDE 被拆成两个 ODE：
...
它要解决的不是“温度随时间怎么变”，而是“这根细杆上哪些空间形状可以出现”。这里的空间形状，就是只沿着 $x$ 方向看温度分布：左端是多少，右端是多少，中间是鼓起来、凹下去，还是来回起伏。

![空间方程选出的正弦温度形状](assets/separation-space-modes.svg)

这张图里的横轴是位置 $x$，从左端 $0$ 到右端 $L$；曲线的高低表示空间函数 $X_1(x)$ 的相对大小，也就是一种最简单的温度分布形状。注意这里还没有讨论时间，所以图上画的不是温度场完整的演化过程，而是某一时刻沿细杆的温度分布。曲线两端都落在 $0$，因为细杆两端温度被固定为 $0$。

不过这些形状不能随便画。因为细杆两端一直固定在温度 $0$，所以无论时间怎么变，乘积形式
...
所以空间方程真正要找的是这样的函数 $X(x)$：它既满足 ODE

$$
X''+\lambda X=0,
$$

又在两端取值为 $0$。满足这两个条件的非零函数，才是这根细杆允许的基本空间形状。
...
代入左端边界 $X(0)=0$，得到 $B=0$；再代入右端边界 $X(L)=0$，得到 $AL=0$，所以 $A=0$。因此 $X(x)$ 只能是零函数。这不是一个真正的基本形状。

如果 $\lambda<0$，可以写成 $\lambda=-\mu^2$，其中 $\mu>0$。空间方程变成
...
$$
A\bigl(e^{\mu L}-e^{-\mu L}\bigr)=0.
$$

因为 $\mu>0$、$L>0$，所以 $e^{\mu L}-e^{-\mu L}\ne 0$，只能有 $A=0$，于是 $B=0$。所以 $\lambda<0$ 时也只剩零解。

真正留下非零形状的是 $\lambda>0$。设 $\lambda=\mu^2$，空间方程变成
...
这里的下标 $n$ 只是编号：第 $1$ 个、第 $2$ 个、第 $3$ 个……每一个正整数 $n$ 都给出一种允许的空间形状。把 $\mu_n$ 代回 $X(x)=B\sin \mu x$，并把不影响形状的常数 $B$ 取成 $1$，得到

$$
X_n(x)=\sin\frac{n\pi x}{L}.
$$

这就是空间方程给出的结果：两端固定为 $0$ 的细杆，只允许这些正弦形状作为基本模式。$n=1$ 时中间鼓起一次；$n=2$ 时中间多一个节点；$n$ 越大，沿着细杆起伏越多。
...
$$
X_n''(x)=-\left(\frac{n\pi}{L}\right)^2\sin\frac{n\pi x}{L}.
$$

又因为 $\lambda_n=\left(\frac{n\pi}{L}\right)^2$，所以

$$
X_n''(x)+\lambda_nX_n(x)=0.
$$

这说明 $X_n(x)=\sin\frac{n\pi x}{L}$ 确实同时满足空间 ODE 和两端固定的边界条件。

#### 第四步：时间方程给出衰减

再看时间方程。空间方程已经告诉我们允许的形状是 $\sin\frac{n\pi x}{L}$；时间方程接着回答：每一种形状会怎样随时间变化。
...
把 $\lambda_n$ 代进去，得到第 $n$ 个时间因子：

$$
T_n(t)=e^{-a^2(n\pi/L)^2t}.
$$

![时间方程给出的衰减因子](assets/separation-time-decay.svg)

这张图只看时间方向。它不再关心温度沿 $x$ 怎么分布，只看同一个空间形状的强度怎样随时间变小。曲线从 $1$ 开始，然后逐渐衰减到 $0$，表示原来的温度形状整体变淡。

于是第 $n$ 个乘积型解是：

$$
u_n(x,t)=e^{-a^2(n\pi/L)^2t}\sin\frac{n\pi x}{L}.
$$

![空间形状乘上时间衰减以后得到温度场](assets/separation-product-field.svg)

这第三张图才有“场”的味道：横轴是位置 $x$，纵轴是时间 $t$，颜色表示温度 $u(x,t)$。空间图告诉我们每一行沿 $x$ 是什么形状；时间图告诉我们这条形状怎样随 $t$ 变弱。把两者乘起来，就得到一个同时依赖 $x$ 和 $t$ 的温度场。
...
因为热方程是线性的，所以如果 $u_1,u_2,u_3,\ldots$ 都是解，把它们乘上常数再相加，仍然是解。也就是说，我们可以把许多基本模式叠加起来，得到更复杂的温度场。

空间算子和边界条件已经一起把空间模式筛成了

$$
\sin\frac{n\pi x}{L},\qquad n=1,2,3,\ldots
$$

所以初始温度 $f(x)$ 要先展开成这些正弦模式的叠加：

$$
f(x)=\sum_{n=1}^{\infty} b_n\sin\frac{n\pi x}{L}.
$$

这里的 $b_n$ 就是第 $n$ 个模式的权重。所谓第 $n$ 个模式，就是

$$
\sin\frac{n\pi x}{L}.
$$

$n$ 小的时候，空间变化慢，起伏少，叫低频模式；$n$ 大的时候，空间变化快，起伏密，叫高频模式。第 $1$ 个模式是最低频、最粗的形状，只在中间鼓起一次；第 $2$ 个模式有更多起伏；第 $3$ 个模式更细。$b_1$ 大，说明初始温度主要像一个大包；$b_3$ 大，说明初始温度里有更细的波纹。
...
这就是为什么不是只算一个 $b_n$，而是要算一整串

$$
b_1,b_2,b_3,\ldots
$$

因为一个普通的初始温度形状，可能同时含有很多种空间模式。每个 $b_n$ 都告诉我们：第 $n$ 个模式在初始温度里占多大分量。

每个模式进入时间演化以后，还要乘上自己的衰减因子：
...
这条公式的意思是：初始条件决定每个模式的权重 $b_n$，热方程决定每个模式随时间怎么衰减。$n$ 越大，指数里的负数越大，高频模式衰减越快。所以在热方程里，$b_1$ 往往控制长期留下来的主要形状；$b_3,b_5$ 这类高频项更多影响早期细节。
...
这条公式里真正由初始条件决定的是 $b_1,b_2,b_3,\ldots$。同一个热方程、同一组边界条件下，如果初始温度 $f(x)$ 不同，算出来的系数就不同；系数不同，叠加出来的温度场也就不同。

![不同初始条件通过 Fourier 系数决定温度场](assets/separation-initial-to-field.svg)

先看图里的 A。A 的初始条件只含第一模式：

$$
f_A(x)=\sin\frac{\pi x}{L}.
$$

因此它的主要系数就是

$$
b_1=1,\qquad b_2=0,\qquad b_3=0.
$$

这时温度沿 $x$ 的形状很简单：从左端 $0$ 升高，到中间达到最高，再降回右端 $0$。随着时间 $t$ 增大，这个形状基本不改变位置和样子，只是整体变淡，因为它只乘上一个时间因子

$$
e^{-a^2(\pi/L)^2t}.
$$

再看图里的 B。B 的初始条件是第一模式加第二模式：

$$
f_B(x)=\sin\frac{\pi x}{L}+0.35\sin\frac{2\pi x}{L}.
$$

所以它的前三个系数是

$$
b_1=1,\qquad b_2=0.35,\qquad b_3=0.
$$

这里最关键的是 $b_2$。第二模式不是简单地增加一个更高的波峰，而是在左半段为正、右半段为负：

$$
\sin\frac{2\pi x}{L}>0\quad (0<x<L/2),
$$

$$
\sin\frac{2\pi x}{L}<0\quad (L/2<x<L).
$$

所以它加到第一模式上以后，左半段被抬高，右半段被压低。结果不是出现左右对称的两个小波峰，而是原来的大波峰向左偏。

![第一模式和第二模式的叠加](assets/mode-b1-b2-superposition.svg)

这张图把叠加过程单独拆开看：蓝线是第一模式，红线是第二模式，黑线是两者相加后的初始温度形状。读它的时候只看 $x$ 方向：同一个位置上，黑线的高度就是蓝线高度加红线高度。左半段红线在基准线上方，所以黑线被抬高；右半段红线在基准线下方，所以黑线被压低。

最后再看时间方向。A 的温度场比较简单，因为只有第一模式在衰减。B 的温度场一开始是偏的，因为第二模式参与了叠加；但第二模式的时间因子是
...
第二模式指数里有 $2^2$，所以它比第一模式衰减得更快。于是 B 的演化过程是：一开始波峰向左偏；过一段时间，第二模式先被热方程抹掉；最后温度场越来越像只剩第一模式的大波峰。

这就是图里两张温度场不同的原因。不是方程变了，也不是边界条件变了，而是初始条件改变了 $b_n$；$b_n$ 改变了各个空间模式的权重；不同模式又有不同的衰减速度，所以最后得到的 $u(x,t)$ 不同。

所以在这个问题里，初始条件 $f(x)$ 确实会决定解。更准确地说：方程、边界条件和初始条件一起决定唯一解。边界条件先决定可以使用哪些空间模式；初始条件决定每个模式前面的系数 $b_n$；这些系数一旦确定，后面的时间演化也就由指数因子自动确定了。
### 初始条件还要和边界条件相容

不过，初始条件不能随便给。只给方程还不够，只给初值也不够；要把方程、区域、边界条件和初始条件配在一起，问题才真正确定。对这个热方程例子来说，两端一直固定为 $0$，所以初始温度最好也满足

$$
f(0)=0,\qquad f(L)=0.
$$

比如能不能取 $f(x)=x$？如果只看杆内部 $0<x<L$，它当然可以表示一种初始温度：越往右温度越高。但它满足

$$
f(0)=0,\qquad f(L)=L,
$$

和右端边界 $u(L,t)=0$ 不一致。因此如果想要经典意义下光滑衔接的解，$f(x)=x$ 不是这个边界条件下最自然的选择。
...
因为它在两端都等于 $0$。它不是单独一个正弦模式，而是要展开成一串正弦模式：
...
=\frac{4L^2\bigl(1-(-1)^n\bigr)}{n^3\pi^3}.
$$

所以当 $n$ 是偶数时，$b_n=0$；当 $n$ 是奇数时，
...
$$
u(x,t)=\sum_{\substack{n=1\\ n\ \mathrm{odd}}}^{\infty}
\frac{8L^2}{n^3\pi^3}
e^{-a^2(n\pi/L)^2t}
\sin\frac{n\pi x}{L}.
$$

这正好说明了 Fourier 法的用处：初始形状 $x(L-x)$ 看起来不是正弦函数，但它可以拆成许多正弦基本模式；之后每个模式按照自己的指数因子衰减，最后叠加成完整的温度场。

### 方法边界：乘积形式什么时候走不通

前面尝试 $u(x,t)=X(x)T(t)$，不是说每个 PDE 都能这样解。如果这个尝试不合适，代回方程以后通常会出现两种情况：要么变量根本分不开，要么分出来的 ODE 加上边界条件以后只有零解。那就说明这条路走不通，需要换别的方法。

所以分离变量法不是万能公式。它更像一种测试：如果方程、区域和条件配合得好，乘积型特解会自然出现；如果配合不好，计算本身会告诉我们这个方法不适用。

举一个最简单的例子。真正会失败的是变量混在一起、没法拆开的情形。比如

$$
\frac{\partial u}{\partial t}=\sin(xt)u.
$$

代入 $u(x,t)=X(x)T(t)$，得到

$$
X(x)T'(t)=\sin(xt)X(x)T(t).
$$
...
左边只依赖 $t$，右边却同时依赖 $x$ 和 $t$。这就不是前面那种“只含 $t$ 的一边 = 只含 $x$ 的一边”的结构。换句话说，一个单独的乘积 $X(x)T(t)$ 没法把这个方程拆成两个独立的 ODE。

这里不是说 $\sin(xt)$ 完全不能处理。它当然可以写成泰勒级数：

$$
\sin(xt)=xt-\frac{x^3t^3}{3!}+\frac{x^5t^5}{5!}-\cdots.
$$

但这会变成一串无穷多项，而不是一个单独的空间因子乘时间因子。分离变量法的优势正在于把一个 PDE 干净地拆成一组 ODE；如果代入以后拆不干净，就说明这个乘积型路线不适合当前问题，需要换别的方法。
### 为什么热方程这个例子能走通

这个热方程例子能走通，关键有两个原因。第一，方程是线性的：如果 $u_1$ 和 $u_2$ 都是解，那么它们的线性组合也是解。这样我们才可以先求一个个简单模式，再把它们加起来。

第二，空间算子和两端固定为 $0$ 的边界条件配合得很规整，会自然筛出一组正弦函数：

$$
\sin\frac{n\pi x}{L},\qquad n=1,2,3,\ldots
$$

这些函数都满足边界条件：在 $x=0$ 和 $x=L$ 处都等于 $0$。它们就像这个区间上的基本振型。一般的初始温度 $f(x)$ 可以展开成这些正弦模式的级数，于是一般解就可以由许多乘积型特解叠加出来。

最后还要验证。不是因为我们写出了一个级数，它就自动是答案；要逐项检查它满足 PDE，满足边界条件，并且在 $t=0$ 时能够还原给定的初始函数 $f(x)$。这三步都过关，这个解才算真正构造完成。

### 分离变量法可以求解哪些方程

分离变量法不是只为热方程准备的。只要方程是线性的，区域比较规则，边界条件能和坐标方向配合起来，它就经常可以把 PDE 拆成几个 ODE。抛物型、双曲型、椭圆型方程里都有这样的例子。

所以从这章的例子看，分离变量法确实可以处理前面三种经典二阶 PDE 的典型代表：热方程、波方程和 Laplace 方程。但这句话要理解准确：它不是说所有抛物型、双曲型、椭圆型方程都能这样解，而是说这三类里最标准、最规整的模型方程，都可以用分离变量法展示出核心结构。

#### 一阶线性 PDE 也可以分离变量吗

可以，但要分情况。分离变量法不是只能处理二阶 PDE；只要方程结构合适，一阶线性 PDE 也可以用它。
...
$$
u(x,t)=Ae^{\lambda(t-x/c)}.
$$

这说明输运方程不是不能分离变量。只是对输运方程来说，特征线法通常更自然。因为特征线法一步就看出

$$
u(x,t)=u_0(x-ct),
$$

也就是初始形状整体沿速度 $c$ 平移。分离变量法则更像把这个平移过程拆成很多指数模式或 Fourier 模式，再叠加回来。

如果在周期区间上，用 Fourier 模式看得更清楚。设
...
$$
e^{ikx}e^{-ickt}=e^{ik(x-ct)}.
$$

这仍然是在说同一件事：每个 Fourier 模式都以速度 $c$ 向右传播。特征线法直接看整体形状；分离变量法看每个模式怎样传播。

因此，不能说“分离变量法处理不了一阶线性 PDE”。更准确地说：它可以处理一些结构规整的一阶线性 PDE，但一阶输运类方程的第一直觉通常是特征线法，因为特征线法直接抓住了传播路线。
#### 抛物型：热方程

前面已经用细杆热传导作为主要例子。热方程的典型形式是

$$
\frac{\partial u}{\partial t}=a^2\frac{\partial^2u}{\partial x^2}.
$$

分离变量以后，空间方向给出正弦模式，时间方向给出指数衰减：

$$
u_n(x,t)=e^{-a^2(n\pi/L)^2t}\sin\frac{n\pi x}{L}.
$$

所以热方程里，每个空间模式都会随时间慢慢变弱，频率越高衰减越快。这就是扩散和耗散的数学表现。

#### 双曲型：弦振动方程

分离变量法也可以解波方程。比如一根长度为 $L$、两端固定的弦，位移场 $u(x,t)$ 满足
...
这和热方程形成鲜明对比：同样是正弦空间模式，热方程让它们衰减，波方程让它们振荡。

#### 椭圆型：矩形区域里的平衡方程

椭圆型方程也可以用分离变量法。比如矩形区域
...
这里没有时间变量，$u(x,y)$ 可以表示稳态温度或静电势。假设左右两边固定为 $0$：
...
$$
Y(y)=Ae^{(n\pi/L)y}+Be^{-(n\pi/L)y}.
$$

也可以等价地写成双曲函数形式：

$$
Y(y)=C\sinh\frac{n\pi y}{L}+D\cosh\frac{n\pi y}{L}.
$$
...
所以我说“指数型”，最后写出 $\sinh$、$\cosh$，本质上是一回事。实际计算时用哪一种写法，取决于边界条件怎样更方便。比如在合适边界条件下，典型项会长成

$$
u_n(x,y)=\sin\frac{n\pi x}{L}\sinh\frac{n\pi y}{L}.
$$

这说明：椭圆型方程虽然不描述时间演化，也仍然可以分离变量。只是这时分离出来的不是“空间 + 时间”，而是 $x$ 方向和 $y$ 方向；一个方向给出正弦模式，另一个方向给出双曲函数模式。它描述的是边界条件如何共同决定内部的平衡场。

#### 齐次二阶 PDE 不一定都能分离变量

这里容易有一个误解：是不是只要一个 PDE 是齐次的、二阶的，就一定可以用分离变量法？答案是否定的。

“齐次”只说明方程里没有额外的外部源项，比如右边是 $0$；“二阶”只说明最高阶导数是二阶。这些条件有帮助，但还不够。分离变量法真正需要的是更强的结构：变量之间要能拆开，区域要适合某种坐标，边界条件要能和这些坐标方向配合起来。
...
这种形式，设 $u(x,t)=X(x)T(t)$ 后，能整理成“只含 $t$ 的一边 = 只含 $x$ 的一边”，于是分离变量法就能走下去。

但如果系数或变量混在一起，比如出现很难拆开的 $x,t$ 耦合项，代入 $X(x)T(t)$ 后就可能分不开。即便方程本身齐次、二阶，也不代表一定能得到一组漂亮的 ODE。

比如下面这个方程是线性的、齐次的，而且含有二阶导数：

$$
\frac{\partial^2u}{\partial t^2}=(x+t)\frac{\partial^2u}{\partial x^2}.
$$
...
$$
\frac{T''(t)}{T(t)}=(x+t)\frac{X''(x)}{X(x)}.
$$

右边的 $(x+t)$ 把两个变量加在一起，没法整理成“只含 $t$ 的一边 = 只含 $x$ 的一边”。这时就不能像热方程那样引入一个分离常数，然后得到两条清楚的 ODE。

再比如

$$
\frac{\partial^2u}{\partial t^2}=\sin(xt)\frac{\partial^2u}{\partial x^2},
$$

代入以后会得到

$$
\frac{T''(t)}{T(t)}=\sin(xt)\frac{X''(x)}{X(x)},
$$

同样无法拆开。它们都说明：问题不在于方程是不是齐次、二阶，而在于方程结构能不能真正分离。

所以判断分离变量法能不能用，不是只看“齐次不齐次”或“二阶不二阶”，而是看三件事：方程结构能不能分开，区域形状是不是规则，边界条件是不是能筛出合适的特征函数。
...
先说什么叫“齐次”和“非齐次”。对方程来说，齐次更准确的意思是：把方程所有含未知函数的项移到左边以后，右边没有额外的外部强迫项，也就是等于 $0$。对边界条件来说，齐次的意思是边界数据为 $0$。比如

$$
u(0,t)=0,\qquad u(L,t)=0
$$

就是齐次边界条件，因为两端都被固定为 $0$。相反，如果边界上给的是非零值，比如

$$
u(0,t)=0,\qquad u(L,t)=U_0,
$$

其中 $U_0\ne 0$，这就是非齐次边界条件。它的意思是：右端一直被外界强制保持在温度 $U_0$，不是自由地回到 $0$。
...
把所有项移到左边就是 $\frac{\partial u}{\partial t}-a^2\frac{\partial^2u}{\partial x^2}=0$，右边没有额外外部输入，可以看成齐次热方程；如果写成

$$
\frac{\partial u}{\partial t}=a^2\frac{\partial^2u}{\partial x^2}+q(x,t),
$$

多出来的 $q(x,t)$ 表示内部热源或外部输入，这就是非齐次方程。

#### 非齐次边界条件：用 $u=w+v$ 把边界拉回零

如果边界条件是非齐次的，常见处理是先做分解。比如把解写成
...
比如还是一根长度为 $L$ 的细杆，但这次两端温度不是 $0$，而是

$$
u(0,t)=0,\qquad u(L,t)=U_0.
$$

这就是非齐次边界条件。直接做正弦展开会不方便，因为正弦模式都在两端等于 $0$，它们没法直接拼出右端恒等于 $U_0$ 的边界。

这时先找一个只负责边界的函数 $w(x)$。最简单的选择是连接两端温度的直线：
...
$$
v(0,t)=u(0,t)-w(0)=0-0=0,
$$

右端有

$$
v(L,t)=u(L,t)-w(L)=U_0-U_0=0.
$$
...
如果原来的初始条件是 $u(x,0)=f(x)$，那么 $v$ 的初始条件就是

$$
v(x,0)=f(x)-w(x)=f(x)-\frac{U_0}{L}x.
$$

这样问题就被拆开了：$w$ 处理非零边界，$v$ 回到两端为 $0$ 的问题，可以继续用前面的 Fourier 正弦级数来解。最后再把两部分加回去，得到 $u=w+v$。

#### 非齐次方程：把内部源项也展开成模式

如果边界仍然是齐次的，但方程本身有外部源项，处理方式会稍微不同。比如
...
这里 $q(x,t)$ 可以理解成内部热源：某些位置、某些时刻不断给细杆加热。这个时候不能只靠自由衰减的模式

$$
e^{-a^2(n\pi/L)^2t}\sin\frac{n\pi x}{L}
$$

来描述全部解，因为系统一直被外部源项推动。
...
注意这里系数不再直接写成 $b_n e^{-a^2(n\pi/L)^2t}$，而是写成时间函数 $c_n(t)$。时间衰减项并没有消失，而是被包含在 $c_n(t)$ 里面了。

如果没有源项，$q(x,t)=0$，那么 $c_n(t)$ 就会退回前面的形式：

$$
c_n(t)=b_n e^{-a^2(n\pi/L)^2t}.
$$

但现在有外部热源，模式强度一边自然衰减，一边又被 $q(x,t)$ 补充或推动，所以不能只写成一个固定指数因子。我们先用 $c_n(t)$ 表示“第 $n$ 个模式在时间 $t$ 的强度”，再由方程去求它。

同样地，把源项也展开成正弦级数：

$$
q(x,t)=\sum_{n=1}^{\infty}q_n(t)\sin\frac{n\pi x}{L}.
$$

代回热方程，并比较每个正弦模式前面的系数，就得到一串一阶 ODE：

$$
c_n'(t)+a^2\left(\frac{n\pi}{L}\right)^2c_n(t)=q_n(t).
$$

这就是非齐次方程的核心变化：原来每个模式只是自由衰减，现在每个模式都受到一个外部输入 $q_n(t)$ 的驱动。换句话说，PDE 仍然被拆成很多 ODE，但这些 ODE 变成了受迫方程。

这个 ODE 的解可以写成

$$
c_n(t)=b_n e^{-a^2(n\pi/L)^2t}
+\int_0^t e^{-a^2(n\pi/L)^2(t-s)}q_n(s)\,ds.
$$

第一项就是原来的自由衰减；第二项是外部源项从 $0$ 到 $t$ 不断输入后留下的影响。越早输入的热量，经过的衰减时间越长；越靠近当前时刻输入的热量，衰减得越少。所以非齐次情形不是没有时间衰减，而是“自由衰减 + 源项累积影响”合在了一起。

比如如果 $q(x,t)$ 只含第一个模式，

$$
q(x,t)=Q\sin\frac{\pi x}{L},
$$

那么只有第一个模式被直接加热。对应的 ODE 是

$$
c_1'(t)+a^2\left(\frac{\pi}{L}\right)^2c_1(t)=Q,
$$

而其它 $c_n(t)$ 没有直接源项。这个方程的解会逐渐靠近一个稳定值，而不是单纯衰减到 $0$。这和非齐次边界条件的直觉类似：外部持续输入会改变系统最后趋向的状态。

### 为什么最后会写成特征函数级数

现在可以回头看分离变量法到底做了什么。它不是一个单纯的计算技巧，也不是碰巧把热方程算成了正弦级数。它背后有一个更大的图像：对很多线性 PDE 来说，求解的关键是找到这个连续系统自己的“坐标系”。

#### 函数也可以有坐标

在线性代数里，一个向量可以写成基向量的线性组合。比如

$$
\mathbf v=a\mathbf e_1+b\mathbf e_2.
$$

这里的 $a,b$ 是坐标，$\mathbf e_1,\mathbf e_2$ 是基向量。到了 PDE 里，未知量不是有限维向量，而是函数，比如初始温度分布 $f(x)$。于是问题变成：能不能也找到一组基本函数，让一般函数写成它们的叠加？

这就是特征函数级数的来源：把一个函数看成“无限维向量”，再用一组适合这个问题的基本函数来表示它。

#### 为什么要找形状不变的函数

为什么要找这种基本函数？可以先和 ODE 对照。解常系数线性 ODE 时，我们常常猜

$$
y=e^{rx},
$$

不是因为指数函数好看，而是因为指数函数求导以后还是自己：

$$
\frac{d}{dx}e^{rx}=re^{rx},
\qquad
\frac{d^2}{dx^2}e^{rx}=r^2e^{rx}.
$$

所以把 $e^{rx}$ 代进常系数 ODE，微分方程就会变成关于 $r$ 的代数方程。PDE 里找正弦、余弦、复指数，也是在做类似的事：找那些被空间微分以后形状不变的基本函数。

用大白话说，我们是在找“方程最顺手处理的形状”。一个普通形状丢进微分算子里，可能会被改得面目全非；但有些特殊形状丢进去以后，还是原来的形状，只是前面多了一个系数。比如

$$
-\frac{d^2}{dx^2}\sin\frac{n\pi x}{L}
=
\left(\frac{n\pi}{L}\right)^2\sin\frac{n\pi x}{L}.
$$

这句话的意思很朴素：$\sin(n\pi x/L)$ 经过 $-d^2/dx^2$ 处理以后，还是同一个正弦形状。它没有变成三角形、抛物线或者别的曲线，只是整体乘上了一个数 $(n\pi/L)^2$。

#### 和特征向量是什么关系

这和矩阵的特征向量非常像。矩阵作用在普通向量上，通常会改变方向；但作用在某些特殊向量上，只会改变长度，不改变方向：

$$
A\mathbf v=\lambda \mathbf v.
$$

PDE 里的特征函数就是这个意思。只不过在线性代数里，特征对象是向量；在 PDE 里，特征对象是一整个函数形状。分离变量法不是硬把函数拆成正弦函数，而是在找空间微分算子的特征形状。

#### 空间算子给候选，边界条件做筛选

这里要分清两层：空间算子决定候选形状，区域和边界条件负责筛选。二阶导数这个算子本身会让指数、正弦、余弦这类函数变得很自然；但在区间 $[0,L]$ 上再加上两端固定

$$
X(0)=X(L)=0
$$

以后，在这个齐次 Dirichlet 边界条件下，余弦项和绝大多数频率都被排除了，最后只留下
...
这一串正弦模式。所以不是边界条件单独决定特征函数，也不是 PDE 单独决定最后答案；更准确地说，PDE 的空间算子给出候选形状，区域和边界条件把候选形状筛成允许的模态。

这里说的是两端为 $0$ 的齐次 Dirichlet 条件。若边界条件换成别的类型，留下来的模态也会变；例如两端绝热的齐次 Neumann 条件 $X'(0)=X'(L)=0$ 会选出余弦模式。若只是边界值非零，比如 $u(0,t)=0, u(L,t)=U_0$，通常不是直接改用余弦基，而是先写 $u=w+v$，让 $w$ 负责非零边界，$v$ 回到两端为 $0$ 的齐次 Dirichlet 边界；这时 $v$ 仍然用正弦基展开。

#### 为什么会有一串函数

为什么不是一个函数，而是一串函数？因为一个连续系统通常不只有一种允许形状。两端固定的细杆可以只鼓起一次，也可以中间多一个节点，还可以有更多起伏：
...
这些模式都满足两端为 $0$，也都经过二阶导数以后保持自己的形状。区别只是频率不同、弯曲程度不同、对应的特征值不同。

一个复杂的初始温度分布，通常不可能只靠其中一个模式表示出来。就像一个普通向量不一定只沿着一个坐标轴，它往往要由多个基向量合成；一个函数也往往要由多个基本函数合成。
...
这些系数不是随便配的，而是由初始函数 $f(x)$ 算出来。对这个两端为 $0$ 的正弦展开，公式是
...
这个公式的意思是：拿初始形状 $f(x)$ 去和第 $n$ 个正弦模式比较，看它们有多“对得上”。对得越上，积分越大，$b_n$ 就越大；完全对不上，$b_n$ 就可能是 $0$。这就是函数版本的“求坐标”。

这里的 $b_n$ 就像坐标。$b_1$ 表示第一个低频模式占多少，$b_2$ 表示第二个模式占多少，$b_3$ 表示第三个模式占多少。换句话说，初始条件决定了这一串系数；这一串系数又决定了每个候选特征函数在最终解里有多大权重。某个 $b_n$ 大，说明第 $n$ 个模式在初始状态里很明显；某个 $b_n=0$，说明这个模式没有被激发出来。

#### 分离后的每个方向都有自己的方程

空间方向不是全部。分离变量法真正做的是把多变量问题拆成几个单变量问题。每个方向都会得到自己的方程：空间方向负责找“允许的空间形状”，时间方向负责找“这些形状怎样随时间变化”；在圆域、球域这类问题里，还可能进一步拆出径向方程、角向方程。初始条件或边界条件再决定这些基本形状的权重。
...
所以分离出来的各个方向其实用的是同一套思想：都在找被相应算子作用以后形状不变、或者能被单独求解的基本函数。在这个热方程例子里，空间算子选出正弦形状，时间方程选出指数形状。
...
再由初始条件算出 $b_n$，决定每个模式的大小，最终解才写成

$$
u(x,t)=\sum_{n=1}^{\infty}b_n e^{-a^2(n\pi/L)^2t}\sin\frac{n\pi x}{L}.
$$

这里 $b_n$ 来自初始条件，正弦函数来自空间算子和边界条件，指数函数来自时间方程；它们合在一起，才组成完整的温度场。

#### 物理图像：固有模态的叠加

物理上，这些特征函数就是系统的固有模态。弦振动里，它们是基本振型；热传导里，它们是基本温度形状；声腔、电磁腔、量子系统里，也会出现类似的模态。复杂现象不是一团没有结构的复杂，而是许多简单模态的叠加。

每个模态有自己的空间形状，也有自己的时间命运：热方程里，高频模态衰减得快；波方程里，模态会振荡；量子问题里，模态会对应能级。

#### 这套图像的边界

不是所有 PDE 都能漂亮地变成“求一组基”。这种图像最适合线性方程、规则区域和自伴随算子。遇到强非线性、复杂边界、冲击波、湍流或者解不够光滑的问题，事情会困难得多，可能需要弱解、能量估计、数值方法或动力系统方法。但在第一遍学习数学物理方程时，特征函数级数仍然是最重要的入口之一，因为它把 PDE 的数学结构和物理图像都说清楚了。

所以，分离变量法真正教给我们的不是“热方程答案长这样”，而是一种看待线性 PDE 的方式：先把空间方向和时间方向都拆成各自最自然的基本函数，再用初始条件决定这些基本函数的权重。空间算子和边界条件给出空间模式，时间方程给出时间模式，初始条件给出模式系数；三者合在一起，就是完整的场。

## 五、行波法和特征线法：沿传播方向求解 PDE

前面一章的分离变量法，是把 PDE 拆成空间模式和时间模式。行波法和特征线法走的是另一条路：它们不先问“这个场由哪些模式叠加而成”，而是问“这个场里的信息沿着哪条路传播”。

这一章主要讲两件事。第一，行波法怎样从“假设一个形状在平移”出发，求解一维波方程。第二，特征线法怎样把一阶 PDE 沿着某些曲线化成 ODE。

这里先抓住一个共同原则：很多 PDE 解法都不是一上来凭空得到答案，而是先提出一个结构假设，再代回方程验证。分离变量法假设变量可以分离，比如 $u(x,t)=X(x)T(t)$；行波法假设解本身就是沿空间移动的波，比如 $u(x,t)=F(x-ct)$。这两个假设都不能只靠感觉成立，最后都要代回 PDE、边界条件和初始条件里检查。能通过检查，才说明这个假设真正给出了解。

### 先认识几种基本的波

在讲行波法之前，先把“波”的几种说法分开。这里最容易混乱的地方是：我们平时说的“机械波、电磁波、行波、驻波、简谐波、脉冲波”，并不是同一种分类表里的并列项目。它们回答的是不同问题。

一个波至少可以从三个角度看：第一，什么物理量在振动；第二，这个振动怎样运动；第三，这个波形长成什么样。三个角度叠在一起，才是一个具体的波。

#### 按物理机制分：什么东西在振动

按物理机制分，先不要列太细，抓住两个大类就够了：机械波和电磁波。机械波需要介质，例如弦波、声波、水波、地震波、弹性波；电磁波不需要机械介质，它是电场和磁场的振荡传播。

![按物理机制给波分类](assets/wave-physical-types.svg)

这张图想表达的是第一层区别：有些波必须依托介质传播，这就是机械波；有些波不需要机械介质，而是电场和磁场自身的振荡传播，这就是电磁波。弦波、声波、水波、地震波都可以放在机械波下面理解：弦上的位移在振动，空气里的压强和密度在振动，水面在起伏，地壳里有弹性扰动。

所以这一层最好先记成：机械波靠介质传播，电磁波靠电磁场自身传播。声波、水波、地震波只是机械波下面更具体的例子。

#### 按运动方式分：行波和驻波

按运动方式分，重点是“波形有没有整体搬家”。如果同一个形状沿着空间传播，就是行波；如果节点固定，整个形状只在原地振荡，就是驻波。

![按运动方式给波分类](assets/wave-motion-types.svg)

这张图要按时间顺序看。左边行波的几条曲线，是同一个波形在几个不同时刻的位置：形状基本不变，只是整体往右走。右边驻波的几条曲线，则不是往右走，而是同一批节点固定不动，节点之间的波腹从最高变矮，经过拉平，再反向变成波谷。
...
这里 $F$ 是一个形状，$c$ 是传播速度。随着 $t$ 增大，想让 $F(x-ct)$ 看到同一个函数值，就要让 $x$ 也增大，所以这个形状向右走。
...
这里 $\sin x$ 决定空间形状，$\cos ct$ 决定时间振荡。凡是 $\sin x=0$ 的地方，不管时间怎么变，$u$ 都等于 $0$，这些点叫节点。驻波图里蓝线和红线看起来很像“平移了半个波长”，这是因为对单一正弦函数来说，$-\sin x$ 和 $\sin(x+\pi)$ 的图像形状一样。但这只是两个特殊时刻的快照相似，不是实际运动方式相同。驻波从 $\sin x$ 变到 $-\sin x$ 的过程中，会先变矮，经过整条弦几乎拉平的状态，再反向变高；节点一直不动。行波则不同，$\sin(kx-\omega t)$ 会随着时间连续平移，任意相位都可以出现，节点、波峰、波谷都会沿着 $x$ 方向移动。

驻波常常可以由两个方向相反、速度相同的行波叠加出来：

$$
\frac12\sin(x-ct)+\frac12\sin(x+ct)=\sin x\cos ct.
$$

所以驻波和行波不是完全无关的两类东西。有限长弦、管乐器里的空气柱、电磁腔里的场模式，都常常因为边界反射形成驻波。前面分离变量法得到的很多波方程解，其实就是一串驻波的叠加。

#### 按波形分：简谐波和脉冲波

按波形分，重点是“这一团扰动长什么样”。最规则的正弦或余弦形状叫简谐波；只在局部出现的一团扰动叫脉冲波。

![按波形形状给波分类](assets/wave-shape-types.svg)

简谐波说的是形状规则，不一定单独说明它怎样运动。图里下面那条时间曲线，严格说是“固定一个位置，看一列简谐行波经过时的信号”。如果这个正弦形状在空间中传播，可以写成

$$
u(x,t)=A\sin(kx-\omega t+\phi).
$$

这时它既是简谐波，也是行波。这里 $A$ 是振幅，$k$ 是波数，$\omega$ 是角频率，$\phi$ 是初相位。波数 $k$ 描述的是空间方向上振荡得有多密：$k$ 越大，同样长度里塞进的波峰、波谷越多，波长越短。如果波长记作 $\lambda$，也就是相邻两个波峰之间的距离，那么

$$
k=\frac{2\pi}{\lambda}.
$$

角频率 $\omega$ 描述的是时间方向上振荡得有多快。$k$ 和 $\omega$ 不是随便配的：$k$ 说空间周期，$\omega$ 说时间周期，它们之间的关系由具体的波方程或介质决定。对最简单的一维波方程来说，代入 $\sin(kx-\omega t)$ 会得到
...
这叫色散关系。它告诉我们：空间里振得越密，时间上也振得越快。对这个简谐行波来说，传播速度是

$$
c=\frac{\omega}{k}.
$$

脉冲波说的是扰动局部集中。比如绳子上被拨出的一下凸起，如果它向右传播，也可以写成 $F(x-ct)$，所以很多脉冲波也是行波。但脉冲波不是驻波：脉冲波的特点是“局部形状跑过去”，驻波的特点是“节点固定，原地振荡”。

最后把关系收束成一句话：机械波、电磁波这些是物理分类；行波、驻波是运动方式；简谐波、脉冲波是形状分类。一个具体例子往往会同时落在多个分类里。比如“向右传播的正弦声波”，按物理机制是声波，按运动方式是行波，按波形是简谐波。

### 行波法是不是一种猜解

可以这么理解：行波法确实先假设解有某种结构，然后代回方程检查这个结构能不能成立。这一点很像 ODE 里解常系数线性方程时猜 $y=e^{rx}$，也有点像待定系数法。

但它不是乱猜。对波方程来说，物理图像已经告诉我们：扰动可能以某个速度向左或向右传播，而且传播时形状不一定马上改变。于是我们先尝试一种最简单的传播形状：

$$
u(x,t)=F(x-ct).
$$

这里要分清两个函数。$F$ 本身就是一个普通的一元函数，它描述某个固定的空间形状，比如一团凸起、一段正弦波。可是 PDE 的解 $u(x,t)$ 要同时告诉我们“在位置 $x$、时间 $t$，这个场的值是多少”，所以它是一个二元函数。行波写成 $u(x,t)=F(x-ct)$，意思不是说没有函数，而是说：这个二元函数其实只通过一个组合变量 $x-ct$ 来取值。

换句话说，$F$ 是形状，$x-ct$ 是这个形状随时间平移的方式。每固定一个时间 $t$，$u(x,t)$ 就变成一条关于 $x$ 的曲线；不同的 $t$ 给出同一条曲线的不同位置。

为什么 $x-ct$ 表示向右传播？因为如果要让 $F(x-ct)$ 的输入保持不变，就必须有
...
这说明同一个形状随着时间向右移动，速度是 $c$。同理，

$$
u(x,t)=G(x+ct)
$$

表示一个向左传播的形状。这里为什么换成 $G$，而不是还写 $F$？不是因为左右行波一定长得不同，而是因为一般情况下方程不要求它们相同。$F$ 和 $G$ 是两个待定的一元函数，最后要由初始位移和初始速度一起决定。

可以看两个具体例子。设初始位移是一团形状 $f(x)$。

第一种情况：只是把弦拉出这个形状，然后静止释放，也就是
...
也就是说，$F$ 和 $G$ 确实来自同一个初始形状，只是各拿一半。

第二种情况：初始速度正好配合这个形状向右平移，例如
...
也就是说，$F=f$，$G=0$，只有向右传播的那一列。为了同时覆盖“左右相同”和“只向一边传播”这些情况，我们写成 $F(x-ct)+G(x+ct)$，而不是一开始强行规定左右两边必须是同一个函数。

![初始条件决定左右行波](assets/wave-left-right-decomposition.svg)

所以行波法的第一步可以概括为：根据方程的传播直觉，先假设解是一个沿空间平移的形状，再代回 PDE 验证。
...
两边正好相等。这说明只要 $F$ 足够光滑，任何形状 $F(x-ct)$ 都是波方程的解。
...
也满足波方程。又因为波方程是线性的，两个解可以相加，于是得到

$$
u(x,t)=F(x-ct)+G(x+ct).
$$

这就是一维波方程最核心的行波结构：一个一般的波，可以看成一列向右走的波和一列向左走的波叠加起来。
...
但 $F$ 和 $G$ 还没有确定。接下来要用初始条件把它们求出来。

先看一维波动方程的柯西问题。这里“波动方程”指 PDE 本身；“柯西问题”不是另一种方程，而是指在整个空间上给定初始形状和初始速度，然后求解这个波动方程：
...
这里 $\varphi(x)$ 是初始形状，$\psi(x)$ 是初始速度。注意它们都是函数，不是两个数。因为波方程描述的是一整条线上的位移场：$u(x,0)=\varphi(x)$ 告诉我们每个位置 $x$ 一开始在哪里，$u_t(x,0)=\psi(x)$ 告诉我们每个位置 $x$ 一开始正以多大速度运动。

为什么要给两个初始函数？因为波方程对时间是二阶的，类似牛顿方程 $x''(t)=F/m$。只知道初始位置不够，还要知道初始速度。区别在于：ODE 里的初始位置和初始速度通常是两个数；波方程里的位置和速度是一整条线上的状态，所以都是关于 $x$ 的函数。

把行波形式代入初始条件。由 $t=0$ 得到
...
$$
F'(x)=\frac12\varphi'(x)-\frac1{2c}\psi(x),
\qquad
...
因为 $F(a)+G(a)=\varphi(a)$，所以要把 $G(a)$ 变成 $G(b)$。根据微积分基本定理，函数值的变化等于导数的积分：

$$
G(b)-G(a)=\int_a^b G'(s)\,ds.
$$

这里的 $s$ 只是一个临时积分变量。不要写成 $dx$，因为外面的 $x$ 已经固定了，表示我们正在求位置 $x$、时间 $t$ 处的 $u(x,t)$。积分是在初始位置区间 $[a,b]$ 上扫一遍，$s$ 表示这段区间里的一个中间位置。于是

$$
F(a)+G(b)=F(a)+G(a)+\int_a^bG'(s)\,ds.
$$
...
F(a)+G(b)
=\varphi(a)+\int_a^b
\left(\frac12\varphi'(s)+\frac1{2c}\psi(s)\right)ds.
$$
...
$$
F(a)+G(b)
=\frac12\varphi(a)+\frac12\varphi(b)
+\frac1{2c}\int_a^b\psi(s)\,ds.
$$

把 $a=x-ct$、$b=x+ct$ 代回去，得到
...
这个公式叫达朗贝尔公式。它不是一开始背出来的，而是从左右行波 $F(x-ct)$、$G(x+ct)$ 和两个初始函数 $\varphi,\psi$ 推出来的。

### 达朗贝尔公式在说什么

达朗贝尔公式可以读成一句话：当前位置 $(x,t)$ 的值，只受初始区间
...
$$
\frac12\bigl[\varphi(x-ct)+\varphi(x+ct)\bigr]
$$

来自初始形状。它表示一开始的形状分成两份，一份向右传播，一份向左传播。
...
来自初始速度。它表示初始速度不是只影响一个点，而是通过从 $x-ct$ 到 $x+ct$ 这一段区间累积起来，影响当前点。

### 和分离变量法得到的结果一致吗

波方程也可以用分离变量法解。只是分离变量法最自然的版本，通常用在有限区间上，比如一根长度为 $L$、两端固定的弦：
...
这时没有两端边界，波不会在端点反射，解直接表现成向左、向右传播的行波：
...
这两种解法并不矛盾。它们是在不同问题设置下看同一个波方程：有限区间上，边界反射会让左右行波叠加成驻波，所以分离变量法很合适；无限直线上，没有边界反射，行波图像最直接。

如果把无限直线上的初始函数用 Fourier 变换分解成很多频率，每个频率也会按波方程演化；把这些频率再叠加回来，就能得到达朗贝尔公式。反过来，在有限区间上，达朗贝尔公式配合奇延拓、周期延拓，也能和正弦级数解对应起来。所以结果本质上一致，只是表达方式不同：分离变量法强调模态，达朗贝尔公式强调传播。

### 特征线法的核心直觉

行波法主要处理波方程这类传播问题。特征线法可以看成这种思想在一阶 PDE 里的系统版本：它先假设存在一些合适的路线，使得我们沿着这些路线看 $u$ 的变化时，原来的 PDE 可以变成 ODE。

这里先强调范围：这一节讲的是**一阶 PDE** 的特征线法。二阶波方程、热方程、Laplace 方程当然也有自己的求解方法，但这里的“沿曲线把 PDE 化成 ODE”，最直接、最标准的入口是一阶方程。
...
这个方程的意思是：$u$ 这个量以速度 $c$ 被搬运。特征线法的核心假设是：也许存在一条路线 $x=x(t)$，也就是在这条特征线上把原来独立的空间变量 $x$ 看成依赖于时间 $t$ 的函数。这样一来，$u(x,t)$ 就变成了沿路线观察到的一元函数 $U(t)=u(x(t),t)$；原来同时包含 $\partial u/\partial t$ 和 $\partial u/\partial x$ 的偏微分方程，也就有机会化成只含一个自变量的普通微分方程。换句话说，特征线法最重要的动作，是把“在 $(x,t)$ 平面上解 PDE”，变成“沿某条曲线解 ODE”。

这和分离变量法、行波法有相似处，也有区别。相似处在于，它们都先抓住一种可能的结构，然后代回方程，看这种结构能不能成立；更重要的是，它们都想把原来的 PDE 尽量化成 ODE。分离变量法假设解可能拆成 $X(x)T(t)$，代回去以后得到空间方向和时间方向的 ODE；行波法假设解可能是某个形状平移，比如 $F(x-ct)$，代回去以后也会得到关于一个变量的 ODE；特征线法假设存在一条跟着信号走的路线 $x=x(t)$，沿着这条路线 PDE 会降成 ODE。

但它不是直接猜 $u$ 的形状。它猜的是“观察路线”的形状。也就是说，我们不是一上来就说 $u$ 一定等于某个函数，而是说：如果有一条路线能跟着被搬运的量一起走，那么沿这条路线看到的 $u$ 应该会更简单。
...
这里 $U(t)$ 是一个一元函数，因为路线一旦选定，位置 $x$ 就由时间 $t$ 决定了。

大白话说，原来我们像站在岸边看河水：固定在某个位置 $x$，看 $u$ 随时间怎么变。但输运方程描述的是“东西被搬走了”，所以更自然的办法是设想一条跟着这团东西走的路线。站着不动，会看到这个位置的 $u$ 变了；跟着东西走，才可能一直盯着同一份信息。

特征线法的核心可以用一句话概括：先假设存在一条跟着信号走的路线，再用 PDE 本身决定这条路线应该怎么走。

![特征线上的链式法则](assets/characteristic-chain-rule.svg)

这张图用同一组编号把左边和右边对应起来：1 是真实沿路线走的一小步，也就是 $U(t)=u(x(t),t)$ 的总变化；2 是计算时拆出来的时间贡献；3 是计算时拆出来的空间移动贡献。红色和绿色箭头不是说真实运动分成两段，而是说同一个小步的变化量可以分成两部分来算。为什么可以这样拆？因为 $u(x,t)$ 是一个同时依赖位置和时间的函数。位置和时间都只变化一点点时，多元微积分告诉我们，函数的一阶变化可以近似写成“时间造成的变化”加上“位置造成的变化”。这件事就叫**全微分**，也是后面链式法则的来源。图里为了避免突然引入简写，直接写成 $\partial u/\partial t$ 和 $\partial u/\partial x$。以后如果看到 $u_t$、$u_x$，它们只是偏导数的简写：$u_t=\partial u/\partial t$，$u_x=\partial u/\partial x$。

先看一个很小的变化量：

$$
du\approx \frac{\partial u}{\partial t}dt
...
$$
du\approx \frac{\partial u}{\partial t}dt
+\frac{\partial u}{\partial x}x'(t)dt.
$$

把右边的 $dt$ 提出来，再两边除以 $dt$，得到

$$
\frac{dU}{dt}
=\frac{d}{dt}u(x(t),t)
=\frac{\partial u}{\partial t}
+x'(t)\frac{\partial u}{\partial x}.
$$

现在让这条路线真正成为“特征线”。原来的 PDE 里出现的是
...
这一步就是特征线法的关键：不是先知道特征线再推方程，而是假设存在一条路线，然后让 PDE 告诉我们这条路线的速度应该是多少。这个 ODE 解出来是
...
第一个 ODE 告诉我们特征线怎么走；第二个 ODE 告诉我们沿这条线 $u$ 的值不变。因为 $U(t)$ 不变，所以

$$
U(t)=U(0)=u_0(x_0).
$$

对平面里的一个点 $(x,t)$，经过它的特征线满足
...
这就是输运方程的解。它说得很清楚：初始形状 $u_0(x)$ 没有被抹平，也没有被扩散，只是整体以速度 $c$ 向右移动。

具体看一个例子。假设一开始的场是一个集中在原点附近的隆起：

$$
u_0(x)=e^{-x^2}.
$$

它表示在 $t=0$ 时，$x=0$ 附近的值最大，离原点越远值越小。代入刚才的公式
...
这就是这个输运问题的具体场。固定一个时刻 $t$ 看，它还是同一个高斯形状；只是峰值位置从 $x=0$ 移到了 $x=ct$。固定一个位置 $x$ 看，$u(x,t)$ 会先变大再变小，因为这个隆起从这个位置经过了。整个过程中没有扩散项，所以它不会像热方程那样越摊越平。

当然，特征线法不是说任何一阶 PDE 都能这样顺利解决。更一般的一阶 PDE 里，特征线可能弯曲，可能相交，也可能让解在某些地方变得不光滑。这里先看常系数线性输运方程，是因为它把特征线法的核心想法表现得最干净：假设路线存在，用链式法则沿路线看变化，再让 PDE 决定路线。

### 输运方程和 Monge-Lagrange 特征方程

这一节先不急着讲一般解法，而是看清楚两个方程为什么像。输运方程是最简单的一阶特征线方程；Monge-Lagrange 特征方程则是把这种结构推广到更一般的一阶 PDE。

#### 从输运方程看结构

输运方程前面已经完整推过，这里不再重复解法，只把它的结构抽出来。它最重要的形式是：

$$
1\cdot \frac{\partial u}{\partial t}
+c\cdot \frac{\partial u}{\partial x}=0.
$$

这个式子有两个关键信息。第一，它不是孤立地规定某一个偏导数，而是在规定时间方向和空间方向偏导数的一个线性组合。第二，这个组合不是随便来的；前面通过链式法则已经看过，只要沿着满足 $dx/dt=c$ 的路线走，沿线导数就由两个偏导数组合而成，而且这个组合正好就是 PDE 左边。

所以在这里，输运方程真正提供的是一个样板：如果 PDE 的最高阶部分是几个偏导数的线性组合，我们就可以尝试找一条路线，让这条路线上的普通导数和这个组合对应起来。Monge-Lagrange 特征方程就是沿着这个样板往一般情形走。

#### 从输运方程变成 Monge-Lagrange 方程

现在把输运方程里的限制一点点放开。

在输运方程里，速度 $c$ 是常数，传播方向也很简单：在 $(x,t)$ 平面里沿直线走。更一般的一阶 PDE 里，方向不一定固定，速度也不一定是常数；它可以随着位置变化，甚至可以随着未知量 $u$ 本身变化。
...
+b(x,t,u)\frac{\partial u}{\partial t}
=d(x,t,u).
$$

这里并没有把时间丢掉，只是把输运方程里的常数系数 $1,c$ 变成了可以随 $x,t,u$ 变化的函数，右边也从 $0$ 变成了可能非零的项。

不过在讲 Monge-Lagrange 特征方程时，教材常把两个自变量统一记成 $x,y$，因为它不一定非要把第二个变量解释成时间。于是同一个结构也常写成
...
这里的 $x,y$ 只是两个自变量的名字，$u=u(x,y)$ 是未知函数。第二个变量 $y$ 可以是空间变量，也可以在具体问题里扮演时间变量。注意右边的 $c(x,y,u)$ 只是一个常用记号，不要和输运方程里的常数速度 $c$ 混为一谈。

这两个方程的相似处在于：它们都不是孤立地看某一个偏导数，而是在看若干偏导数的线性组合。输运方程里的组合系数是常数 $1$ 和 $c$；Monge-Lagrange 方程里的组合系数变成了函数 $a(x,y,u)$ 和 $b(x,y,u)$，右边也可以不是 $0$。

#### Monge-Lagrange 特征方程是什么
...
那么 $u(x,y)$ 的图像就是三维空间里的一张曲面。这里的 $z$ 不是新多出来的未知量，$z$ 就是 $u$ 的值，只是换了一个几何说法：原来我们说“在点 $(x,y)$ 处函数值是 $u(x,y)$”，现在说“在空间点 $(x,y,z)$ 里，曲面的高度是 $z$”。这样做的目的，是把求函数的问题几何化：求 $u(x,y)$，就变成找一张曲面 $z=u(x,y)$。

接下来在这张曲面上取一条曲线。沿着这条曲线走时，底面上的位置 $x,y$ 会变；因为点始终落在曲面 $z=u(x,y)$ 上，所以高度 $z$，也就是函数值 $u$，也会随之变化。于是可以写成
...
这里的 $dz$ 就是沿着这条曲线走一小步时，曲面高度 $z$ 的微小变化。既然曲线在三维空间里走，它当然同时有 $dx$、$dy$、$dz$ 三个方向的变化。

Monge-Lagrange 特征方程要做的，就是在曲面上找一族这样的曲线，使得沿曲线的普通导数正好对应上 PDE 里的偏导数组合。这里的 $a,b,c$ 仍然是前面那些函数，只是沿着曲线看时，它们也会变成随参数 $s$ 变化的量：
...
只是这组 ODE 的压缩写法。它不是说 $a,b,c$ 变成了常数分母，而是在省略同一个参数 $ds$：如果 $dx=a\,ds$、$dy=b\,ds$、$dz=c\,ds$，那么形式上就写成上面这种比例关系。下一节会把这个比例形式展开成真正的 ODE 系统。

所以，从输运方程到 Monge-Lagrange 特征方程，变化的是复杂度：直线变成曲线，常数速度变成可变速度，沿线不变变成沿线按某个 ODE 变化。但不变的是核心结构：沿合适的曲线看，PDE 可以化成 ODE。

#### 和 Monge 问题不是一回事

这里要特别注意，Monge-Lagrange 特征方程说的是一阶 PDE 的特征线方法，不要和最优传输里的 Monge 问题混成一件事。它们名字里都可能出现 Monge，但这里讨论的是怎样沿特征曲线求解 PDE；最优传输里的 Monge 问题讨论的是怎样把一个分布搬到另一个分布，后面讲数学物理方程和机器学习应用时再说。

### 特征线的一般解法

更一般地，很多一阶线性或拟线性 PDE 可以写成
...
这类方程的结构是：左边是 $u$ 在 $x$、$y$ 两个方向上的偏导数组合，右边给出这个组合应该等于什么。特征线法要做的，就是找一条曲线，使得沿曲线的普通导数正好等于这个偏导数组合。

这里有一个内在假设：我们假设这样的曲线可以用一个参数 $s$ 来描述。也就是说，先不急着把 $y$ 写成 $x$ 的函数，也不急着把函数值单独看成另一个自变量，而是让
...
分别依赖于同一个隐藏参数 $s$。这里 $U(s)$ 表示沿着这条曲线看到的函数值，也就是 $u(x(s),y(s))$。这样写的好处是，$x,y,U$ 在形式上先各自跟着 $s$ 变化，彼此之间的关系暂时不直接写死；它们真正的联系，是通过同一条曲线和函数关系 $U(s)=u(x(s),y(s))$ 建立起来的。于是，原来在平面或曲面上同时变化的量，就被压到一条曲线上观察。

换句话说，特征线法背后确实有一个“换坐标”的假设：我们希望在原来的平面里找到一族合适的曲线，用这族曲线重新组织变量之间的变化关系。这里的“一族曲线”，不是指孤零零的一条线，而是指很多条同类曲线：通常从不同的初始点出发，每个初始点对应一条特征线；这些特征线一起覆盖我们关心的区域。

说得准确一点，它不是把整个二维平面粗暴地压成一条线，而是先沿某一条特征线观察变化；在这条线上，原来同时依赖多个变量的 $u(x,y)$，就变成了只依赖一个参数 $s$ 的 $U(s)$。把许多条这样的特征线合在一起，才重新组织出整个区域里的解。于是 PDE 里某个特别方向上的偏导数组合，就可以改写成沿曲线的普通导数。微分几何里的曲线坐标也是这个思想：坐标不一定非要是一横一竖的直线，也可以顺着曲线、流线、曲面上的自然方向来取。

它和隐函数求导有一点相似，但不是同一件事。隐函数求导里，我们知道一个约束关系，比如 $F(x,y)=0$，于是可以沿着这条曲线求 $y$ 怎么随 $x$ 变。特征线法里，约束关系换成了 PDE 本身；我们要找的不是普通平面曲线，而是让 PDE 左边那串偏导数组合变成普通导数的曲线。找到以后，原来要同时看几个方向的偏微分，就变成了沿这条曲线解一个 ODE。

把未知函数的图像看成一张曲面。几何上可以把曲面高度写成 $z=u(x,y)$；为了和前面的记号统一，下面沿特征线时把这个高度记成 $U(s)$。
...
其中 $s$ 是沿着曲线走的参数。由链式法则，

$$
\frac{dU}{ds}
=\frac{\partial u}{\partial x}\frac{dx}{ds}
+\frac{\partial u}{\partial y}\frac{dy}{ds}.
$$

现在希望这条曲线真的成为特征线。原来的 PDE 里有
...
为了看清楚这条曲线应该怎么选，先不要急着写 $a,b$。假设曲线在 $x$ 方向和 $y$ 方向的速度分别是
...
现在问题就很清楚了：要让沿曲线的普通导数正好对应 PDE 里的这个偏导数组合，就应该让两个系数分别对上：
...
这不是从 $ds/dx$ 变形来的，也不是把分母倒过来算出来的；它是通过“比较系数”选出来的。$s$ 是沿曲线走的参数，$dx/ds$ 表示沿曲线走时 $x$ 方向的变化速度，$dy/ds$ 表示 $y$ 方向的变化速度。我们把这两个速度选成 PDE 里 $\partial u/\partial x$、$\partial u/\partial y$ 前面的系数，链式法则才会和原方程对上。

#### 沿特征线走一步：位置和函数值怎样更新

这里的迭代是在逐步计算一条特征线，以及这条线上各个位置的函数值。假设起点在平面里的位置是 $(x_0,y_0)$，初始数据给出了 $U_0=u(x_0,y_0)$。把这三个数代入系数函数 $a,b,c$，就能算出起点处的三个变化率：

$$
\left.\frac{dx}{ds}\right|_{s=s_0}=a(x_0,y_0,U_0),
\qquad
\left.\frac{dy}{ds}\right|_{s=s_0}=b(x_0,y_0,U_0),
\qquad
\left.\frac{dU}{ds}\right|_{s=s_0}=c(x_0,y_0,U_0).
$$

这就像在这个点放了一支小箭头。箭头的平面方向是

$$
(a(x_0,y_0,U_0),\,b(x_0,y_0,U_0)),
$$

前两个分量告诉我们平面上的位置往哪里走、走多快；$c(x_0,y_0,U_0)$ 告诉我们沿着这条路线走时，函数值怎样变化。这里 $s$ 是沿曲线的参数，不一定是物理时间。取一个小步长 $\Delta s$，用当前的变化率估算下一步的位置和函数值：

$$
x\approx x_0+a(x_0,y_0,U_0)\Delta s,
\qquad
y\approx y_0+b(x_0,y_0,U_0)\Delta s,
\qquad
U\approx U_0+c(x_0,y_0,U_0)\Delta s.
$$

到达新点后，再把新点的 $x,y,U$ 代入 $a,b,c$，算出新的变化率，继续走下一步。这是在对特征 ODE 做数值积分，上面的近似更新称为显式 Euler 法。连起这些计算点，就得到特征线及沿线函数值的近似。

例如，对于 $2\frac{\partial u}{\partial x}+\frac{\partial u}{\partial y}=3$，三个变化率分别是 $2,1,3$。从已知点 $(x_0,y_0,U_0)=(0,0,1)$ 出发，取 $\Delta s=0.1$，第一步得到 $(0.2,0.1,1.3)$，第二步得到 $(0.4,0.2,1.6)$。每一步算的是沿线下一个位置上的函数值。这个例子的变化率恰好是常数，因此这些更新点正好落在精确特征曲线上；一般情况下会有数值误差。

一个起点只能给出一条线上的信息。从初始曲线上的不同点分别出发，就能算出一族特征线及各自携带的函数值。当这些线能在局部不重叠地覆盖一片区域时，就能把沿线结果还原成这片区域里的场 $u(x,y)$。如果特征 ODE 能直接积分，也可以直接写出沿线的解，不必做小步迭代。

它与梯度下降有相似的更新形式，但计算目的不同：梯度下降调整变量，是为了让目标函数变小；这里是在沿着 PDE 指定的方向推进位置和函数值。$U$ 可以增大、减小或保持不变，方向 $(a,b)$ 也不必来自某个函数的负梯度。
...
也常写成比例形式。这里要记住，$a,b,c$ 是沿曲线取值的函数，不是固定常数；这个写法只是把共同的参数 $ds$ 省略掉：

$$
\frac{dx}{a}=\frac{dy}{b}=\frac{dU}{c}.
$$

这就是很多教材里说的 Lagrange 特征方程，或者 Monge-Lagrange 特征方程。它的意思并不神秘：原来 PDE 在整张曲面上约束偏导数；现在沿着某些特殊曲线走，问题就变成了曲线上的 ODE。

### 特征线法的例子

先看一个比输运方程稍微一般一点、但还能手算的例子。只给 PDE 本身还不够，因为那样通常只能求出含任意函数的通解；要得到一个具体解，还需要给定初始条件或边界条件。这里考虑下面这个问题：
...
这里 $u(1,y)=g(y)$ 不是时间意义上的初始条件，而是在直线 $x=1$ 上给出的边界数据。它的作用和 ODE 里的初值类似：用来从一族可能的解里挑出一个具体解。

几何上也可以把 $z=u(x,y)$ 看成曲面高度；但为了符号统一，沿特征线仍然把函数值记成 $U(s)$。这个方程对应
...
$$
\frac{y}{x}=\frac{C_2}{C_1}.
$$

同时 $U/x$ 也不变，因为

$$
\frac{U}{x}=\frac{C_3}{C_1}.
$$

因此解可以写成

$$
\frac{U}{x}=\Phi\!\left(\frac{y}{x}\right),
$$

也就是

$$
u(x,y)=x\Phi\!\left(\frac{y}{x}\right).
$$

这里的 $\Phi$ 是任意函数，需要由条件决定。现在使用题目给出的边界条件
...
$$
u(x,y)=xg\!\left(\frac{y}{x}\right).
$$

这就是一个完整的特征线法例子：先把 PDE 写成特征 ODE，再从 ODE 里找沿特征线不变的量，最后用条件确定任意函数。

#### 输运方程作为特殊例子

输运方程正是这个框架的最简单情形。把变量看成 $(x,t)$，方程
...
这三句话分别表示：时间正常往前走，位置以速度 $c$ 移动，沿着这条移动路径 $u$ 不变。因为 $dt/ds=1$，可以直接把 $t$ 当成参数，于是
...
不过要注意，它不是说“任何一阶 PDE 都能这样顺利解完”。这个形式最直接对应的是上面这种一阶线性或拟线性方程：最高阶导数 $u_x,u_y$ 是一次出现的，前面的系数可以依赖 $x,y,u$。如果方程是更一般的全非线性形式，比如 $F(x,y,u,u_x,u_y)=0$，仍然有特征的思想，但公式会更复杂，通常要用 Charpit 方法或 Hamilton-Jacobi 理论。即便是一阶拟线性方程，特征线也可能相交、解可能产生间断，或者边界条件刚好给在特征方向上导致问题不适定。所以特征线法是理解和求解一阶 PDE 的核心工具，但不是对所有一阶 PDE 都自动给出全局光滑解的万能方法。

### 特征线法怎样重新组织平面

前面说“沿一条特征线看”，只是第一步。真正要解出一片区域里的 $u$，不能只靠一条线，而要靠一族特征线。这里有两个参数要分清楚：

- 一个参数用来标记“是哪一条特征线”；
- 另一个参数用来表示“在这条特征线上走到了哪里”。

比如从一条初始曲线出发，可以用 $r$ 标记初始曲线上的点。每一个 $r$ 对应一个出发点，也就对应一条特征线。然后再用 $s$ 表示沿着这条特征线前进的距离或时间。于是原来的平面坐标 $(x,y)$，就可以改用 $(r,s)$ 来描述：
...
这里 $r$ 负责区分不同的特征线，$s$ 负责沿着同一条特征线移动。这样看就更接近曲线坐标了：原来的坐标网格是一横一竖，现在新的坐标网格是一族特征线，加上横穿这些特征线的初始标记。

如果这个对应关系足够好，也就是不同的 $(r,s)$ 不会混到同一个点，并且这些曲线确实覆盖了我们关心的区域，那么就可以把平面上的问题换成特征线坐标里的问题。沿着每一条特征线，$u$ 只随 $s$ 变化，所以 PDE 变成 ODE；不同的 $r$ 给出不同的特征线，最后把所有特征线上的结果拼起来，就得到整个区域里的解。

用输运方程看最清楚。初始线取 $t=0$，用 $\xi$ 标记初始位置：
...
这就是一次很具体的“换坐标”：旧坐标是 $(x,t)$，新坐标是 $(\xi,s)$。其中 $\xi$ 表示这条特征线从哪里出发，$s$ 表示沿着这条特征线走了多久。反过来，由 $t=s$ 和 $x=\xi+cs$ 得到
...
这说明 $U$ 沿着这条线不变。也就是说，这条特征线从初始点 $\xi$ 出发时，$u$ 的值是 $u_0(\xi)$；走到后来的点 $(x,t)$ 时，值还是 $u_0(\xi)$。而刚才已经算出
...
这句话的意思是：想知道时刻 $t$、位置 $x$ 的值，就往回沿着特征线追到初始时刻。它在初始时刻的位置是 $x-ct$，所以现在的值等于初始函数在 $x-ct$ 那里的值。

所以特征线法不是凭空把 PDE 变简单，而是找到了适合这个方程的曲线坐标。在这套坐标里，沿特征线方向的变化由 ODE 控制；横向的参数只负责标记不同的特征线。

### 如果特征线假设失败怎么办

现在再回头看“失败”是什么意思，就应该从一般解法的假设说起。特征线法的基本假设不是简单地写一句 $x=x(t)$，而是更一般地假设：存在一族曲线，可以用同一个参数 $s$ 表示成
...
沿着这样的曲线，链式法则可以把偏导数组合改写成普通导数。如果这一步能做成，PDE 就被化成 ODE；如果做不成，特征线法就会遇到麻烦。

第一种失败，是特征方向本身退化。一般的一阶拟线性方程可以写成

$$
a(x,y,u)\frac{\partial u}{\partial x}+b(x,y,u)\frac{\partial u}{\partial y}=c(x,y,u).
$$

特征线的方向来自 $\partial u/\partial x$、$\partial u/\partial y$ 前面的系数，也就是
...
如果在某些点上 $a,b$ 同时变成 $0$，那就相当于方程没有告诉我们特征线应该往哪个方向走。方向场在那里停住了，特征线没法正常穿过去。
...
在普通点上，这个方向很好理解：如果 $(x,y)=(2,1)$，特征线就大致沿着 $(2,1)$ 这个方向走；如果 $(x,y)=(-1,3)$，就沿着 $(-1,3)$ 这个方向走。但在原点 $(0,0)$，方向变成了 $(0,0)$。这就像站在原点时速度表显示“速度为零”，没有告诉你下一步该往左、往右、往上还是往下。

更麻烦的是，把 $(0,0)$ 代回原方程，左边直接变成 $0$，可是右边是 $1$，得到

$$
0=1.
$$

这说明这个方程在原点根本不相容：不是我们不会算，而是这个点附近的方程结构本身出了问题。实际做题时，遇到这种退化点，通常要把它单独拿出来分析，或者看边界条件、物理背景能不能补充信息。

第二种失败更严重：特征线存在，但不同特征线后来相交。比如非线性输运方程里，传播速度可能依赖于 $u$ 本身。值大的地方跑得快，值小的地方跑得慢，快的特征线就可能追上慢的特征线。

一旦两条特征线撞到同一个点，就会出现麻烦：沿第一条特征线追过去，可能给这个点一个值；沿第二条特征线追过去，又可能给这个点另一个值。一个点不能同时有两个不同的函数值，所以经典光滑解就不能继续保持。这时解往往会形成间断，也就是所谓激波。再往后就要引入弱解、熵条件这些概念，用来决定间断以后哪个解才是物理上合理的。

所以，特征线法确实有一点隐函数求导的味道：它不是把所有变量都孤立摊开看，而是在 PDE 约束下找一条曲线，让 $x,y,U$ 都通过同一个参数 $s$ 变化，再沿这条曲线求导。它成功的前提，是这样的参数曲线足够好、特征方向不退化、特征线不在我们关心的区域里制造矛盾。遇到困难时，通常要么补充条件，要么换解的概念，要么换求解方法。

### 行波法和特征线法的区别

行波法和特征线法关系很近，但侧重点不同。行波法通常先假设解具有 $F(x-ct)$、$G(x+ct)$ 这样的传播形状，再代回方程验证并确定未知函数；它最典型地用于波动方程。特征线法更强调沿某些曲线把 PDE 化成 ODE，尤其适合一阶方程和输运问题。

可以粗略地说：行波法看的是“形状怎样传播”，特征线法看的是“沿哪条路线追踪这个量最简单”。在简单输运方程里，这两件事几乎是同一件事；在更复杂的一阶拟线性方程里，特征线会弯曲，甚至可能相交，进而产生冲击波等现象。

### 高维波方程和半空间问题

三维和二维波动方程也有类似思想，只是公式更复杂。三维情形里，一个点的值和以它为中心、半径为 $ct$ 的球面上的初始信息有关；二维情形里，会涉及圆盘内部的信息。这说明波的影响不是瞬间传遍全空间，而是按照有限速度向外传播。

半空间问题常用延拓法处理。比如只在 $x>0$ 上求解，并且边界 $x=0$ 固定为 $0$，可以把初始函数延拓成奇函数；如果边界导数为 $0$，则常用偶延拓。这样可以把半空间边界问题转化成全空间问题，再用达朗贝尔公式或其它全空间公式求解。

### 小结：模态观点和传播观点

分离变量法和行波法不是互相排斥的。它们只是从不同角度看同一个 PDE。

分离变量法偏向模态观点：先找系统允许的空间模式，再看每个模式怎样随时间变化。行波法偏向传播观点：直接追踪扰动怎样沿空间移动。对有限长弦，两端反射会自然形成驻波，分离变量法特别合适；对无限长弦，左右传播的行波图像最直接。

所以以后看到一个 PDE，不要急着问“套哪个公式”，而要先看它的结构：如果边界整齐、模态明显，优先想到分离变量；如果传播方向清楚、特征线明显，优先想到行波法或特征线法。

## 六、PDE 求解方法的工具箱

前面已经把分离变量法、行波法和特征线法作为两条主线讲过了。接下来再看 PDE 求解方法的工具箱：空间无限延伸时想到 Fourier 变换；时间初值很突出时想到 Laplace 变换；方程里有源项时想到 Green 函数；区域和边界复杂时，往往要转向有限元、有限差分、有限体积等数值方法。

下面这些方法先作为地图来讲。重点不是一次把所有技术细节都推到底，而是知道它们各自解决什么问题，和前面的分离变量法、行波法有什么关系。
...
## 十三、最优控制：变分法、Pontryagin 最大值原理和 HJB 方程
