...
行波法主要处理波方程这类传播问题。特征线法可以看成这种思想在一阶 PDE 里的系统版本：它要找一些曲线，让 PDE 沿着这些曲线变成 ODE。
...
这个方程的意思是：$u$ 这个量以速度 $c$ 被搬运。沿着直线

$$
x(t)=x_0+ct,
$$

现在不要把 $u$ 看成单纯的 $u(x,t)$，而是沿着这条运动路线去看它：

$$
u(x(t),t).
$$

这已经变成了一个只关于 $t$ 的复合函数。现在只有一个问题：当我们沿着路线走时，$u(x(t),t)$ 怎样随 $t$ 变化？

这里没有两种观察方式，只有一种观察方式：跟着 $x(t)$ 走。只不过在这一小段时间 $dt$ 里，同时发生了两件事：时间从 $t$ 变成 $t+dt$，位置从 $x(t)$ 变成 $x(t)+dx$。所以沿路看到的总变化要把这两部分都算进去。

![特征线上的链式法则](assets/characteristic-chain-rule.svg)

这张图里，蓝线是观察者真正走的路线。红色箭头表示时间增加 $dt$ 带来的贡献，绿色箭头表示同一小段时间里位置移动 $dx$ 带来的贡献。两部分合起来，才是沿蓝线看到的总变化。
...
+x'(t)\frac{\partial u}{\partial x}\right)dt.
$$
...
这里的 $u$ 是沿着路径看的 $u(x(t),t)$，所以更完整地写就是
...
- 第一部分是时间贡献。即使位置先不动，时间从 $t$ 变到 $t+dt$，也会产生变化量 $(\partial u/\partial t)dt$，除以 $dt$ 后就是 $\partial u/\partial t$。
- 第二部分是空间移动贡献。沿路径走时，位置还会移动 $dx=x'(t)dt$，空间移动带来的变化量是 $(\partial u/\partial x)dx$，除以 $dt$ 后就是 $x'(t)\partial u/\partial x$。

这两部分加起来，才是跟着路径 $x(t)$ 看到的总变化率。对这条特征线来说，$x'(t)=c$，所以
...
这一步就是“把 PDE 化成 ODE”的核心。原来的输运方程

$$
\frac{\partial u}{\partial t}+c\frac{\partial u}{\partial x}=0
$$

同时涉及 $x$ 和 $t$ 两个方向上的偏导数。特征线法的想法，是不要在整张 $(x,t)$ 平面上同时看，而是找一条路线 $x=x(t)$，沿着这条路线看 $u$ 的变化。

大白话说，原来我们像站在岸边看河水：固定在某个位置 $x$，看 $u$ 随时间怎么变。但输运方程描述的是“东西被搬走了”，所以更自然的看法是跳到这团东西上，跟着它一起走。

如果你站着不动，当然会看到这个位置的 $u$ 变了，因为原来的东西流走了，新的东西流过来了。可是如果你跟着速度 $c$ 一起走，就一直盯着同一团东西。

特征线法的核心可以用一句话概括：找一条跟着信号走的路线，让沿路的变化变成一元函数的变化。

这句话里有三层意思。

第一，为什么要“跟着信号走”？因为输运方程里的 $u$ 不是在原地自己变形，而是在被速度 $c$ 搬运。站在固定位置看，会把“东西流过来、流过去”误读成这个位置的数值变化；跟着信号走，才是在追踪同一份信息。

第二，为什么会变成一元函数？原来的 $u(x,t)$ 依赖两个变量：位置 $x$ 和时间 $t$。一旦选定一条路线 $x=x(t)$，位置就不再是独立变量，而是由时间决定。于是沿路看到的值变成

$$
u(x(t),t),
$$

它只剩下一个自变量 $t$。这就像沿着山路走看海拔：整张地形图是二维函数，但你沿一条路线走时，看到的海拔高度只随行走时间变化。

第三，为什么这能把 PDE 化成 ODE？因为沿路线的变化率可以用普通导数表示：

$$
\frac{d}{dt}u(x(t),t).
$$

如果路线选得对，这个普通导数正好等于原来 PDE 里的偏导数组合。于是“在整张平面上满足的偏微分关系”，就变成了“沿一条曲线满足的常微分关系”。

第一步，路线怎么选？这一步确实有点像“让系数对上”，但不是待定系数法里的随便猜。理由来自刚才的链式法则。沿一条路线 $x=x(t)$ 看 $u$，总导数会长成
...
要让沿路线的总导数和 PDE 里的这个组合完全对应，就应该选
...
这里 $U(t)$ 是沿路径看到的值，所以它是一个普通的一元函数。由链式法则，
...
这就是第二个 ODE。合起来看，原来的 PDE 变成了下面这个 ODE 系统：
...
第一个 ODE 告诉我们沿哪条线走；第二个 ODE 告诉我们沿这条线 $u$ 的值不变。这句话的意思是：如果你跟着速度 $c$ 一起走，就会发现沿路看到的 $u$ 值没有变化。也就是说，$u$ 不是消失了，而是被搬运过去了。所以沿着这条线，$u$ 的值保持不变。这条线就叫特征线。因为 $x=x_0+ct$，所以 $x_0=x-ct$，解就是
...
先说它们的关系：输运方程是特征线法最简单、最直观的例子；Monge-Lagrange 特征方程是处理一大类一阶 PDE 时，把这种思路系统写出来的形式。

也就是说，二者不是两个互不相干的东西。输运方程里，我们沿着直线 $x=x_0+ct$ 追踪 $u$；Monge-Lagrange 特征方程里，我们沿着更一般的曲线 $(x(s),y(s),z(s))$ 追踪未知曲面 $z=u(x,y)$。前者是最简单的特例，后者是这类一阶方程的系统写法。
...
可以看成特征线法最简单的样板。它告诉我们：不要站在固定位置 $x$ 上看 $u$ 怎么变，而是跟着速度 $c$ 一起走。沿着
...
所以输运方程的解法，本质上就是把 PDE 变成沿路径的 ODE。
...
$$
x=x(s),\qquad y=y(s),\qquad z(s)=u(x(s),y(s)),
$$
...
回头看输运方程，它正是这个框架的最简单情形。把变量看成 $(x,t)$，对应的特征 ODE 是
...
这三句话分别表示：时间正常往前走，位置以速度 $c$ 移动，沿着这条移动路径 $u$ 不变。解出来就是
...
这里要注意，Monge-Lagrange 特征方程说的是一阶 PDE 的特征线方法，不要和最优传输里的 Monge 问题混成一件事。它们名字里都可能出现 Monge，但这里讨论的是怎样沿特征曲线求解 PDE；最优传输里的 Monge 问题讨论的是怎样把一个分布搬到另一个分布，后面讲数学物理方程和机器学习应用时再说。
