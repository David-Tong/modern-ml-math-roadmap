...
在有边界的区域里，抛物型方程常常会把系统带向某个稳态。比如一块介质的边界温度被持续固定，内部温度一开始可以很乱；随着时间推进，内部会逐渐扩散、调整，最后不再变化。

到了不再变化的时候，左边的时间导数为 $0$：
...
也就是说，抛物型方程描述的是“走向平衡的过程”，椭圆型方程描述的是“已经达到平衡后的形状”。这正好把热方程和 Laplace 方程联系起来：一个讲时间演化，一个讲最终稳态。

### 双曲型方程：传播与波动

双曲型方程最重要的直觉是：扰动不会只是原地被抹平，而会以波的形式向外传播。
...
第一条告诉我们一开始弦是什么形状，第二条告诉我们每个位置一开始往哪里动、动得多快。这和牛顿力学很像：只知道位置还不够，还要知道速度，才能决定后面的运动。

#### 有限传播速度

双曲型方程和抛物型方程最大的区别之一，是有限传播速度。

热方程里，一个局部扰动会被扩散开，数学上常常会立刻影响到很远的地方，只是远处影响很小。波方程则更像真实的信号传播：扰动需要时间才能传到远处。如果波速是 $c$，那么距离很远的地方不会马上知道这里发生了什么。

这就是双曲型方程的“传播”味道。它不是把尖峰直接抹平，而是把扰动沿着空间传出去。声波、水波、弦上的波、弹性波，都有这种有限速度传播的图像。

#### 特征线和传播方向

在一维波方程里，最基本的传播结构可以写成：

$$
u(x,t)=F(x-ct)+G(x+ct).
$$

这表示波可以分成两部分：$F(x-ct)$ 向右传播，$G(x+ct)$ 向左传播。形状本身可以保持，但位置随时间移动。

这里的 $x-ct$ 和 $x+ct$ 就体现了传播方向。沿着
...
走，波形的某个特征会被保持下来。这类线就叫特征线。更复杂的双曲型方程里，特征线或特征方向会告诉我们信息沿着哪里传播。
...
这句话非常直观：初始形状 $u_0$ 没有被抹平，只是以速度 $c$ 向右移动。它强调的不是扩散，而是搬运。

所以双曲型方程的第一印象可以概括为：它描述扰动、信号和波的传播；信息有传播方向和传播速度；系统往往带有守恒、振荡和反射这些现象。
