...
简谐波说的是形状规则，不一定单独说明它怎样运动。如果它在空间中传播，可以写成

$$
u(x,t)=A\sin(kx-\omega t+\phi).
$$

这时它既是简谐波，也是行波。这里 $A$ 是振幅，$k$ 是波数，$\omega$ 是角频率，$\phi$ 是初相位，传播速度是
...
这里 $F$ 是某个形状，$c$ 是传播速度。为什么 $x-ct$ 表示向右传播？因为如果要让 $F(x-ct)$ 的输入保持不变，就必须有
...
所以沿着这条线，$u$ 的值保持不变。这条线就叫特征线。因为 $x=x_0+ct$，所以 $x_0=x-ct$，解就是
