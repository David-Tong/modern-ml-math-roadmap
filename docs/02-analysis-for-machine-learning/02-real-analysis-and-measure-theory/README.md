...
### 先定义一个函数：真实测度相对于模型测度的密度差异

设 $P$ 和 $Q$ 是定义在同一个可测空间 $(\Omega,\mathcal{F})$ 上的两个概率测度。可以把它们理解成两套给事件分配概率的规则：$P$ 是真实世界的规则，$Q$ 是模型的规则。

Radon-Nikodym 定理本质上是在测度意义下定义“微分”。普通微积分里，我们可以写

$$
\frac{df}{dx}
$$

表示函数 $f$ 相对于变量 $x$ 的变化率。更直观地说，先看一个很小的区间，计算

$$
\frac{\Delta f}{\Delta x}.
$$

当区间越来越小时，这个比值的极限就是 $\frac{df}{dx}$。它表示“每一单位 $x$ 的变化，会带来多少 $f$ 的变化”。

测度论里也是类似的想法。我们不再比较 $\Delta f$ 和 $\Delta x$，而是比较同一个小区域 $A$ 上两个测度给出的大小：

$$
\frac{P(A)}{Q(A)}.
$$

如果区域 $A$ 越缩越小，这个比值在极限意义下就对应

当然，这需要一个前提：$P$ 不能把质量放到 $Q$ 完全测不到的地方。直觉上，这就像普通分数里分母不能为 0：如果某个地方 $Q$ 给的质量是 0，但 $P$ 还给了正的质量，那么 $\frac{dP}{dQ}$ 就没法正常表示“$P$ 是 $Q$ 的多少倍”。数学上写作

$$
P\ll Q.
$$

在这个条件下，Radon-Nikodym 定理告诉我们，确实存在一个可测函数

$$
\frac{dP}{dQ}.
$$

它就是测度 $P$ 相对于测度 $Q$ 的导数。类比起来看：

- $\frac{df}{dx}$：每一单位 $x$ 对应多少 $f$ 的变化。
- $\frac{dP}{dQ}$：每一单位 $Q$ 的概率质量，对应多少 $P$ 的概率质量。

所以如果用 $Q$ 作为底层尺子来量，$\frac{dP}{dQ}$ 就是在告诉我们 $P$ 在各个地方相对于 $Q$ 有多密。

翻译成概率论语言：在某个很小的区域里，模型分布 $Q$ 觉得这里大概有多少概率，真实分布 $P$ 觉得这里大概有多少概率。$\frac{dP}{dQ}$ 比较的就是这两个概率的比例。
...
这里的 $h$ 是我们关心的某个函数，可以是损失函数、某个指标，或者一个统计量。比如我们真正想算的是
...
可以把它想成：我们真正关心的是 $P$，但 $P$ 太难直接抽样；于是先从一个好抽样的 $Q$ 里拿样本，再用权重把这些样本修正成像是从 $P$ 来的。
...
如果写成概率论里常见的密度形式，可以一步步展开。先从测度论的积分公式开始：

$$
\int h\,dP.
$$

它表示把函数 $h$ 按概率测度 $P$ 加权平均。在概率论里，这个东西通常写成
...
如果 $P$ 的密度是 $p(x)$，那么这个测度积分就可以写成普通积分：
...
而“某个函数乘上 $q(x)$ 再积分”，就是在 $Q$ 下取期望。所以
...
注意权重是 $\frac{p(X)}{q(X)}$，意思是“目标分布 $P$ 的密度除以采样分布 $Q$ 的密度”。这和测度写法里的 $\frac{dP}{dQ}$ 是同一个东西。
