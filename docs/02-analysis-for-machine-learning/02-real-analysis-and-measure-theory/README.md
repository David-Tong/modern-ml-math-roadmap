...
这一章只看一个机器学习里非常常见的指标：KL 散度，也叫相对熵。

它常出现在变分推断、生成模型、语言模型训练、信息论和概率模型里。表面上看，它是一个公式；从测度论角度看，它是在比较两个概率测度。

### 一句话概括

KL 散度衡量的是：当我们用一个近似分布 $Q$ 去描述真实分布 $P$ 时，平均会多损失多少信息。
...
如果 $Q$ 很接近 $P$，KL 散度就小；如果 $Q$ 在真实数据经常出现的地方给了很小概率，KL 散度就会变大。

翻译成测度论语言，$P$ 和 $Q$ 都是概率测度。KL 散度比较的是：两个概率测度对同一个空间分配概率的方式有多不一样。

### 预测类比：模型到底有多意外

可以把真实分布 $P$ 想成真实世界里事件发生的频率，把模型分布 $Q$ 想成模型给出的预测概率。

比如某种样本 $x$ 在真实世界里其实很常见，真实概率是
...
当这个样本真的出现时，真实世界并不意外，因为它本来就常见；但模型会非常意外，因为模型之前几乎没把它当回事。

KL 散度衡量的就是这种平均意义下的“模型意外程度”：真实世界按 $P$ 产生样本，而模型按 $Q$ 去理解这些样本，平均会多意外多少。

如果模型给出的概率 $Q$ 和真实概率 $P$ 很接近，它就不会额外意外，KL 散度接近 0；如果模型总是在真实常见的地方给很低概率，KL 散度就会变大。

### 第一步：单个事件的意外程度

先从一个事件 $x$ 看起。

在信息论里，事件发生概率越小，它发生时越让人意外。这个“意外程度”常用

$$
-\log p(x)
$$

表示。概率越小，$-\log p(x)$ 越大；概率越大，$-\log p(x)$ 越小。
...
这说明 $0.01$ 概率的事件发生时，比 $0.9$ 概率的事件发生更“意外”。
...
这就是 KL 散度公式里 $\log\frac{p(x)}{q(x)}$ 的来源。它衡量的是：对单个事件 $x$，用模型 $Q$ 描述真实世界 $P$ 时，多出来的信息代价。
...
如果 $q(x)<p(x)$，说明模型低估了真实事件发生的概率；当这个事件真的发生时，模型会更“意外”，额外代价也会变大。

### 第三步：对真实世界取平均

单个事件的额外代价还不够。我们要衡量模型 $Q$ 整体描述真实世界 $P$ 的效果，就要把所有事件的额外代价平均起来。

这个平均应该按谁来加权？应该按真实分布 $P$。因为真实世界里经常发生的事件，如果模型预测错了，影响更大；真实世界里很少发生的事件，影响相对小。
...
表示用模型分布 $Q$ 描述真实分布 $P$ 的平均代价；而

$$
H(P)=-\sum_x p(x)\log p(x)
$$

表示用真实分布 $P$ 自己描述自己的平均代价。两者相减，就是使用近似模型 $Q$ 多花的那部分代价。
...
对数的底决定单位。以 $2$ 为底，单位是 bit；以自然对数为底，单位是 nat。

翻译成测度论语言，$p(x)\,dx$ 就是真实概率测度 $P$。所以 KL 散度不是普通的函数差，而是按真实概率测度加权后的平均额外信息损失。

### 测度论怎么改写 KL 散度

现在把上面的概率论公式搬到测度世界里。

前面讲密度函数时说过，密度本质上是“一个测度相对于另一个测度的变化率”。如果比较的是两个概率测度 $P$ 和 $Q$，那么

$$
\frac{dP}{dQ}
$$

可以理解成“真实概率测度 $P$ 相对于模型概率测度 $Q$ 的密度比”。它在问：在某个位置附近，真实分布给的概率质量，相对于模型分布给的概率质量，大多少。
...
- $\frac{dP}{dQ}$：比较两个概率测度的局部比例。
- $\log\frac{dP}{dQ}$：把比例变成代价。
- $dP$：按照真实概率测度 $P$ 做平均。

这就是 KL 散度的测度论核心：它不是简单比较两个函数，而是在用真实概率测度 $P$，去平均“真实测度相对于模型测度的密度比”带来的代价。

### 为什么机器学习会用它

先用概率论语言说：很多机器学习问题都可以理解成，我们不知道真实分布 $P$，于是用模型分布 $Q$ 去逼近它。

比如生成模型希望模型生成的数据分布接近真实数据分布；语言模型希望给真实文本更高概率；变分推断希望用一个容易处理的分布去近似复杂后验分布。

这时 KL 散度就很自然：它给了一个衡量“$Q$ 描述 $P$ 描述得有多差”的指标。

如果真实数据常出现的地方，模型也给了高概率，那么 KL 散度会比较小。如果真实数据常出现的地方，模型却给了很低概率，那么 $\log\frac{p(x)}{q(x)}$ 会变大，KL 散度也会变大。

翻译成测度论语言，就是：训练模型时，我们在调整模型概率测度 $Q$，让它尽量接近真实概率测度 $P$。KL 散度给了一个比较这两个概率测度的方式。

这也是为什么很多训练目标会和 KL 散度、交叉熵、最大似然联系在一起。它们背后都在做同一件事：让模型给真实数据更合理的概率。

### 顺便看一个例子：重要性采样

重要性采样也是深度学习和概率机器学习里经常出现的东西。它和 KL 散度一样，也会自然用到“两个概率测度之间的密度比”。

先用概率论语言说。

假设我们想算某个函数 $h(X)$ 在真实分布 $P$ 下的期望：

$$
\mathbb{E}_{P}[h(X)].
$$

如果能直接从 $P$ 采样，那就简单：采很多个样本，算平均就行。但有时候 $P$ 很难采样，或者直接从 $P$ 采样效率很低。于是我们改从另一个更容易采样的分布 $Q$ 里抽样。

问题是：样本来自 $Q$，但我们想算的是 $P$ 下的平均。怎么办？

重要性采样的想法是：用一个权重把 $Q$ 下的样本修正回 $P$ 下的样子。如果 $P$ 和 $Q$ 有密度 $p(x)$ 和 $q(x)$，那么
...
这句话的大白话是：虽然样本是从 $Q$ 来的，但每个样本要乘上一个修正权重

$$
\frac{p(X)}{q(X)}.
$$

如果某个样本在真实分布 $P$ 下更重要、但在采样分布 $Q$ 下不那么常见，这个权重就会变大；反过来，如果它在 $Q$ 下被采得太频繁，权重就会把它压回去。
...
长得很像。它们都在说同一件事：当我们在两个概率测度之间切换时，需要知道一个测度相对于另一个测度的密度比。

### 一个需要注意的地方

先用概率论语言说：KL 散度虽然常被叫作“距离”，但它不是普通意义上的距离。
...
也就是说，用 $Q$ 描述 $P$ 的代价，和用 $P$ 描述 $Q$ 的代价，不一定一样。

翻译成测度论语言，这点也很自然：公式里是对 $P$ 做积分，

$$
\int \log\frac{dP}{dQ}\,dP,
$$

所以它特别关心 $P$ 认为重要的地方。如果换成 $D_{\mathrm{KL}}(Q\|P)$，权重就变成了 $Q$，关注重点也会变。

### 小结

KL 散度是一个很好的例子，说明测度论为什么对机器学习有用。

它把几个概念连在一起：

- 概率测度：$P$ 和 $Q$ 描述两个概率世界。
- 密度：$p(x)$、$q(x)$ 或更一般的 $\frac{dP}{dQ}$ 描述两个概率测度的相对变化。
- Lebesgue 积分：用真实概率测度 $P$ 对代价做平均。

所以 KL 散度不是一个凭空出现的机器学习公式。它本质上是在测度论语言里比较两个概率测度，并用积分把这种差异变成一个整体指标。
