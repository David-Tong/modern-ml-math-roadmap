# 普通工科生也能看懂的实变函数和测度论：用测度重新理解积分和概率
...
8. Radon-Nikodym 导数为什么是测度论里的“密度”。
9. 这些概念怎样连接到机器学习中的 KL 散度和重要性采样。
...
测度论本身最初不是为了概率论发明的，而是为了修补传统积分和“长度、面积”概念的不足。19 世纪末到 20 世纪初，数学家发现很多集合和函数已经超出了几何直觉能处理的范围：有些集合到处都是但总长度为 0，有些函数处处不连续，有些极限过程会把原来好好的函数变得很坏。Lebesgue 的工作把“区间长度”推广成了“测度”，再用测度重新定义积分，这才有了更稳定的 Lebesgue 积分。
...
设 $P$ 是真实概率测度，$Q$ 是模型给出的概率测度。KL 散度想回答的问题是：如果真实世界按照 $P$ 发生，但我们用 $Q$ 去描述它，平均会多付出多少信息代价？

### Radon-Nikodym 导数：测度之间的密度

先回到一个更熟悉的对象：概率密度 $p(x)$。它不是 $X=x$ 的概率，而是概率沿着数轴铺开时，在 $x$ 附近每单位长度上有多少概率质量。真正有概率意义的是积分：

$$
P_X(A)=\int_A p(x)\,dx.
$$

为什么要用积分表达式？因为概率测度 $P_X$ 的输入是集合，输出是这个集合的概率；单个数值 $p(x)$ 不是概率，不能直接回答“$X$ 落在集合 $A$ 里的概率是多少”。只有把 $A$ 上的局部密度按长度累加起来，才得到真正的概率。

这就是 Radon-Nikodym 导数出现的地方。上面的公式说明：概率测度 $P_X$ 可以写成“某个函数乘上长度测度 $dx$”。如果 $P_X$ 不会把正概率放到 Lebesgue 测度为 0 的集合上，也就是 $P_X\ll dx$，Radon-Nikodym 定理保证这样的函数存在。这个函数就是

$$
p(x)=\frac{dP_X}{dx}.
$$

这个定理的历史来源也很朴素：Radon 在 1913 年研究什么时候一个集合函数可以写成对 Lebesgue 测度的积分；Nikodym 在 1930 年把这个思想推广到一般测度空间。换句话说，他们关心的是：一个测度能不能表示成“另一个测度乘上某个密度函数”。

一般地，如果测度 $\nu$ 不会把质量放到 $\mu$ 看不见的地方，也就是 $\nu\ll\mu$，那么在合适条件下存在可测函数 $f$，使得

$$
\nu(A)=\int_A f\,d\mu.
$$

这个 $f$ 就叫 $\nu$ 相对于 $\mu$ 的 Radon-Nikodym 导数，记作

$$
f=\frac{d\nu}{d\mu}.
$$

它在测度论上的意义是：把“两个测度之间的关系”变成一个函数。函数可以积分、取对数、加权、优化，所以这个转换非常重要。概率密度 $p(x)$、重要性采样里的权重 $\frac{dP}{dQ}$、KL 散度里的 $\log\frac{dP}{dQ}$，本质上都来自这个概念。

### 用 $Q$ 作底尺比较 $P$

回到 KL 散度。我们要衡量“用 $Q$ 描述 $P$”的代价，所以先把 $Q$ 当成参照尺，问：真实测度 $P$ 相对于模型测度 $Q$ 有多密？这个密度比就是

$$
\frac{dP}{dQ}.
$$

它不是普通函数相除，而是下面这个测度关系：

$$
dP=\frac{dP}{dQ}\,dQ.
$$

这句话的意思是：如果以 $Q$ 作为底层测度，那么 $\frac{dP}{dQ}$ 告诉我们 $P$ 在各个地方相对于 $Q$ 的密度。这个导数存在需要一个条件：

$$
P\ll Q.
$$

也就是说，$P$ 不能把正概率放到 $Q$ 完全测不到的地方。直觉上，如果某个地方 $Q$ 认为概率是 0，但 $P$ 认为会发生，那么用 $Q$ 描述 $P$ 就会出大问题；这也是 KL 散度可能变成 $+\infty$ 的原因。

如果 $\frac{dP}{dQ}\approx 1$，说明模型和真实分布在这里差不多；如果 $\frac{dP}{dQ}\gg 1$，说明真实分布在这里给的概率远大于模型，模型低估了这里；如果 $\frac{dP}{dQ}\ll 1$，说明模型高估了这里。

KL 散度接下来做一件事：对这个密度比取对数，把“概率质量相差多少倍”变成“信息代价差多少”：
...
这个函数常叫 **relative information** 或 **information density**，也可以理解成对数似然比。它在单个结果 $\omega$ 上比较 $P$ 和 $Q$ 的判断差异：值为 0 表示两者差不多；值很大表示模型低估了真实世界中重要的结果；值为负表示模型高估了这个结果。

在离散情形下，这就是熟悉的概率比。如果真实概率是 $p(x)=0.9$，模型只给 $q(x)=0.01$，那么
...
有了局部信息差 $\log\frac{dP}{dQ}$ 以后，KL 散度就是把它按真实测度 $P$ 做平均：
...
这里看起来好像又从 $Q$ 回到了 $P$，但这不是矛盾。前面 $\frac{dP}{dQ}$ 是用 $Q$ 作底尺来表达 $P$；现在对 $P$ 积分，是因为 KL 问的是“真实世界 $P$ 下，平均信息代价是多少”。

如果想完全用 $Q$ 作底尺，也可以把 $dP$ 换成 $\frac{dP}{dQ}\,dQ$：

$$
D_{\mathrm{KL}}(P\|Q)
=\int \frac{dP}{dQ}\log\frac{dP}{dQ}\,dQ.
$$

所以这两种写法是同一件事：

- $\int \log\frac{dP}{dQ}\,dP$ 强调“按真实分布 $P$ 取平均”。
- $\int \frac{dP}{dQ}\log\frac{dP}{dQ}\,dQ$ 强调“以模型分布 $Q$ 为底尺来表达”。

机器学习里更常用前一种说法，因为它更直接表达了 KL 的含义：模型在真实世界会出现的地方，平均错得多贵。真实世界经常出现的地方权重大，真实世界很少出现的地方权重小，$P$ 认为概率为 0 的地方不影响这个平均。

虽然 $\log\frac{dP}{dQ}$ 局部可以为负，但 KL 散度整体总是非负的：

$$
D_{\mathrm{KL}}(P\|Q)\ge 0.
$$

直觉上，用真实测度自己描述自己不会比用错误模型更差。最理想的情况是

$$
D_{\mathrm{KL}}(P\|Q)=0.
$$

这表示 $Q$ 和 $P$ 在概率意义下完全一致，模型没有额外信息损失。若 KL 散度很大，说明模型在真实测度 $P$ 很重视的地方给了太小的概率；若某些 $P$ 会发生的地方，$Q$ 直接给了 0 概率，那么 KL 散度甚至可能是 $+\infty$。
...
上面的测度论定义看起来比较抽象，但它退回到概率论里，就是我们熟悉的 KL 散度公式。

如果是离散概率分布，$P$ 和 $Q$ 在点 $x$ 上的概率分别是 $p(x)$ 和 $q(x)$，那么 Radon-Nikodym 导数就对应成概率比：
...
但只能从 $Q$ 采样，那么就用同一个 Radon-Nikodym 导数做修正：
...
这里的 $\frac{p(X)}{q(X)}$ 就是 $\frac{dP}{dQ}(X)$。样本虽然来自 $Q$，但乘上这个权重以后，平均值就被修正回 $P$ 下的平均。

所以重要性采样和 KL 散度的共同点是：它们都依赖同一个 Radon-Nikodym 导数。
...
读到这里，不需要觉得自己已经“学完了实变函数和测度论”。这篇文章更像是给工科读者搭一座桥：先知道为什么要有测度、为什么要有 Lebesgue 积分、为什么概率论和机器学习里会不断出现对某个测度积分的公式。

如果读完以后，你再看到机器学习和深度学习里出现下面这种公式：
...
不会一头雾水，以为公式是不是写错了；也不会把所有积分都硬理解成“对区间长度 $dx$ 积分”，那这篇文章的目的就已经达到了。

因为现代概率论和机器学习里，积分经常不是在问“横轴上的面积是多少”，而是在问：

$$
\text{这个函数在某个测度给出的权重下，整体平均是多少？}
$$

当测度是长度测度 $dx$ 时，它就是微积分里熟悉的积分。当测度是概率测度 $P$ 时，它就是数学期望。当测度是经验测度 $P_n$ 时，它就是样本平均。当测度换成另一个概率测度时，就会出现重要性采样、KL 散度、分布距离这些概念。

### 可以继续深入的方向

后面如果想继续学，可以沿着几条线往下走。

第一条线是更严格的实变函数。这里会系统学习 $\sigma$-代数、可测函数、Lebesgue 测度、外测度、几乎处处、$L^p$ 空间、收敛定理等内容。这条线能帮助你真正理解“哪些函数可以积分”“极限和积分什么时候能交换”“为什么零测集可以忽略”。

第二条线是测度论概率。这里会把概率空间、随机变量、分布、条件期望、独立性、乘积测度、大数定律、弱收敛等概念放在测度论框架下重新讲。很多高等概率论、随机过程、统计学习理论都会默认你熟悉这套语言。

第三条线是泛函分析和 $L^p$ 空间。机器学习里很多误差、范数、正则化和函数空间观点，都和 $L^1$、$L^2$、$L^\infty$ 这些空间有关。比如均方误差本质上就是一种 $L^2$ 意义下的距离。

第四条线是信息论和统计学习。KL 散度、交叉熵、互信息、最大似然、变分推断、重要性采样，都可以用“概率测度 + Radon-Nikodym 导数 + 积分”这套语言统一起来。

第五条线是最优传输和生成模型。Wasserstein 距离把概率测度看成可以搬运的质量分布，问的是把一个概率测度搬成另一个概率测度需要多少成本。这条线和 WGAN、扩散模型、生成建模都有联系。

最后可以把本文的主线压缩成一句话：

$$
\text{Lebesgue 积分}=\text{按测度加权的整体平均}.
$$

理解了这句话，很多机器学习公式就不会显得神秘了。它们只是把“面积”换成了“按概率、按样本、按某个抽象测度加权的平均”。这也是实变函数和测度论对机器学习最直接的价值。
