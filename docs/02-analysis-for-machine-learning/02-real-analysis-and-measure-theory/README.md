...
所以这里的核心不是公式，而是一句话：先会算有限层函数，再用越来越精细的有限层函数逼近一般函数。每一个简单函数的积分都能直接算；当这些简单函数越来越贴近原函数时，它们的积分也会趋向一个极限。Lebesgue 积分就是把这个极限当成原函数的积分。
...
用一个简单函数从下方近似它。可以把 $f(x)=x$ 想象成一个从 0 慢慢升到 1 的斜面。我们现在把这个斜面改造成一节一节往上的台阶，每一节台阶都用这一段左端点的高度。台阶一定在斜面下面，但台阶越切越细，就越贴近原来的斜面。
