...
所以变分法和机器学习虽然方法不同、场景不同，但它们共享一个核心画面：不是只计算某个函数的值，而是在函数之间比较，寻找最好的那个函数。在本系列的新目录里，变分法已经和优化理论、凸分析合并到“最优化理论和变分法”一章中展开。本文则专注于泛函分析这块地基：函数应该放进什么空间，函数之间怎么比较，函数序列怎样收敛，算子为什么可能稳定，以及模型族为什么能逼近目标函数。
...
前面讲的东西，主要还是泛函分析里最经典、最基础的一条线：把函数看成空间里的点，然后给这个空间定义距离、范数、内积、完备性，再讨论算子、不动点、函数逼近和泛函表示。

这条线很重要，因为它是很多机器学习理论的底层语言。比如损失函数可以看成泛函，模型训练可以看成在函数空间里找一个好函数，最小二乘可以看成 Hilbert 空间里的正交投影，神经网络的表达能力可以放进函数逼近的框架里理解。

但这还只是入口。

更现代的泛函分析，不是简单地把更多名词往上堆，而是继续追问一些古典视角还没完全解决的问题。

第一个问题是：函数不够光滑怎么办？

前面我们说函数可以有范数、有距离、有内积。但很多真实问题里的函数并不那么听话。图像边缘可能突然跳变，物理场可能只有很弱的导数，偏微分方程的解也未必处处可导。那这时还能不能谈“导数”？还能不能谈能量？还能不能说一个函数是合理的解？

这就会引出 Sobolev 空间。它关心的不只是函数本身有多大，还会把函数的导数一起放进空间里度量。大白话说，Sobolev 空间是在问：这条函数不仅值要受控制，变化方式也要受控制。它常常出现在偏微分方程、图像处理、物理建模和深度学习里的平滑性分析中。

第二个问题是：无穷维里的“矩阵”应该怎么理解？

在线性代数里，我们研究矩阵怎么作用在向量上，研究特征值、特征向量、对角化。更准确地说，矩阵可以看成有限维空间里的线性算子：它输入一个向量，输出另一个向量。到了函数空间，核心对象仍然是算子，只是这个算子作用的对象从有限维向量变成了函数，而且很多时候不能再简单写成一个有限维矩阵。微分是算子，积分是算子，卷积可以看成算子，很多迭代更新也可以看成算子。

这时就会引出算子理论。它研究的是：一个算子是否连续？是否有界？是否稳定？是否可逆？它会不会把小误差放大？它反复作用以后会不会收敛？从机器学习角度看，很多训练过程、特征变换、卷积结构、注意力结构，都可以用“算子如何作用在函数或向量上”这个视角去理解。

第三个问题是：有限维里的特征值思想，到了无穷维还管不管用？

一个矩阵的特征值能告诉我们很多事：系统会不会爆炸，迭代会不会收敛，哪些方向变化最快，哪些方向最稳定。可是当对象变成无穷维空间上的算子时，事情就复杂了。特征值可能不再是有限个，甚至不一定像矩阵那样好找。

这就会引出谱分析。谱分析可以粗略理解成“无穷维版本的特征值分析”。它研究算子的谱结构，用来理解稳定性、振动模式、扩散过程、信号分解，以及很多迭代算法为什么会收敛或发散。

第四个问题是：只用一种收敛方式够不够？

前面我们主要谈的是范数收敛，也就是两个函数在某个范数下越来越近。但现代分析里经常会遇到一种情况：强收敛太严格，要求太高；可问题本身又需要某种“弱一点但仍然有用”的收敛概念。

于是就会出现弱收敛、弱拓扑、对偶空间这些概念。它们的大白话意思是：有时候我们不直接要求两个函数本身贴得非常近，而是要求它们在所有重要的测试规则下看起来越来越像。这个视角在优化、概率、偏微分方程和机器学习泛化理论里都很重要。

所以第十一章不是要把现代泛函分析的所有方向都写完。这里先启发几个问题：函数不光滑怎么办？无穷维算子怎么研究？特征值思想怎么推广？收敛方式能不能更灵活？

现代观点大概就是这样推进的：先遇到一个古典工具不够用的问题，再为它设计更合适的空间、更合适的范数或拓扑、更合适的算子语言，最后用这些结构去研究存在性、稳定性、逼近性和收敛性。

这些内容后面会单独展开。有机会可以再补一篇“现代观点下的泛函分析”，专门谈 Sobolev 空间、谱分析、算子理论、弱收敛和对偶空间这些方向。这里先记住一句话就够了：这篇文章讲的是泛函分析的入门骨架，不是泛函分析的全部；真正往后走，泛函分析会越来越像一套研究“空间、函数、算子和极限”的通用语言。
