...
这也是机器学习会关心微分几何的第一个入口。模型处理的样本常常被写成向量，但学习真正想抓住的，不只是这些向量坐标本身，而是数据能沿哪些有意义的方向变化。比如图像里的物体可以旋转、平移、改变光照，语言里的句子可以改变语气、主题和句法结构。微分几何提供的切空间、度量和曲率这些语言，正是在问：局部有哪些合法变化方向？这些方向之间怎样量距离？数据所在的形状是不是弯的？
...
放回机器学习语境，第一基本形式对应的是一个很朴素的问题：表示空间里的距离到底怎么量？

这里要先分清两层。很多模型会把图片、文本或用户行为映射成 embedding 向量。这个向量所在的外部空间，通常确实可以先看成普通欧氏空间。比如词向量、句向量、图像 embedding，常常就是某个高维向量空间里的点。从这个角度说，embedding 空间本身是“直的”。

但真正可能弯的，不一定是整个外部向量空间，而是数据在这个空间里实际占据的那一片区域。比如自然语言句子的 embedding 不会均匀填满整个高维空间，而是集中在某些有结构的区域附近；图像 embedding 也可能沿着姿态、光照、类别、风格这些有效变化方向形成弯曲结构。这个低维、有结构、可能弯曲的区域，才是“数据流形”想表达的对象。

所以问题不是“词向量空间是不是弯的”，而是：这些向量里真实有意义的变化，是否沿着某个弯曲的低维结构发生。如果是，普通欧氏距离就不一定等于语义距离。更合理的问题可能是：沿着数据自然变化的方向走，两个样本到底相隔多远？这就是“局部尺子”的思想在表示学习里的影子。
...
机器学习里也会遇到类似问题。比如生成模型常常有一个潜空间，我们会在两个潜变量之间做插值。最简单的做法是走直线，但这默认潜空间是平的，而且直线中间的点仍然落在合理样本附近。如果潜空间或数据流形本身是弯的，更自然的问题就变成：怎样沿着数据流形走，从一个样本自然地过渡到另一个样本？这时测地线提供的是一种几何直觉：不要只问外部坐标里的直线，而要问沿着模型认为“合理”的空间走，什么路径最短、最自然。
...
对机器学习来说，这条线后面可以继续通向很多问题：数据是否集中在低维流形附近，表示空间里的距离应该怎样定义，受约束参数应该怎样在切空间里更新，概率分布之间有没有更自然的几何距离，生成模型里的插值路径怎样更符合数据结构。这些都不是本文要展开的内容，但它们说明微分几何不是远离机器学习的抽象语言，而是在帮助我们重新理解“数据、模型和学习过程到底生活在什么空间里”。
