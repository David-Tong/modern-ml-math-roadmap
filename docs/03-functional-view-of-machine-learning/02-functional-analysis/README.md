...
有了“泛函”这个概念以后，很自然会继续问：如果我们总是在评价一整条函数，那这些函数本身应该放在哪里比较？函数之间有没有距离？有没有长度？一串函数越来越接近时，到底算不算收敛？

这就进入了泛函分析。泛函分析不是只研究“泛函”这个词，也不是只研究怎么求最优函数。更准确地说，它是在把函数当成空间里的对象以后，研究这个空间本身。  
如果说数学分析更常问的是：一个函数在某个点附近怎么变？它连续吗？可导吗？积分怎么算？极限是什么？  
那么泛函分析更常问的是：一整条函数能不能当成一个点？这些点组成的空间有没有距离、长度和角度？一列函数会不会收敛？一个操作把一个函数变成另一个函数时，会不会保持稳定？一个泛函评价整条函数时，有没有连续性、有没有最优值？  
所以数学分析更像拿放大镜看一条函数的局部变化；泛函分析更像把很多函数放到同一个空间里，看这个空间的结构。  
它的范围大概可以分成几块：函数空间、范数和内积、收敛和完备性、线性算子，以及把函数映射成数字的泛函。  
后面谈 Banach 空间、Hilbert 空间、Lebesgue 空间，其实都是在讲函数空间和度量方式；谈 Banach 不动点，是在讲空间里的操作为什么会稳定；谈函数逼近，是在讲我们能不能用一个函数空间里的函数去接近另一个函数。
...
$$
\operatorname{proj}_b(a)=\frac{\langle a,b\rangle}{\langle b,b\rangle}b
$$
...
$$
\|f\|_\infty=\operatorname*{ess\ sup}_{x\in X}\lvert f(x)\rvert
$$
...
$$
\operatorname*{ess\ sup}_{x\in X}\lvert e(x)\rvert
$$
