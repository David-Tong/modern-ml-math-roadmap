...
但高斯绝妙定理说：虽然这种理解方式借助了外部法向量，最后得到的高斯曲率 `K`，其实只依赖第一基本形式。
...
那么 `K` 可以由 `E`、`F`、`G` 以及它们的变化算出来。只是这个公式比较长，直接写出来会把重点带偏，所以这里不展开。

这并不是说“高斯曲率很容易算”。恰恰相反，只从第一基本形式直接算高斯曲率，通常比借助第二基本形式更麻烦。高斯绝妙定理真正强调的是：**计算可能复杂，但所需信息是内部的。**

可以用小圆周长来感受这个意思。这里的小圆不是画在外部空间里的圆，而是曲面内部的人这样做出来的：从一个点出发，沿曲面向所有方向都走同样远，把这些终点连起来。

在普通平面上，从一个点出发，沿所有方向都走同样的距离。假设这个距离是：
...
但在球面上，同样从一点出发，沿球面走同样的距离，得到的小圆周长会比平面公式短。这个偏差不需要站在球外观察，只要在球面上实际量距离和周长就能发现。

这和高斯曲率的关系是：高斯曲率描述的是曲面内部测量规则偏离平面的方式。
...
这并不奇怪。不同曲面可以选择最方便的坐标。坐标名字不重要，真正重要的是：它们都只有两个自由度。

平面上的点需要两个数确定；圆柱面上的点也需要两个数确定。对圆柱来说，一个数说明绕圆柱转到哪里，另一个数说明高度是多少。虽然圆柱放在三维空间里，但你一旦被限制在圆柱面上运动，就只有这两个独立方向可以走。

所以圆柱面可以理解成一种放在三维世界里的二维流形。外部空间是三维的，但圆柱面自身的自由度只有两个。

所以平面和圆柱都是二维曲面。它们都可以用两个参数来描述，也都会有两个基本切向量。这就是后面计算可以放在同一套框架里的原因。

坐标名字不重要，重要的是同一套步骤：先写出曲面在外部空间里的坐标表达，再算两个切向量、法向量，最后看二阶变化在法向量方向上的分量。
...
$$
r_{uu}=0,\quad r_{uv}=0,\quad r_{vv}=0
$$
...
这说明平面的第二基本形式是 `0`。从外面看，它没有弯出去。

再看圆柱。设圆柱半径是 `R`，用绕圆柱的角度和高度作坐标：
...
沿高度方向，法向量不变；沿圆周方向，法向量会转。直接看二阶变化：
...
所以圆柱的第二基本形式不是全零。它记录了这样一个事实：圆柱沿圆周方向向外弯，沿高度方向不弯。
...
这里 `E`、`F`、`G` 来自第一基本形式，`L`、`M`、`N` 来自第二基本形式。

这个公式要放在这里理解：第二基本形式描述外在弯曲，但高斯曲率不是第二基本形式的全部信息，而是第一、第二基本形式组成的一个特殊比例。下一章计算平面、圆柱面和球面时，就会直接用到它。

这点很关键：**圆柱面的高斯曲率是 `0`，但第二基本形式并不为 `0`。**

这和第一基本形式不矛盾。圆柱内部可以像平面一样量长度，所以高斯曲率是 `0`；但圆柱在外部空间里确实弯了，所以第二基本形式不是 `0`。
