...
曲面在一个点附近有很多方向可以走。你可以沿南北方向走，也可以沿东西方向走，也可以沿任意斜方向走。每选一个方向，就相当于用一个穿过法向量的平面去切曲面，切出来一条曲线。于是，曲面的弯曲可以先理解成：从不同方向切它，会看到不同的曲线曲率。
...
这两个方向对应的曲率，叫主曲率。通常记作：

$$
k_1,\quad k_2
$$

沿第一个主方向，法向量变化的速度由第一个主曲率控制；沿第二个主方向，法向量变化的速度由第二个主曲率控制。曲面上一小块面积经过“取法向量”这个操作以后，在两个主方向上分别被这两个倍率拉伸或压缩，所以总的面积倍率就是二者的乘积：

$$
K=k_1\cdot k_2
$$

所以“法向量扫过的面积 / 曲面原来的面积”和“两个主曲率的乘积”不是两套互不相关的说法。前者给出高斯曲率的几何图像，后者给出它在两个主方向上的计算方式。

比如平面没有弯。两个方向上的曲率都是 `0`，所以高斯曲率是：
...
圆柱面最能说明高斯曲率的特别之处。圆柱沿绕圈方向是弯的，但沿竖直方向是直的。也就是说，一个主曲率不是 `0`，另一个主曲率是 `0`。所以：
...
这样看，高斯曲率比曲线曲率更微妙。它不是单独看某一个方向的转弯，而是看曲面上一小块区域的法向量整体散开得多快；落实到计算上，就是看两个主方向的弯曲如何合在一起。它真正关心的是曲面内部几何和普通平面相比，到底有没有发生本质变化。
...
到这里，这篇文章想建立的古典微分几何直觉就差不多完整了：先用线元理解怎样沿曲线和曲面自己测量，再用第一基本形式把坐标变化换算成真实长度、角度和面积，然后用曲率描述这种局部平直近似怎样失效，最后用测地线回答弯曲空间里的最短路径问题。
